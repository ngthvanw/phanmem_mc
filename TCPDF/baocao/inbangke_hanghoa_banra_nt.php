...
foreach ($DATACIBANRACHUADC as $thuesuat=>$ItemData){
    if($thuesuat=="10" ||$thuesuat=="8") {
        foreach ($ItemData as $itemDuLieu) {
            $Tax = round(($itemDuLieu['thue']/$itemDuLieu['thanhtiennt'])*100);
            if ($Tax==8){
...
?>
<html>
<head><title>IN BẢN KÊ BÁN RA (Nhấn CTRL + P để in , ALT + F4 để thoát)</title>
        ...
        $(document).ready(function() {
            /*$('#xuatexcel').click(function(){
                $(".BangInExcel").table2excel({
                    ...
                    filename: "Bang_Ke_Ban_Ra_" + new Date().toISOString().replace(/[\-\:\.]/g, "") + ".xls",
                    fileext: ".xls",
                    exclude_img: true,
                    exclude_links: true,
                    exclude_inputs: true,
...
<table border="1" class="display dataTable" width="100%" border="0" cellspacing="1" cellpadding="1">
    ...
    <tr>
        ...
        <th   rowspan="2">Doanh số chưa có thuế(NT)</th>
        <th   rowspan="2">Thuế giá trị gia tăng(NT)</th>
    ...
    foreach ($DATACIBANRA as $k=>$itemTS) {
        ...
        foreach ($itemTS as $itemCT) {
            ...
            if((string)$k=="k" || (string)$k=="K"){
                ...
                $tongdoanhthucothue+= $itemCT["thanhtiennt"];
            }
            $tongdoanhthu += $itemCT["thanhtiennt"];
            $tongthue += 0;
            $tongdoanhthu_theothue += $itemCT["thanhtiennt"];
            $tongthue_theothue += 0;
            $html_ct .= '
        <tr >
        ...
        <td width="120px" class="td_center" style="text-align:right;">' . NB_Format($itemCT["thanhtiennt"]) . '</td>
        <td width="90px" class="td_center" style="text-align:right;">' . number_format(0, 0, ",", ".") . '</td>
    ...
    ';
        ...
        <td class="td_full"  style="text-align:right;"><b>' . NB_Format($tongdoanhthu_theothue) . '</b></td>
        <td class="td_full"  style="text-align:right;"><b>' . number_format(0,0,",",".") . '</b></td>
  ...
  <tr>
    ...
    <td colspan="1" align="right"><b>'.NB_Format($tongdoanhthucothue).'</b></td>
  </tr>
  <tr>
    <td colspan="5"><strong>Tổng thuế GTGT của hàng hóa, dịch vụ bán ra:</strong></td>
    <td colspan="1" align="right"><b>'.NB_Format($tongthue).'</b></td>
  </tr>
  <tr>
