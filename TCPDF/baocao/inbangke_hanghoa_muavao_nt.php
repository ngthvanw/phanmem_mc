        ...
        $(document).ready(function() {
            /*$('#xuatexcel').click(function(){
                $(".BangInExcel").table2excel({
                    ...
                    filename: "Bang_Ke_Mua_Vao_" + new Date().toISOString().replace(/[\-\:\.]/g, "") + ".xls",
                    fileext: ".xls",
                    exclude_img: true,
                    exclude_links: true,
                    exclude_inputs: true,
			...
			$(document).on('click','#xuatexcel',function(e) {
				...
				link.download = "Bang_Ke_Mua_Vao_" + new Date().toISOString().replace(/[\-\:\.]/g, "") + ".xls",
				link.href = result;
				link.click();
			});
...
$html_tt = '<table width="100%" border="0" style="font-size:13px;">
  <tr>
    ...
    <td colspan="3" WIDTH="20%" align="center" style="font-size:11px;">
		<i>Mẫu số: 01 -2/GTGT<br/>
...
<table border="0" align="center">
  ...
  <tr>
    <td colspan="11"><b>(Kèm theo tờ khai thuế GTGT mẫu số 01/GTGT ngày ' . $ngaylap . ')</b></td>
  </tr>
  <tr>
    <td colspan="11"><b>[01] Kỳ tính thuế: ' . $_SESSION["THONGTINPHIEU"]['ngayhoadon'] . '</b></td>
  </tr>';
if ($loaibangke == "0") {
    $html_tt .= '<tr>
    <td><b colspan="11">Loại bảng kê: Bổ sung</b></td>
  </tr>';
}
if ($loaibangke == "ALL") {
    $html_tt .= '<tr>
    <td colspan="11"><b>Loại bảng kê: Tất cả</b></td>
  </tr>';
}
$html_tt .= '
</table>
<table border="0" cellspacing="2" cellpadding="2" width="100%" >
  <tr>
    ...
    <td colspan="7" STYLE="border-bottom:0.5px dashed  #000;font-weight: bold">' . $_SESSION['TenCongTy'] . '</td>
  </tr>
  <tr>
    <td colspan="4">[03] Mã số thuế:</td>
	<td colspan="7" STYLE="border-bottom:0.5px dashed  #000;font-weight: bold">' . $_SESSION['MST'] . '</td>
  </tr>
  <tr>
    <td colspan="4">[04] Tên đại lý thuế(nếu có):</td>
	<td colspan="7" STYLE="border-bottom:0.5px dashed  #000;font-weight: bold">
...
<table width="100%" border="0">
  <tr>
	<td colspan="10"></td>
    <td colspan="1" align="right" style="font-size:11px;">Đơn vị tiền: <b>Ngoại tệ</b></td>
  </tr>
</table>';
echo $html_tt;
?>
<div></div>
<table border="1" class="display dataTable" width="100%" cellspacing="1" cellpadding="1">
    <thead>
    <tr>
        ...
        <th STYLE="border:0.5px solid #000;" width="280px" rowspan="2">&nbsp;<br/>Tên người bán</th>
        <th STYLE="border:0.5px solid #000;" width="110px" rowspan="2">Mã số thuế người bán</th>
        <th STYLE="border:0.5px solid #000;" width="160px" rowspan="2">&nbsp;<br/>Mặt hàng</th>
        <th STYLE="border:0.5px solid #000;" rowspan="2" width="120px">Giá trị HHDV mua vào chưa có thuế(NT)</th>
        <th width="27px" STYLE="border:0.5px solid #000;" rowspan="2">Thuế suất %</th>
        <th STYLE="border:0.5px solid #000;" width="90px" rowspan="2">Thuế giá trị gia tăng(NT)</th>
        <th width="80px" STYLE="border:0.5px solid #000;" rowspan="2">Ghi chú hoặc thời hạn thanh toán chậm</th>
    </tr>
    <tr>
    ...
    foreach ($DATACIBANRA as $k => $itemTS) {
        ...
        foreach ($itemTS as $itemCT) {
            $sott++;
            $tongdoanhthu += $itemCT["thanhtienntnt"];
            $tongtienloaihanghoa += $itemCT["thanhtiennt"];
            $tongthue += 0;
            $tongthueloaihanghoa += 0;
        ...
        <td class="td_center" >';
            ...
            $html_ct .= '</td>
        <td class="td_center"  align="left">' . mb_convert_case($itemCT["tenkh"],MB_CASE_TITLE) . '</td>
        <td class="td_center" >' . $itemCT["masothue"] . '</td>
        <td class="td_center"  align="left">' . $itemCT["tenvt"] . '</td>
        <td class="td_center" align="right" >' . NB_Format($itemCT["thanhtiennt"]) . '</td>
        <td class="td_center" align="center" >';
		$ThueSuatTam = round((0/$itemCT["thanhtienntnt"])*100);
		if($itemCT["thuesuat"]=='10'){
			if($ThueSuatTam==8){
				$html_ct .= '8';
			}else{
				$html_ct .=strtoupper($itemCT["thuesuat"]);
			}
		}else{
				$html_ct .=strtoupper($itemCT["thuesuat"]);
		} 
		...
		$html_ct .='</td>
        <td class="td_center" align="right" >' . number_format(0, 0, ",", ".") . '</td>
        <td class="td_end"  align="right">' . $ngaythanhtoan . '</td>
      </tr>
    ';
        }
        $html_ct .= '
        ...
        <td class="td_full" >';
        $html_ct .= '</td>
        <td class="td_full"  align="left"></td>
        <td class="td_full" ></td>
        <td class="td_full"  align="left" style="text-align: right" ><b><i>Tổng &nbsp;&nbsp;</i></b></td>
        <td class="td_full" align="right" style="text-align: right" ><b><i>' . NB_Format($tongtienloaihanghoa) . '</i></b></td>
        <td class="td_full" align="center" ></td>
        <td class="td_full" align="right" style="text-align: right" ><b><i>' . number_format($tongthueloaihanghoa, 0, ",", ".") . '</i></b></td>
    ...
    <td class="td_full" align="right" STYLE="border:0.5px solid #000;text-align:right;"><b>' . NB_Format($tongdoanhthu) . '</b></td>
    ...
    <td class="td_full" STYLE="border:0.5px solid #000;" ><input style="background-color: #00c6ff;font-size: 10px;'.$display.'" width="100%" class="no-print duyettaikhoan" data-name="1#'.$_SESSION["THONGTINPHIEU"]['ngayhoadon'].'#'.$tongdoanhthu.'#'.$tongthue.'" type="button" value="DUYỆT"></td>
  </tr>
</table>';
    echo $html_ct;
    $html_sum = '<table width="700" class="page_break" border="0">
  <tr>
    <td colspan="5" width="280"><strong>Tổng doanh thu hàng hóa, dịch vụ mua vào:</strong></td>
    <td colspan="1" width="70" align="right"><b>' . NB_Format($tongdoanhthu) . '</b></td>
  </tr>
  <tr>
    <td colspan="5"><strong>Tổng thuế GTGT của hàng hóa, dịch vụ mua vào:</strong></td>
  ...
  <tr>
    ...
    <td width="35%" rowspan="2" colspan="4" align="center" valign="top">
