...
$DATANHOMKH = $_SESSION["LISTNHOMKH"];
?>
<html>
...
$tongdoanhthu=0;
$tongthue=0;
    foreach ($DATACIBANRA as $k=> $item) {// Duyệt từng TK
        if($xemtonghop=="false"){
        $tongducodktk = 0;
        $tongdunodktk = 0;
		...
		foreach($DATANHOMKH as $item_MaNhom){// Duyệt từng nhóm
			...
			$tongducocktk_Nhom = 0;
			$tongdunocktk_Nhom = 0;
			foreach ($item as $itemCT) {// Duyệt từng khách hàng
            $sott++;
				if($itemCT["manhom"] == $item_MaNhom['manhom']){
				...
				<td class="td_end" ' . $imdam . ' align="right">';
					...
					if ($itemCT['makhcha'] == "0") {
						...
						$tongducodktk_Nhom += $itemCT["soduco"];
						$tongdunodktk_Nhom += $itemCT["soduno"];

						$tongducopstk_Nhom += $itemCT["soducops"];
						$tongdunopstk_Nhom += $itemCT["sodunops"];
					...
					if ($_SESSION['butrucongno']==1 && $itemCT['matk']==131) {// Nếu có bù trừ công nợ
						if ($itemCT['makhcha'] == "0") {
							...
							$tongducocktk_Nhom += $itemCT["tongducock"];
							$tongdunocktk_Nhom += $itemCT["tongdunock"];
						}
					}else {
						...
						$tongducocktk_Nhom += $itemCT["cock_"];
						$tongdunocktk_Nhom += $itemCT["nock_"];
					}
				}
			}
			$TongNhomKH = $tongducodktk_Nhom+$tongdunodktk_Nhom+$tongducopstk_Nhom+$tongdunopstk_Nhom+$tongducocktk_Nhom+$tongdunocktk_Nhom;
				if($TongNhomKH!=0){
					$html_ct .= '<tr style="color:red;">
								<td STYLE="border:0.5px solid #000;"></td>
								<td STYLE="border:0.5px solid #000;"></td>
								<td STYLE="border:0.5px solid #000;" align="center"><b>Tổng nhóm: ' . $item_MaNhom["tennhom"] . '</b></td>
								<td STYLE="border:0.5px solid #000;"></td>
								<td align="right" STYLE="border:0.5px solid #000;"><b>' . number_format($tongdunodktk_Nhom, 0, ",", ".") . '</b></td>
								<td align="right" STYLE="border:0.5px solid #000;"><b>' . number_format($tongducodktk_Nhom, 0, ",", ".") . '</b></td>
								<td align="right" STYLE="border:0.5px solid #000;"><b>' . number_format($tongdunopstk_Nhom, 0, ",", ".") . '</b></td>
								<td align="right" STYLE="border:0.5px solid #000;"><b>' . number_format($tongducopstk_Nhom, 0, ",", ".") . '</b></td>
								<td align="right" STYLE="border:0.5px solid #000;"><b>' . number_format($tongdunocktk_Nhom, 0, ",", ".") . '</b></td>
								<td align="right" STYLE="border:0.5px solid #000;"><b>' . number_format($tongducocktk_Nhom, 0, ",", ".") . '</b></td>
							  </tr>';
				}
        }
        $html_ct .= '<tr>
