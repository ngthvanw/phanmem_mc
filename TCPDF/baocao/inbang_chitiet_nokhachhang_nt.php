    ...
    <script type="text/javascript" language="javascript">
        $(document).ready(function() {
			$(document).on('click','#xuatexcel',function(e) {
				...
				link.download = "Bang_ChiTiet_CNKH_NT_" + new Date().toISOString().replace(/[\-\:\.]/g, "") + ".xls",
				link.href = result;
				link.click();
			});
...
$tamtinhdauky = ($_SESSION["GETNODK"]['sdkcont'] - $_SESSION["GETNODK"]['sdknont']);
if ($tamtinhdauky > 0) {
    $nodk = 0;
    $codk = $tamtinhdauky;
} else {
...
<thead>
  ...
  <tr>
    ...
    <th align="right" STYLE="border-top:0.5px solid #FFF;border-right:0.5px solid #000;border-left:0.5px solid #000;border-bottom:0.5px solid #000;text-align: right;"><b>' . (($nodk != 0) ? NB_Format($nodk) : "") . '</b></th>
    <th align="right" STYLE="border-top:0.5px solid #FFF;border-right:0.5px solid #000;border-left:0.5px solid #000;border-bottom:0.5px solid #000;text-align: right;;"><b>' . (($codk != 0) ? NB_Format($codk) : "") . '</b></td>
  </tr>
   </thead>
  ';
$sott = 0;
...
foreach ($DATA as $k => $item) {
    ...
    foreach ($item as $itemCT) {
        if (($itemCT["tienco"]) == 0 && ($itemCT["tienno"]) == 0) {
        ...
        <td class="td_center" width="40px" align="center" >' . date("d-m", strtotime($itemCT["ngayghiso"])) . '</td>
        <td class="td_center" align="center" width="70px" >' . $itemCT["sct"] . '</td>
		<td class="td_center" align="center"  width="40px" >' . date("d-m", strtotime($itemCT["ngayhoadon"])) . '</td>
        ...
        <td align="right" class="td_center" width="100px" >';
            $html_ct .= $sodunops = (($itemCT["tienntno"] != 0) ? NB_Format($itemCT["tienntno"]) : "");
            $html_ct .= '</td>
        <td align="right" class="td_end" width="100px" >';
            $html_ct .= $soducops = (($itemCT["tienntco"] != 0) ? NB_Format($itemCT["tienntco"]) : "");

            $html_ct .= '</td></tr>';
            $tongduco += $itemCT["tienntco"];
            $tongduno += $itemCT["tienntno"];

            /////////////////////////////////////////////
            $tongducotk += $itemCT["tienntco"];
            $tongdunotk += $itemCT["tienntno"];
        }
    ...
    <td style="text-align: right;" class="td_full"><i>' . NB_Format($tongdunotk) . '</b></td>
    <td style="text-align: right;" class="td_full"><i>' . NB_Format($tongducotk) . '</b></td>
  </tr>';
    ...
    $html_ct .= ($nodk == 0) ? "" : NB_Format($nodk);
    $html_ct .= '</b></td>
    <td style="text-align: right;" class="td_full"><i>';
    $html_ct .= ($codk == 0) ? "" : NB_Format($codk);
    $html_ct .= '</b></td>
  </tr>';
}
$dunocktmp = (($tongduco + $_SESSION["GETNODK"]['sdkcont']) - ($tongduno + $_SESSION["GETNODK"]['sdknont']));
if ($dunocktmp > 0) {
    $tongnock = 0;
...
<tr>
    ...
    <td align="right" STYLE="border:0.5px solid #000;"><b>' . NB_Format($tongduno) . '</b></td>
    <td align="right" STYLE="border:0.5px solid #000;"><b>' . NB_Format($tongduco) . '</b></td>
  </tr>
...
$html .= ($tongnock == 0) ? "" : NB_Format($tongnock);
$html .= '</b></td>
<td STYLE="border:0.5px solid #000;" align="right" ><b>';
$html .= ($tongcock == 0) ? "" : NB_Format($tongcock);
$html .= '</b></td>
  </tr>
</table>
<table><tr><td></td></tr></table>
