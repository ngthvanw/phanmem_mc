...
function restore_Database($hostName, $userName, $password, $DbName, $zipFilePath, $dir) {
    $zip = new ZipArchive;
    // Giải nén file .zip để lấy file .sql
    if ($zip->open($zipFilePath) === TRUE) {
        $zip->extractTo($dir);
        $zip->close();
        // Kiểm tra nếu file .sql có tồn tại sau khi giải nén
        $sqlFiles = glob($dir . '*.sql');
        if (!empty($sqlFiles)) {
            $sqlFileName = $sqlFiles[0]; // Lấy file .sql đầu tiên được giải nén
            // Kiểm tra file .sql đã được giải nén
            if (file_exists($sqlFileName)) {
                // Dùng lệnh mysql để phục hồi cơ sở dữ liệu
                $command = "mysql --host={$hostName} --user={$userName} --password={$password} {$DbName} < {$sqlFileName}";
                // Thực thi lệnh phục hồi
                system($command, $output);
                // Xóa file .sql sau khi phục hồi xong
                unlink($sqlFileName);
                return $output === 0; // Trả về true nếu lệnh thành công
            } else {
				echo "<script>alert('Không tìm thấy file .sql sau khi giải nén.');</script>";
                return false;
            }
        } else {
			echo "<script>alert('Không có file .sql nào trong file zip.');</script>";
            return false;
        }
    } else {
		echo "<script>alert('Không thể mở file zip.');</script>";
        return false;
    }
}
...
if (isset($_POST['submit'])) {
    ...
    if ($NienDo != "") {
        if ($MacDinh != "on") {
            ...
            $_SESSION['TenCN'] = $_SESSION['TenCongTy'];
            if($theothongtu=='200'){
                $filename = 'datafile/khac/datafile_tt200.zip';
            }else{
                $filename = 'datafile/khac/datafile_tt133.zip';
            }
            $dbname = $_SESSION['TIENTO'] . $_SESSION['MST'] . "_" . $NienDo;
            mkdir($driver_htdocs."/log_login");
            mkdir($driver_htdocs."/log_login/" . $dbname);
			...
			if ($restore_response) {
				...
				alert("Thiết lập cơ sở dữ liệu thất bại.");
			}
        }
    }
}
