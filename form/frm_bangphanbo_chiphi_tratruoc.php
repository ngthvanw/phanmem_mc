...
<div id="dialog-bangkekhauhao_taisan" title="BẢNG PHÂN BỐ CHI PHÍ TRẢ TRƯỚC...">
    ...
    <form method="post" id="Form-chinh" enctype="multipart/form-data">
        ...
        <fieldset style="background-color: #afd9ee">
            <legend>Không tính phân bổ các công cụ sau</legend>
            <table WIDTH="100%">
                <tr>
                    <td>
						<button onclick="$('.dialog_main_danhmuc_cptratruoc').load('form/frm_danhsach_macptratruoc_khongkhauhao.php?loai=ccdc');" type="button" style="width:100%;border:1px solid #990" class="ui-button ui-corner-all ui-widget">CHỌN CÔNG CỤ KHÔNG PHÂN BỔ</button></td>
					</td>
                </tr>
            </table>
        </fieldset>
		<fieldset style="background-color: #afd9ee">
            <legend>Tính phân bổ công cụ theo sản lượng</legend>
            <table WIDTH="100%">
                <tr>
                    <td>
                        <button onclick="$('.dialog_main_mataisan').load('form/frm_danhsach_congcu_sanluong.php');" type="button" style="width:100%;border:1px solid #990" class="ui-button ui-corner-all ui-widget">CHỌN CÔNG CỤ TÍNH PHÂN BỔ THEO SẢN LƯỢNG</button></td>
                </tr>
            </table>
        </fieldset>
        <fieldset style="background-color: #afd9ee;display:none;">
            ...
            <table>
                <tr>
                    <td><input  id="btntaisankhongkhauhao" type="button" value="Chọn tài sản không tính khấu hao..."/></td>
                </tr>
            </table>
        </fieldset>
         <fieldset style="background-color: #afd9ee">
            ...
            <table>
                <tr>
                    ...
                    <td>&nbsp;Xóa phân bổ đã trích trong kỳ</td>
                </tr>
                 <tr>
                    <td><input type="checkbox" name="xoakhauhaodatrichtrongky" id="ghidekhauhaodatrich"></td>
                    <td>&nbsp;Ghi đè nếu có số liệu phân bổ đã trích</td>
                </tr>
                 <tr>
        ...
        <fieldset style="background-color: #afd9ee">
            <legend>Phân bổ theo</legend>
            <table width="100%">
                <tr>
                    <td>
                        <select style="height:25px;width:100%;" id="khauhaotheo">
                            <option value="">CHỌN HÌNH THỨC PHÂN BỔ</option>
                            <option value="ngaysd">KHẤU HAO THEO NGÀY SỬ DỤNG CÔNG CỤ DỤNG CỤ</option>
                            <option value="ngayghiso">KHẤU HAO THEO NGÀY GHI SỔ CÔNG CỤ DỤNG CỤ</option>
                        </select>
                    </td>
                </tr>
            </table>
...
<script>
    ...
    $dir_module_danhmuccptratruoc = "modules/danhmuccptratruoc/";////////////////Khai báo đường dẫn vào mudole
    $(function () {
        readonlyInput();
        var dialog, form,
		...
		$("#btntaisankhongkhauhao").click(function(e) {
            $('.dialog_main_danhmuc_cptratruoc').load('form/frm_danhsach_macptratruoc_khongkhauhao.php?loai=ccdc');
        });
...
///-------------------Kết thúc--------------------------------
        ...
        function ChucNang_ThuChi() {// Xử lý khi nhấp button đồng ý
            ...
            if($khauhaotheo==""){
                alert("CHƯA CHỌN PHÂN BỔ CÔNG CỤ DỤNG CỤ THEO NGÀY .");
                $("#khauhaotheo").focus();
                return false;
            }
            if ($tonghopcanam == false) {
                if (confirm("Dữ liệu đã tồn tại. bạn có muốn ghi đè không ?")) {
                    ...
                    for ($i = $tuthang; $i <= $denthang; $i++) {
                        $.ajax({
                            url: $dir_module_danhmuccptratruoc + "themkhauhaotaisan.php",
                            dataType: 'json',
                            data: {
                                tuthang: $i,
                                denthang: $i,
                    ...
                    $('.dialog_main_thongbao').load("form/frm_insolieu_bangke_phanbocp.php?tuthang=" + $tuthang + "&denthang=" + $denthang + "&tonghopcanam=" + $tonghopcanam);
                }else{
                    $('.dialog_main_thongbao').load("form/frm_insolieu_bangke_phanbocp.php?tuthang=" + $tuthang + "&denthang=" + $denthang + "&tonghopcanam=" + $tonghopcanam);
                }
            }else{
