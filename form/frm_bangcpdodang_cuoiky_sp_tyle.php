...
<div id="dialog-bangcandoiketoan" title="BẢNG CHI PHÍ DỠ DANG CUỐI KỲ SẢN PHẨM (THEO PHƯƠNG PHÁP PHÂN BỔ NVL)">
    ...
    <form method="post" id="Form-chinh" enctype="multipart/form-data">
        ...
        <fieldset style="background-color: #afd9ee;">
            ...
            <table border="0" style="width: 100%;">
                ...
                <tr style="display: none">
                    <td colspan="2">
                        <select name="theocongdoan" style="width:100%;height:25px; display: none;" id="theocongdoan">
        ...
        <fieldset style="background-color: #afd9ee;">
            <legend style="color: red">Phân bổ tỷ lệ</legend>
            <table>
                <tr>
                    <td><input type="checkbox" checked name="TuDongPhanBoTheoTyLe" id="TuDongPhanBoTheoTyLe"></td>
                    <td>&nbsp;Phân bổ NVL theo tỷ lệ <i style="color:red;">(Nếu tính giá thành theo giá thành tiêu chuẩn thì không chọn phân bổ)</i></td>
                </tr>
            </table>
        </fieldset>
        <fieldset style="background-color: #afd9ee;">
            <legend style="color: red">Phương pháp phân bổ NVL</legend>
            <table style="width: 100%;">
                <tr>
                    <td colspan="2" style="width:100%;height:25px;">
                        <select name="phanbotheo" id="phanbotheo" style="width:100%;height:25px;">
                            <option value="dinhmuc_nvl">1. Nguyên vật liệu định mức</option>
                            <option value="doanhthu_thuan">2. Doanh thu thuần</option>
                            <option value="doanhthu_thuchien">3. Doanh thu thực hiện</option>
                        </select></td>
                </tr>
            </table>
        </fieldset>
...
///-------------------Kết thúc--------------------------------
        ...
        function ChucNang_ThuChi() {// Xử lý khi nhấp button đồng ý
			...
			$TuDongPhanBoTheoTyLe = $("#TuDongPhanBoTheoTyLe").prop("checked");
            ...
            if(res){
                $.confirm({
                    ...
                    content: function() {
                        var self = this;
                        self.setContent('Đang cập nhật dữ liệu...');
                        ...
                        return $.ajax({
                            ...
                            self.setContent('Đang phân bổ đơn giá...'); // Hiển thị loading trước AJAX 2
                            return $.ajax({
                                url: $dir_module_dmsanpham + "tudong_phanbo_dongia_nhapkho_thanhpham.php",
                                dataType: 'json',
                                method: 'post',
                                data: {
                                    tungay: $tungay,
                                    ...
                                    masp: $masp,
                                    TuDongTaoButToanPhatSinh: $TuDongTaoButToanPhatSinh,
                                    TuDongPhanBoTheoTyLe: $TuDongPhanBoTheoTyLe,
                                    nhomcttheo: $nhomcttheo,
                                    theocongdoan: $theocongdoan,
                                    phanbotheo: $phanbotheo,
                                    loaipb: 'SP'
                                }
                            });
                        }).then(function(response2) {
                            self.setContent('Cập nhật dữ liệu thành công.');
                        });
                    },
                    buttons: {
                        "OK": {
                            keys: ['Y'],
                            action: function() {
                                $('.dialog_main_thongbao').load("form/frm_insolieu_bangchiphidodang_cuoiky.php?tungay=" + $tungay + "&denngay=" + $denngay + "&loaisanpham=" + $loaisanpham + "&nhomcttheo=" + $nhomcttheo + "&theocongdoan=" + $theocongdoan + "&theopp=tyle");
                            }
                        }
                    }
                });
