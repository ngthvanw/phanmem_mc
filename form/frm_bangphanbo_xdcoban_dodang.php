...
<div id="dialog-bangkekhauhao_taisan" title="BẢNG PHÂN BỐ XÂY DỰNG CƠ BẢN DỞ DANG...">
    ...
    <form method="post" id="Form-chinh" enctype="multipart/form-data">
        ...
        <fieldset style="background-color: #afd9ee">
            <legend>Không tính phân bổ sau</legend>
            <table>
                <tr>
                    <td><input  id="btntaisankhongkhauhao" width="100%" type="button" value="Chọn danh sách không phân bổ..."/></td>
                </tr>
            </table>
        </fieldset>
...
<script>
    ...
    $(function () {
		...
		$("#btntaisankhongkhauhao").click(function(e) {
            $('.dialog_main_danhmuc_cptratruoc').load('form/frm_danhsach_macptratruoc_khongkhauhao.php?loai=xdcb');
        });
        ...
        function ChucNang_ThuChi() {// Xử lý khi nhấp button đồng ý
            ...
            if ($tonghopcanam == false) {
                if (confirm("Dữ liệu đã tồn tại. bạn có muốn ghi đè không ?")) {
                    ...
                    $('.dialog_main_thongbao').load("form/frm_insolieu_bangke_phanbocp.php?loai=xdcb&tuthang=" + $tuthang + "&denthang=" + $denthang + "&tonghopcanam=" + $tonghopcanam);
                }else{
                    $('.dialog_main_thongbao').load("form/frm_insolieu_bangke_phanbocp.php?loai=xdcb&tuthang=" + $tuthang + "&denthang=" + $denthang + "&tonghopcanam=" + $tonghopcanam);
                }
            }else{
