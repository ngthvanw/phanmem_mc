                                ...
                                <td colspan="3">
                                    <table width="100%" border="0">
                                        ...
                                        <tr>
                                            ...
                                            <td align="right" width="21%">
                                                <select id="hoadondientu" style="width: 100%;height:20px;">
                                                    <option value="0">Không lập HĐĐT</option>
                                                    <option value="1">Lập hoá đơn gốc</option>
                                                    <option value="2">Điều chỉnh thông tin</option>
                                                    <option value="3">Điều chỉnh tiền</option>
                                                    <option value="5">Lập hoá đơn thay thế</option>
                                                    <option value="4">Huỷ xoá đơn</option>
                                                    <option value="6">Lập hoá đơn chuyển đổi</option>
                                                </select>
                                                <input type="hidden" id="MaBiMatHoaDon">
                                                <input type="hidden" id="LoaiHoaDonDienTu">
                                            </td>
                                            <td width="4%">&nbsp;</td>
                                            <td width="21%"><b style="font-size: 12px;background-color: orange;"
                                        ...
                                        <tr id="trvanbanthoathuan">
                                            ...
                                            <td align="right" width="21%"><b style="font-size: 12px;">Ngày thoả thuận:</b></td>
                                            <td width="4%">&nbsp;</td>
                                            <td width="21%"><input type="date" align="right" name="ngaythoathuan" value="<?php echo date("Y-m-d") ?>" id="ngaythoathuan"
                                                                   required="require"
                                                                   placeholder="Ngày ghi sổ" class="text ui-widget-content ui-corner-all"/></td>
                                        </tr>
                                    </table>
                                </td>
                            </tr>
...
$("#trvanbanthoathuan").show();
        ...
        function checkSoHoaDonTrung_Vuot($LoaiPhieu) {// Check khi vượt 20tr trên 1 ngày
            ...
            $tk = $("#tkco").val().trim();
            if ($LoaiPhieu == 2 && ($loaict == 1 || $loaict == 2 || $loaict == 3 || $loaict == 4)) {
                $.ajax({
...
// Khai báo các btntkco trên form
        ...
        $(btnthem).click(function (event) {// Gọi table khách hàng để chọn
            ...
            if ($LoaiPhieu == 2 && $LoaiCT == '1' && $hoadondientu != 0 && $loaitokhai == '1' && "<?php echo $_SESSION['thietlaphddt'] ?>" == "1" && "<?php echo $_SESSION['txt_hddt_tendangnhap'] ?>" != "" && "<?php echo $_SESSION['txt_hddt_matkhau'] ?>" != "") {// Nếu là phiếu chi và loại chứng từ là thuế GTGT thì thực hiện lập hoá đơn điện tử
                $res = {
                ...
                var newDate = $NgayHoaDon[1] + "," + $NgayHoaDon[2] + "," + $NgayHoaDon[0];
                $NgayLapHoaDon = ((new Date(newDate).getTime()));
                $tencanhan = "";
                $tendonvi = "";
                if (masothue.val().trim() == "") {
                    $tencanhan = $("#tenkhachhang").val().trim();
                } else {
                    $tendonvi = $("#tenkhachhang").val().trim();
                }
                if ($hoadondientu == '1') {// Lập hoá đơn gốc
                    var $data = {
                        "generalInvoiceInfo": {
                        ...
                        "itemInfo": [
                            {
                                ...
                                "unitPrice": $("#sotien1").val().toString().split(",").join(""),
                                "quantity": 1,
                                "itemTotalAmountWithoutTax": $("#sotien1").val().toString().split(",").join(""),
                                "taxPercentage": $("#thuesuat1").val(),
                                "taxAmount": $("#sotien2").val().toString().split(",").join(""),
                                "discount": 0.0,
                                "itemDiscount": 0.0
                            }
                        ],
                        "discountItemInfo": [],
                        "summarizeInfo": {
                            "sumOfTotalLineAmountWithoutTax": $("#sotien1").val().toString().split(",").join(""),
                            "totalAmountWithoutTax": $("#sotien1").val().toString().split(",").join(""),
                            "totalTaxAmount": $("#sotien2").val().toString().split(",").join(""),
                            "totalAmountWithTax": $("#tongtien").val().toString().split(",").join(""),
                        ...
                        "taxBreakdowns": [
                            {
                                "taxPercentage": $("#thuesuat1").val(),
                                "taxableAmount": $("#sotien1").val().toString().split(",").join(""),
                                "taxAmount": $("#sotien2").val().toString().split(",").join("")
                            }
                        ]
                    };
                    $res = $.parseJSON(LapHoaDonDienTu(1, $data));
                    ...
                    var $data = {
                        "generalInvoiceInfo": {
                            ...
                            "originalInvoiceIssueDate": $NgayLapHoaDon,
                ...
                if ($res.errorCode != null) {
                    $alert = confirm($res.description + " .\n Nhấp [OK] để bỏ qua, [HUỶ] để nhập lại hoá đơn ?");
                    if ($alert) {
                        valid = true;
                    } else {
                        valid = false;
                    ...
                    $kyhieu = $invoiceNo.slice(0, 4);
                    $sohoadon = $invoiceNo.slice(6, 12);
                    console.log($kyhieu);
                    $("#kyhieu").val($kyhieu);
        ...
        $(sohoadon).focus(function (event) {// Gọi table mã nội dung để chọn
            ...
            $loaict = $("#loaict").val();
            //alert($sohoadon);
            if ($sohoadon == "" && $LoaiPhieu == 1 && $loaict == 1) {
                $.ajax({
        ...
        function phantrang($page, $sophieu) {
            ...
            if ($data.loaihddt == 0) {
                ...
                $("#TXTLoaiHoaDonDienTu").text("Đã lập hoá đơn");
        ...
        function ChucNang_ThuChi() {// Xử lý khi nhấp button đồng ý
...

            {
                ...
                $LoaiCT = loaict.val().trim();
                $hoadondientu = $("#hoadondientu").val();
                if (valid && $LoaiPhieu == 2 && $LoaiCT == '1' && $hoadondientu != 0 && $loaitokhai == '1' && "<?php echo $_SESSION['thietlaphddt'] ?>" == "1" && "<?php echo $_SESSION['txt_hddt_tendangnhap'] ?>" != "" && "<?php echo $_SESSION['txt_hddt_matkhau'] ?>" != "") {// Nếu là phiếu chi và loại chứng từ là thuế GTGT thì thực hiện lập hoá đơn điện tử
                    $res = {
                        "errorCode": "INVOICE_ISSUED_DATE_INVALID",
                        "description": "Không kết nối được tới máy chủ !"
                    }
                    ...
                    if ($hoadondientu == '1') {// Lập hoá đơn gốc
                        ...
                        var $data = {
                            "generalInvoiceInfo": {
                                "invoiceNo": $("#kyhieu").val() + "" + $("#sohoadon").val(),
                                "invoiceType": $("#mauso").val().substr(0, 7),
                                "templateCode": $("#mauso").val(),
                                "invoiceIssuedDate": $NgayLapHoaDon,
                                "currencyCode": "VND",
                                "invoiceNote": $("#ghichu").val(),
