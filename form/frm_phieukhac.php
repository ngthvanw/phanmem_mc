                ...
                <tr>
                    ...
                    <td style="text-align:right"><input type="button" class="button" style="margin-right:3px;"
                        ...
                        <input type="button" class="button" style="margin-right:3px;" disabled="disabled"
                               ...
                               value="Về cuối"/>&nbsp;
                        <input type="button" class="button" name="btntim" id="btntim" value="Tìm"/>&nbsp;&nbsp;
                        ...
                        <table width="100%" border="0">
                            ...
                            <tr>
                                ...
                                <td><input style="text-transform: uppercase" type="text" name="kyhieu"
                                           onkeyup="return KhuDauTiengViet(this.value,'#kyhieu')" value="<?php echo $_SESSION['txt_kyhieu'] ?>"
                            ...
                            <tr>
                                <td><label for="name"><input type="checkbox" id="congaykhaithue"
                                                             style="margin-top: 2px;"> <abbr title="NGÀY KHAI THUẾ BAO GỒM KHAI CHÍNH THỨC VÀ KHAI BỔ SUNG">Ngày
                                            KT</abbr></label></td>
                                ...
                                <td colspan="3">
                                    <table width="100%" border="0">
                                        ...
                                        <tr>
                                            ...
                                            <td width="4%"></td>
                                            <td width="21%" style="<?php echo (($_SESSION['thietlaphddt'] == '1') ? '' : 'display:none'); ?>"><b style="font-size: 11px;background-color: orange;"
                                        ...
                                        <tr>
                                            ...
                                            <td colspan="2" align="left" style="<?php echo (($_SESSION['duandautu'] == '1') ? '' : 'display:none'); ?>">&nbsp;&nbsp;
                                                <input name="duandautu" type="checkbox" class="checkbox" style="margin-left: 5px;"
                                                       id="duandautu" value="1"/><b style="font-size: 12px;">D.A Đầu tư</b></td>
            ...
            <table class="table-dialog" style="vertical-align: middle;width: 780PX" border="0">
                ...
                <tr>
                    <td style="padding: 2px"><input type="hidden" name="btnnoidung2" id="btnnoidung2" class="button"
                        ...
                        <input type="text" name="noidung2" id="noidung2" required="required" style="width: 75%"
                               placeholder="Nội dung" class="text ui-widget-content ui-corner-all"/>
                        <select id="thuesuat1" class="text ui-widget-content ui-corner-all" name="thuesuat1"  style="width: 47px; height: 21px;" tabindex="-1">
                            <?php
                            foreach ($cb_ThueSuat as $iTem_ThueSuat){
                                foreach ($iTem_ThueSuat as $Key=>$iTem) {
                                    ?>
...
///-------------------------Di chuyễn các phần tử bằng enter----------------
        ...
        ////////////////////////////////////////////////////////////////////////
        <?php
            // build options server-side (đặt trước <script>)
            $options = '<option value="0">---Không chọn---</option>';
            foreach ($cb_ThueSuat_HKD as $k => $v) {
                $options .= '<option value="' . htmlspecialchars($k, ENT_QUOTES) . '">' . htmlspecialchars($v, ENT_QUOTES) . '</option>';
            }
        ?>
        $("#phieuthu").on('change', function(){
        // json_encode trả về một JS string đã escape an toàn
            $("#loaihanghoadichvu").html(<?php echo json_encode($options); ?>);
        });
        ...
        $("#hoadondientu").change(function (event) {// Gọi table mã nội dung để chọn
            ...
            if ($LoaiHoaDon == 11) {
                ...
                if ($KQ) {
                    ...
                    $tencanhan = "";
                    $tendonvi = "";
                    $buyerIdNo = ""; // Cập nhất Số giấy tờ
                    $CCCD = ""; // Cập nhất CCCD cho hoá đơn từ máy tình tiền
                    $MaHC = ""; // Cập nhất Mã Hộ Chiếu cho hoá đơn từ máy tình tiền
                    $MaNS = ""; // Cập nhất Mã Ngân Sách cho hoá đơn từ máy tình tiền
                    $buyerIdType = "";
                    if ($("#masothue2").val().trim() == "" && $latendonvi == false) {
                        $tencanhan = $("#tenkhachhang2").val().trim().split("&#039;").join("'");
                        //if($LoaiHoaDonDT=="M"){// Cập nhất CCCD cho hoá đơn từ máy tình tiền
								var timCCCD = $tencanhan.match(/CCCD\s*:\s*(\d+)\s*/);								
								if (timCCCD) {
                                    $tencanhan = $tencanhan.split('(')[0].trim();
                                    $buyerIdType = 1;
									$buyerIdNo = timCCCD[1]; // Kết quả là số CCCD
								}
                                var timMaHC = $tencanhan.match(/MaHC\s*:\s*(\d+)\s*/);								
								if (timMaHC) {
                                    $tencanhan = $tencanhan.split('(')[0].trim();
                                    $buyerIdType = 3;
									$buyerIdNo = timMaHC[1]; // Kết quả là số Hộ chiếu
								}
                                var timMaNS = $tencanhan.match(/MaNS\s*:\s*(\d+)\s*/);								
								if (timMaNS) {
                                    $tencanhan = $tencanhan.split('(')[0].trim();
                                    $buyerIdType = "";
									$buyerIdNo = timMaNS[1]; // Kết quả là số Hộ chiếu
								}
							//}
                    } else {
...
// Khai báo các btntkco trên form
        ...
        function goitablemakhachhang($vale) {
            if ($vale == "") {// nếu tk nợ trống thì bật tra bảng
                $('.dialog_main_makh').load("form/frm_dm_makh_form_select.php?idstyle=Form-chinh&idinput=makhachhang***tenkhachhang***diachi***masothue***makhachhang2***tenkhachhang2***diachi2***masothue2&idfocus=loaict");
            } else {// nếu tk có không trống
                $data = "";
                $.ajax({
        ...
        function goitablemakhachhang3($vale) {
            if ($vale == "") {// nếu tk nợ trống thì bật tra bảng
                ...
                if ($data != null) { // Nếu tk có tòn tại trong hệ thống tài khoản thì lấy giá trị và gán vào textbox
                    ...
                    $('.dialog_main_makh').load("form/frm_dm_makh_form_select.php?idstyle=Form-chinh&idinput=makhachhang3***tenkhachhang3***diachi3&idfocus=makhachhang&ma=" + $("#makhachhang3").val().trim());
                }
            }
        }
        ...
        function goitablebbophan($vale) {
            ...
            if ($vale == "") {// nếu tk nợ trống thì bật tra bảng
                if ($loaisp == "SP") {
                    ...
                    $('.dialog_main_danhmuc_sanpham').load("form/frm_dm_hopdong_form_select.php?idstyle=Form-chinh&idinput=mabophan***bophan&idfocus=tygia&ma=" + $("#mabophan").val().trim());
                } else {
                    mabophan.val("0001");
                    bophan.val("Toàn Bộ");
                }
        ...
        $(btnthem).click(function (event) {// Gọi table khách hàng để chọn
            ...
            if (($tkno1.substring(0, 3) == 111 || $tkno1.substring(0, 3) == 112) && $makhachhang == $makhachhang3) {
                alert("Không thể sử dụng tài khoản " + $tkno1 + " trong phiếu định khoản .");
                tkno1.focus();
                return false;
            }
            if (($tkno2.substring(0, 3) == 111 || $tkno2.substring(0, 3) == 112) && $makhachhang == $makhachhang3) {
                alert("Không thể sử dụng tài khoản " + $tkno2 + " trong phiếu định khoản .");
                tkno2.focus();
                return false;
            }
            ...
            if (($LoaiPhieu % 2) == 0 && $("#chonloaisp").val() == 0 && ($tkco == 621 || $tkco == 622 || $tkco == 623 || $tkco == 627 || $tkno1 == 621 || $tkno1 == 622 || $tkno1 == 623 || $tkno1 == 627 || $tkno2 == 621 || $tkno2 == 622 || $tkno2 == 623 || $tkno2 == 627)) {
            ...
            valid = valid && checkNull($('#thuesuat1'), " Thuế suất ");
        ...
        $(sohoadon).focus(function (event) {// Gọi table mã nội dung để chọn
            ...
            $loaict = $("#loaict").val();
            //alert($sohoadon);
            if ($sohoadon == "" && $LoaiPhieu == 3 && ($loaict == 1 || $loaict == 2)) {
                $.ajax({
            ...
            if ($STT == "") {//Nếu số thứ tự null sẽ tạo số mới tự động tăng theo mã
                ...
                if ($checkphieuthuchi == 0) {
                    ...
                    try {
                        ...
                        $("#ThamChieuTS").text("Tham chiếu: " + $data.sophieu);
                        $("#cothuegtgt").attr("disabled", true);
        ...
        function ChucNang_ThuChi() {// Xử lý khi nhấp button đồng ý
            ...
            valid = valid && checkChonCapTongHop('makh','makhcha',$('#makhachhang3'),'Mã khách hàng ');
            ...
            if (($tkco.substring(0,3) != "341" && $tkno1.substring(0,3) != "341" && $tkno2.substring(0,3) != "341" && $("#makhachhang3").val().trim()!="" && $("#makhachhang").val()!=$("#makhachhang3").val() ) ) {
                $thongbao = "";
                $thongbao = "CẢNH BÁO \n\n Hai mã khách không giống nhau. Bạn có muốn tiếp tục không ?";
                var result1 = confirm($thongbao);
                if (!result1) {
                    $("#makhachhang3").focus();
                    return false;
            ...
            if('<?php echo $_SESSION['nhacungcaphddt']; ?>'=='viettel') {////// HOÁ ĐƠN ĐIỆN TỬ CỦA VIETTEL
                ...
                if (valid && $LoaiPhieu == 3 && ($loaict == '1' || $loaict == '2') && $hoadondientu != 0 && $loaitokhai == '1' && "<?php echo $_SESSION['thietlaphddt'] ?>" == "1" && "<?php echo $_SESSION['txt_hddt_tendangnhap'] ?>" != "" && "<?php echo $_SESSION['txt_hddt_matkhau'] ?>" != "" && "<?php echo $_SESSION['txt_hddt_duongdan'] ?>" != "") {// Nếu là phiếu chi và loại chứng từ là thuế GTGT thì thực hiện lập hoá đơn điện tử
                    $res = {
                        "errorCode": "INVOICE_ISSUED_DATE_INVALID",
                        "description": "Không kết nối được tới máy chủ !"
                    }
					...
					$kyhieuhoadon = $("#kyhieu").val().toUpperCase();// Cập nhất CCCD cho hoá đơn từ máy tình tiền
					$LoaiHoaDonDT = $kyhieuhoadon.substring(3,4); // Nếu M là hoá đơn khởi tạo từ máy tính tiền
					$CCCD = ""; // Cập nhất CCCD cho hoá đơn từ máy tình tiền
					$MaHC = ""; // Cập nhất Mã Hộ Chiếu cho hoá đơn từ máy tình tiền
                    $MaNS = ""; // Cập nhất Mã Ngân Sách cho hoá đơn từ máy tình tiền
                    if ($("#masothue2").val().trim() == "" && $latendonvi == false) {
						...
						//if($LoaiHoaDonDT=="M"){// Cập nhất CCCD cho hoá đơn từ máy tình tiền
								...
								if (timCCCD) {
                                    $tencanhan = $tencanhan.split('(')[0].trim();
									$CCCD = timCCCD[1]; // Kết quả là số CCCD
								}
                                var timMaHC = $tencanhan.match(/MaHC\s*:\s*(\d+)\s*/);								
								if (timMaHC) {
                                    $tencanhan = $tencanhan.split('(')[0].trim();
									$MaHC = timMaHC[1]; // Kết quả là số Hộ chiếu
								}
                                var timMaNS = $tencanhan.match(/MaNS\s*:\s*(\d+)\s*/);								
								if (timMaNS) {
                                    $tencanhan = $tencanhan.split('(')[0].trim();
									$MaNS = timMaNS[1]; // Kết quả là số Hộ chiếu
								}
							//}
                    } else {
                        $tendonvi = $("#tenkhachhang2").val().trim().split("&#039;").join("'");
                            ...
                            "CommandObject":[
                                {
                                    "Invoice":{
                                        ...
                                        "BuyerName":$tencanhan,
										"CCCD":$CCCD,// Cập nhất CCCD cho hoá đơn từ máy tình tiền
                                        "PassportNumber":$MaHC,// Cập nhất Mã Hộ Chiếu cho hoá đơn từ máy tình tiền
                                        "FiscalCodes":$MaNS,// Cập nhất Mã Ngân Sách cho hoá đơn từ máy tình tiền
                                        "BuyerTaxCode":$("#masothue2").val(),
                ...
                if ("<?php echo $_SESSION['thietlaphddt'] ?>" == "1") {
                    if (valid && $LoaiPhieu == 3 && ($LoaiCT == '1' || $LoaiCT == '2') && $hoadondientu != 0 && $hoadondientu != 9 && $loaitokhai == '1' && "<?php echo $_SESSION['txt_hddt_tendangnhap'] ?>" != "" && "<?php echo $_SESSION['txt_hddt_matkhau'] ?>" != "" && "<?php echo $_SESSION['txt_hddt_duongdan'] ?>" != "") {// Nếu là phiếu chi và loại chứng từ là thuế GTGT thì thực hiện lập hoá đơn điện tử
                        $res = {
                            "errorCode": "INVOICE_ISSUED_DATE_INVALID",
                            "description": "Không kết nối được tới máy chủ !"
                        }
            ...
            if (valid) {
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        if ($LoaiPhieu == 4 && (tkno1.val().trim().substring(0,3) == "211" || tkno2.val().trim().substring(0,3) == "211" || (tkno1.val().trim().substring(0, 3) == "212") || (tkno2.val().trim().substring(0, 3) == "212") || (tkno1.val().trim().substring(0,3) == "242") || (tkno1.val().trim().substring(0,3) == "241") || tkno1.val().trim() == "1562")) {
                            if ((tkno1.val().trim().substring(0,3) == "211" || tkno2.val().trim().substring(0,3) == "211") || (tkno1.val().trim().substring(0, 3) == "212") || (tkno2.val().trim().substring(0, 3) == "212")) {
                                if (window.confirm("Số liệu phát sinh này liên quan đến tài sản cố định . \n Muốn hiển thị cửa sổ nhập tài sản ?")) {
                                    xoadialog_phieukhac();
                                    $('.dialog_main_tangtaisan').load('form/frm_tangtaisan.php?sottphieukhac=' +$sottthamchieutaisan + '&form=frm_phieukhac&nguyengia=' + sotien1.val().trim() + '&tk=' + tkno1.val().trim() + '&ngayghiso=' + ngayghiso.val().trim());

                                } else {
                                    if ($('.dialog_main_thongbao').html() == "") {
                                        $('.dialog_main_thongbao').load('form/frm_thongbao_phieukhac.php?loaiphieu=' + $LoaiPhieu + '&ngayhd=' + ngayhoadon.val().trim() + '&sophieu=' + sophieu.val().trim() + '&mapskt=' + STT.val().trim() + '&invoiceNo=' + $kyhieu + '' + $("#sohoadon").val().trim() + '&mabimat=' + $("#MaBiMatHoaDon").val() + '&mauso=' + $mauso);
                                    }
                                }
                                ...
                                if (window.confirm("Số liệu phát sinh này liên quan đến công cụ dụng cụ . \n Muốn hiển thị cửa sổ nhập công cụ dụng cụ ?")) {
                                    xoadialog_phieukhac();
                                    $('.dialog_main_tangtaisan').load('form/frm_tang_chiphi_tratruoc.php?sottphieukhac=' + $sottthamchieutaisan + '&form=frm_phieukhac&nguyengia=' + sotien1.val().trim() + '&tk=' + tkno1.val().trim() + '&ngayghiso=' + ngayghiso.val().trim());
                            ...
                            }else if ((tkno1.val().trim().substring(0,3) == "241") ) {
                                if (window.confirm("Số liệu phát sinh này liên quan đến xây dựng cơ bản dở dang . \n Muốn hiển thị cửa sổ nhập xây dựng cơ bản dở dang ?")) {
                                    xoadialog_phieukhac();
                                    $('.dialog_main_tangtaisan').load('form/frm_tang_xdcoban_dodang.php?sottphieukhac=' + $sottthamchieutaisan + '&form=frm_phieukhac&nguyengia=' + sotien1.val().trim() + '&tk=' + tkno1.val().trim() + '&ngayghiso=' + ngayghiso.val().trim());
                            ...
                            if ($('.dialog_main_thongbao').html() == "") {
                                $('.dialog_main_thongbao').load('form/frm_thongbao_phieukhac.php?loaiphieu=' + $LoaiPhieu + '&ngayhd=' + ngayhoadon.val().trim() + '&sophieu=' + sophieu.val().trim() + '&mapskt=' + STT.val().trim() + '&invoiceNo=' + $kyhieu + '' + $("#sohoadon").val().trim() + '&mabimat=' + $("#MaBiMatHoaDon").val() + '&mauso=' + $("#mauso").val());
                            }
                        }
                    }
                });
