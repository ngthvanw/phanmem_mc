...
<script>
    ...
    $(function () {
        ...
        function xoadialog_ds_nhapkho() { // ----------------------đóng form
            reset_dialog(".dialog-ds_tanggiam_ts");
            reset_dialog(".dialog_main_ds_nhapkho");
        }

        $("#dialog-ds_tanggiam_ts").dialog({ // ------------------Gọi dialog
            resizable: false,
        ...
        $("#dialog-ds_tanggiam_ts").keydown(function (event) {//--------------Các phím tắt
            var $grid_pb = $("#grid_editing_ds_tanggiam_ts").closest('.pq-grid');//---- Lưới----------------
            ...
            var rowEditting = $("#grid_editing_ds_tanggiam_ts").pqGrid("getRowsByClass", {cls: 'pq-row-edit'});//---Lấy đối tượng đang sửa
        ...
        var manoidung_select = function (ui) {
            ...
            var $inp = $("<input type='text'  name='" + dataIndx + "' class='" + cls + " pq-cell-editor pg-cel-define' readonly />")
                .appendTo($cell)
                .val(dc).keypress(function (e) {
                    $('.dialog_main_manoidung').load("form/frm_dm_manoidung_select.php?idstyle=grid_editing_ds_tanggiam_ts");
                });
        }
        ...
        var makh_select = function (ui) {
            ...
            var $inp = $("<input type='text'  name='" + dataIndx + "' class='" + cls + " pq-cell-editor pg-cel-define' readonly />")
                ...
                .val(dc).keypress(function () {
                    $('.dialog_main_makh').load("form/frm_dm_makh_select.php?idstyle=grid_editing_ds_tanggiam_ts");
                });
        }
        ...
        var $grid = $("#grid_editing_ds_tanggiam_ts").pqGrid(obj);
        ...
        function getRowSelect() { // --------------------Lấy dữ liệu theo dòng trên gird----------------------
            var arr = $("#grid_editing_ds_tanggiam_ts").pqGrid("selection", {
                type: 'cell',
                method: 'getSelection'
        ...
        //-----------------------------Hết lưới---------------------------------------------------------------------
		 setTimeout(function(){
		$("#grid_editing_ds_tanggiam_ts .pq-search-hd-field").focus();
	},1000);
    });
</script>
<div id="dialog-ds_tanggiam_ts"
     title="Danh sách tăng/giảm tài sản... (INSERT: chọn mã, ESC: Thoát)">
    <!-- dialog -->
    <div id="grid_editing_ds_tanggiam_ts" style="margin:5px auto;border: 0px !important;"></div>
</div>