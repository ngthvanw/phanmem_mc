    ...
    <form method="post" id="Form-chinh" enctype="multipart/form-data">
        ...
        <fieldset style="background-color: #afd9ee">
            ...
            <table border="0" style="width: 100%;">
                ...
                <tr>
                    <td width="6px;" style="padding-top:3px;"><input title="Tự động lấy thuế GTGT khấu trừ kỳ trước" name="tudonglaygtgtkytruoc" id="tudonglaygtgtkytruoc"
                                                                     type="checkbox"/></td>
...
<script>
    ...
    $(function () {
		...
		function LayThongTinToKhaiGTGT(){
		    $thangtinhthue = $("#thangtinhthue").val();
            $namtinhthue = $("#namtinhthue").val();
            $loaitokhai = $("#loaitokhai").val();
            $machinhanh = $("#ChiNhanhCongTy").val();
			$tudonglaygtgtkytruoc = $("#tudonglaygtgtkytruoc").prop("checked");
            if ($loaitokhai == 1) {
                $("#LoiDoKhaiBoSung").hide();
                $datakt = "";
				if($tudonglaygtgtkytruoc==true){
                $.ajax({// Thêm dữ liệu vào tokhaithue
                    ...
                    success: function (response) {
                        if (response != 0) {
                            try {
                                $data = $.parseJSON(response);
								if($tudonglaygtgtkytruoc !=true){
									$("#thuegtgtkytruoc").val(FormatNumber($data.B.thue));
								}
                                ...
                                try{
                                    $("#thuegtgttanggiam").val(FormatNumber($data.THTG.thuegtgttanggiam));
                                }catch (e) {
        ...
        $("#loaitokhai").change(LayThongTinToKhaiGTGT);
		$("#thangtinhthue").change(LayThongTinToKhaiGTGT);
		$("#tudonglaygtgtkytruoc").change(LayThongTinToKhaiGTGT);
...
///-------------------Kết thúc--------------------------------
        ...
        function ChucNang_ThuChi() {// Xử lý khi nhấp button đồng ý
            ...
            if (valid) {
                if ('<?php echo substr($ppkhautru, 2, 1) ?>' == 2 || '<?php echo substr($ppkhautru, 2, 1) ?>' == 3){// Nếu khe khai theo phương pháp trược tiếp doanh thu
                    if ($thangtinhthue != "V") {
                        ...
                        if ($dulieutontai == true) {
                            if ($dulieukhoa && $loaitokhai == 1) {
                                ...
                                if('<?php echo substr($ppkhautru, 2, 1); ?>' == 3){
                                    $('.dialog_main_thongbao').load("form/frm_insolieu_bangke_thuegtgt_hkd.php?thangtinhthue=" + $thangtinhthue + "&loaitokhai=" + $loaitokhai+"&namtinhthue="+$namtinhthue+"&machinhanh="+$machinhanh);
                                }else{
