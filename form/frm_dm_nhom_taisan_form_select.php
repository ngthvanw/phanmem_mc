...
<script>
    ...
    $width = getWidth()-50;
    $(function() {
        var $dir_module_nhomtaisan="";
        $dir_module_nhomtaisan = "modules/nhomtaisan/";//--------------------------------------------Thay đổi khi copy
        function xoadialog_mact() { // ----------------------đóng form 
            reset_dialog(".dialog-nhom_taisan");
            reset_dialog(".dialog_main_nhom_taisan");
        }
		 if (typeof $(".dialog-nhom_taisan").html() == "undefined") {
			$("#dialog-nhom_taisan").dialog({ // ------------------Gọi dialog
				resizable: false,
				height: $height,
				width: $width,
				modal: true
     ...
     $("#dialog-nhom_taisan").keydown(function(event) {//--------------Các phím tắt
        var $grid_pb = $("#grid_editing_nhom_taisan").closest('.pq-grid');//---- Lưới----------------
        ...
        var rowEditting = $( "#grid_editing_nhom_taisan" ).pqGrid( "getRowsByClass", { cls : 'pq-row-edit' } );//---Lấy đối tượng đang sửa
          if ( $('div').hasClass('jconfirm')==false) {
              if (event.keyCode == Keys.F4) {
                  var rowSelect = getRowSelect();//------ Lấy đối tượng được chọn
                  if(rowSelect==false){
                      var colM = $("#grid_editing_nhom_taisan").pqGrid("option", "colModel");
                      colM[1].editable = true;
                      $("#grid_editing_nhom_taisan").pqGrid("option", "colModel", colM);
                      ...
                      colM[1].editable = true;
                      $("#grid_editing_nhom_taisan").pqGrid("option", "colModel", colM);
                      var _dataRow = {manhom : rowData.manhom,tennhom :rowData.tennhom,manhomcha :rowData.manhom};
              ...
              if (event.keyCode == Keys.F7) { // copy
                  if (rowSelect != false){
                      ...
                      $("#grid_editing_nhom_taisan").pqGrid("option", "colModel", colM);
                      //--------------------------------------------Thay đổi khi copy---------------------------------
                      var _dataRow = {manhom : rowData.manhom,tennhom :rowData.tennhom,manhomcha :rowData.manhomcha};
                      addRow(rowIndx+1,'manhom',$grid_pb,_dataRow);
                  }
              }
			 ...
			 if (event.keyCode == Keys.INSERT) {
                        ...
                        $ten = rowSelect[0].rowData.tennhom;
    ...
    var makh_select = function (ui) {
            ...
            .val(dc).keypress(function(){
                 $('.dialog_main_makh').load("form/frm_dm_makh_select.php?idstyle=grid_editing_nhom_taisan");
            });            
        }
    var formart_num = function (ui) {
        ...
        function addRow(rowIndx,$name='mavt',$grid,$obj_addrow="") {
            ...
            if($obj_addrow!=""){
                $.ajax({
                    url: $dir_module_nhomtaisan+"taoma.php",
                    async: false,
                    data:{
                        makhcha:$obj_addrow.manhomcha
                    },
                    success: function (response) {
                        $ma = response;
                    }
                ...
                var rowData = {tennhom:"",manhom:"",manhomcha:"0"}; //empty row template
            }
            rowData.manhom = $ma;
                ...
                if($('div').hasClass('jconfirm')==false){
                    $.confirm({
                        ...
                        buttons: {"Đồng ý": {keys: ['Y'],action: function () {
                                ...
                                $.ajax($.extend({}, ajaxObj, {
                                    context: $grid,
                                    url: $dir_module_nhomtaisan+"del.php",
                                    data: { id: sott,ma:ma },
       ...
       var obj = {
					...
					if (type == 'update') {                        
						...
						if(rowData.mavt==""){
                            ...
                            if (rowData[recIndx] == null) {
                                url = $dir_module_nhomtaisan+"add.php";
                            }
                            else {
                                url = $dir_module_nhomtaisan+"edit.php";
                            }
                        }
                    }
                if (valid) {
            ...
            colModel: [//-------------------------------------------Khai báo các cột trong lưới-----------------------
                ...
                { title: "Mã nhóm", dataType: "string", dataIndx: "manhom", minWidth: 80,sortable: true,editable:false,
                    validations: [
                        { type: 'minLen', value: 1, msg: "Mã nhóm không được trống !" },
                        { type: 'maxLen', value: 14, msg: "Mã nhóm không lớn hơn 14 ký tự !" },
                        { type: function (ui) {
                            var value = ui.value,
                                _found = false,sott = ui.rowData.sott;
                            //remote validation
                            $.ajax({
                                url: $dir_module_nhomtaisan+"checkkey.php",
                ...
                { title: "Tên nhóm", minWidth: 200, dataType: "string", dataIndx: "tennhom",
                    validations: [
                ...
                { title: "Mã nhóm cha", minWidth: 80, dataType: "string", align: "left", dataIndx: "manhomcha",hidden:false,
                    validations: [
                        { type: 'minLen', value: 1, msg: "Mã nhóm cha không được trống !" },
                        { type: 'maxLen', value: 14, msg: "Mã nhóm cha không lớn hơn 14 ký tự !" },
                        { type: function (ui) {
                            var value = ui.value,
                                _found = false,sott = ui.rowData.sott;
                            //remote validation
                            $.ajax({
                                url: $dir_module_nhomtaisan+"checkkeycha.php",
                                data: { 'id': value,'sott':sott },
                                async: false,
                                success: function (response) {
                                    if (response == 1) {
            ...
            dataModel: {
                ...
                location: "remote",
                recIndx: "sott",
                url: $dir_module_nhomtaisan+"list.php",//-- Load danh sách lên lưới
                getData: function (dataJSON) {
                var data = dataJSON.data;
                return {data: data };
            }
		}};
        var $grid = $("#grid_editing_nhom_taisan").pqGrid(obj);
        $grid.one("pqgridload", function (evt, ui) {
                $find = $("#grid_editing_nhom_taisan .pq-grid-table td:nth-child(1):contains('<?php echo $ma; ?>')");

                if(typeof($find[0])=="undefined")
                {
                    $find = $("#grid_editing_nhom_taisan .pq-grid-table td:nth-child(1)");
                    $find[5].focus();
                    $( "#grid_editing_nhom_taisan").pqGrid( "setSelection", {rowIndx: 0,colIndx: 1} );
                }else{
                    $find[0].focus();
                    $colIndx = $("#grid_editing_nhom_taisan .pq-grid-table td:nth-child(1):contains('<?php echo $ma; ?>')").attr('pq-col-indx');
                    $rowIndx = $("#grid_editing_nhom_taisan .pq-grid-table td:nth-child(1):contains('<?php echo $ma; ?>')").parent().attr('pq-row-indx');
                    $( "#grid_editing_nhom_taisan" ).pqGrid( "setSelection", {rowIndx: $rowIndx,colIndx: $colIndx} );
                }
			});
        ...
        function getRowSelect() { // --------------------Lấy dữ liệu theo dòng trên gird----------------------                 
             var arr = $("#grid_editing_nhom_taisan").pqGrid("selection", {
                 type: 'cell',
                 method: 'getSelection'
		 ...
		 function isEditCell(rowIndex,dataIndx) { // --------------------Lấy dữ liệu theo dòng trên gird----------------------                 
             var isEdit = false;
			  isEdit = $("#grid_editing_nhom_taisan").pqGrid("isDirty"); //Lấy giá trị đang chọn
             
             return isEdit;
         }

		 setTimeout(function(){
		//$("#grid_editing_nhom_taisan .pq-search-hd-field").focus();
	},100);
 //-----------------------------Hết lưới---------------------------------------------------------------------       
            
    });     
</script>    
<div id="dialog-nhom_taisan" title="DANH SÁCH NHÓM TÀI SẢN (ENTER : SỬA VÀ LƯU , F7: SAO CHÉP , F8: XÓA , ESC : THOÁT)"><!-- dialog -->
  <div id="grid_editing_nhom_taisan" style="margin:5px auto;border: 0px !important;"></div>
</div>