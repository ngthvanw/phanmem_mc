          ...
          if ( $('div').hasClass('jconfirm')==false) {
             ...
             if (event.keyCode == Keys.F7) { // copy
                if (rowSelect != false){
                   ...
                   //--------------------------------------------Thay đổi khi copy---------------------------------
                   var _dataRow = {mand : rowData.mand,tennoidung :rowData.tennoidung,mapl :rowData.mapl,rate_tax :rowData. rate_tax,tkno :rowData.tkno,tkco :rowData.tkco,ghichu :rowData.ghichu};
             ...
             if (event.keyCode == Keys.END) { // Lưu
              ...
              if (event.keyCode == Keys.ENTER) {
                  if (!isEditing($grid_pb)) {
                      ...
                      $.ajax({
                          url: $dir_module_manoidung_select + "updaterank.php",
                          data: {'id': $ma},
                          async: false,
             ...
             build: function($trigger, e) {
                 return {
                     callback: function(key, options) {
                         var $grid_pb = $("#grid_editing_manoidung_select").closest('.pq-grid');
    ...
    //----------------------------------------------------Bắt đầu lưới-----------------------------------------
                ...
                if($('div').hasClass('jconfirm')==false){
                    $.confirm({
                        ...
                        buttons: {"Đồng ý": {keys: ['Y'],action: function () {
                                ...
                                var ma = $grid.pqGrid("getRecId", { rowIndx: rowIndx });
        ...
        var obj = {
            ...
            colModel: [//-------------------------------------------Khai báo các cột trong lưới-----------------------
                    ...
                    { title: "Mã ND", dataType: "integer", dataIndx: "mand", width: 80,sortable: true,
                        validations: [
                            { type: 'minLen', value: 6, msg: "Mã kho phải có 6 ký tự !" },
                            { type: 'maxLen', value: 6, msg: "Mã kho phải có 6 ký tự !" },
                            { type: function (ui) {
                                    var value = ui.value,
                    ...
                    { title: "Phân loại", width: 100, dataType: "string", align: "right", dataIndx: "mapl",render:function( ui ){return ui.rowData.tenpl},
                        filter: {
            				type: "select",
                	        condition: 'equal',
                	        prepend: { '': '--Tất cả--' },
                    ...
                    { title: "Thuế suất", width: 0, dataType: "integer", align: "right", dataIndx: "rate_tax",render:function( ui ){return ui.rowData.rate_tax +"%"}},
        ...
        var $grid = $("#grid_editing_manoidung_select").pqGrid(obj);
...
<div id="dialog-manoidung_select" title="Thông tin nội dung (F2: Sửa , F7: Sao chép , F8: Xóa , F9: Lưu , END : Hủy dòng đang sửa)"><!-- dialog -->
