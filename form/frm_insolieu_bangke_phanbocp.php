...
if($tonghopcanam=="false"){
    $ngayhoadon="Tháng ".$denthang."-".$_SESSION['NienDo'];
}else{
    $ngayhoadon="Năm ".$_SESSION['NienDo'];
}
?>
<style>
    #dialog-insolieu_bangkhauhao_taisan fieldset {
...
<script>
    $(function () {
        ...
        $("#dialog-insolieu_bangkhauhao_taisan").dialog({
            resizable: false,
            height: "auto",
            width: 650,
            modal: true
        });
        $("#dialog-insolieu_bangkhauhao_taisan").keydown(function (event) {//--------------Các phím tắt
            if (event.keyCode == Keys.ESCAPE) {
                xoadialog_insolieu_thuchi();
            }
        });
        function xoadialog_insolieu_thuchi() {
            reset_dialog(".dialog-insolieu_bangkhauhao_taisan");
        ...
        $("#batđauin_insolieu_thuchi").click(function () {
            ...
            $.ajax({// Lấy thông tin phiếu và lưu vào session
                url: $dir_module_danhmuccptratruoc + "laythongtinkhauhaotaisan.php",
                data: {
                    tuthang:"<?php echo $tuthang ?>",
                    denthang:"<?php echo $denthang ?>",
                    tonghopcanam:"<?php echo $tonghopcanam ?>",
                    loai:"<?php echo $loai ?>",
                    tenphieu:$TenPhieu_ThuChi,
                    ngaylap:$NgayLap_InPhieuThuChi,
                    ngayhd:$NgayHoaDon_InPhieu_ThuChi
                },
                async: false,
            ...
            if($("#XuatExcel").prop("checked") == true){
                window.open($dir_module_danhmuccptratruoc+"xuatexceltkchitiet_theonhom.php");
            }else{
                loadiFrame('TCPDF/baocao/inbang_bangphanbo_cptratruoc.php?denthang=<?php echo $denthang; ?>');
        ...
        $("#xemtruockhiin_insolieu_thuchi").click(function () {
			...
			window.open("TCPDF/baocao/inbang_bangphanbo_cptratruoc.php?denthang=<?php echo $denthang; ?>","bangchiphitratruoc","menubar=0,resizable=1");
        });
...
<div id="dialog-insolieu_bangkhauhao_taisan" title="In số liệu">
    <p>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <input type="text" name="TenPhieu_ThuChi" style="width:100%" id="TenPhieu_ThuChi"
                                       value="<?php if($loai=='xdcb'){ echo "BẢNG PHÂN BỐ XÂY DỰNG CƠ BẢN DỞ DANG"; }else{ echo "BẢNG PHÂN BỐ CHI PHÍ TRẢ TRƯỚC";} ?>"/>
                            </td>
                        </tr>
                        <tr>
                            <td>Ngày HĐ</td>
