...
<script>
	...
	function capnhatdauky() {
		$.confirm({
			title: 'Xác nhận',
			type: 'orange',
			content: 'Bạn có chắc chắn muốn cập nhật đầu kỳ không?',
			buttons: {
				"Xác nhận": {
					btnClass: 'btn-green',
					action: function () {
						$.confirm({
							...
							type: 'green',
							autoClose: 'OK|1000',
							content: 'url:' + $dir_module_makh + 'capnhat_congnokh_vnd_dauky.php',
							contentLoaded: function (data, status, xhr) {
								this.setContentAppend('Cập nhật đầu kỳ thành công.');
							},
							buttons: {
								"OK": {
									keys: ['Y'], action: function () {
										$("#grid_editing_soduno_kh_dauky").pqGrid("refreshDataAndView");	
									}
								}
							}
						});
        ...
        var objmakhdk = {
            ...
            toolbar: {
                items: [
                        ...
                        }, attr: 'id=matkcongno', listeners: [
                            {
                                change: function (evt) {
                                        $grid.pqGrid( "option", "dataModel.postData", function( ui ){
                                            ...
                                            return {xemtatca: $xemtatca,xemchitiet: $xemchitietno, tkcongno:$matkcongno};
                                        } );
                            ...
                            change: function (evt) {
                                if ($(this).is(":checked")) {
                                    $grid.pqGrid( "option", "dataModel.postData", function( ui ){
                                        ...
                                        return {xemtatca: "0",xemchitiet: "1",tkcongno:$matkcongno};
                                    } );
                                }
                                else {
                                    $grid.pqGrid( "option", "dataModel.postData", function( ui ){
                                        $("#xemtatca").prop("checked",false);
                                        $matkcongno  = $("#matkcongno").val();
                                        return {xemtatca: "0",xemchitiet: "0",tkcongno:$matkcongno};
                                    } );
                                }
                                $grid.pqGrid("refreshDataAndView");
                            }
                            ...
                            change: function (evt) {
                                if ($(this).is(":checked")) {
                                    $grid.pqGrid( "option", "dataModel.postData", function( ui ){
                                        ...
                                        $matkcongno  = $("#matkcongno").val();
                                        return {xemtatca: "1",xemchitiet: "1",tkcongno:$matkcongno};
                                    } );
                                }
                                else {
                                    $grid.pqGrid( "option", "dataModel.postData", function( ui ){
                    ...
                    },
                    {
                        ...
                        listeners: [{
                            "click": function (evt) {
                                window.open('form/frm_tai_excel_nokh.php', 'NoKHDK', 'height=1000','width=2000')
                            }
                        }]
                    },
					{
                        type: 'button',
                        label: "Cập nhật số dư đầu kỳ",
            ...
            change: function (evt, ui) {// Khi dữ liệu thay đổi                
                ...
                var obj = rowList[0],
                    ...
                    rowData = obj.rowData;
                try {
                    $tygiapt = parseFloat(rowData.tygiapt.toString().split(",").join(""));// Lấy số lượng nhập vào
                    $tygiaptr = parseFloat(rowData.tygiaptr.toString().split(",").join(""));// Lấy số lượng nhập vào

                    $thanhtienntpt = parseFloat(rowData.thanhtienntpt.toString().split(",").join(""));// Lấy đơn giá nhập vào
                    $thanhtienntptr = parseFloat(rowData.thanhtienntptr.toString().split(",").join(""));// Lấy đơn giá nhập vào
            ...
            colModel: [//-------------------------------------------Khai báo các cột trong lưới-----------------------
                ...
                {
                    title: "Mã KH", dataType: "string", dataIndx: "makh", minWidth: 100, sortable: true,editable: false,
                    validations: [
                ...
                {
                    title: "Mã số thuế", minWidth: 120, dataType: "string", dataIndx: "masothue",editable: false,
                    filter: {
                        type: 'textbox',
                        condition: 'begin',
                        listeners: ['keyup']
                ...
                {
                    title: "Tên KH", minWidth: 300, dataType: "string", dataIndx: "tenkh",editable: false,
                    validations: [
                ...
                {
                    title: "Tỷ giá PT", minWidth: 120, dataType: "integer", dataIndx: "tygiapt", align: "right", hidden: true,
                    render: function (ui) {
                ...
                {
                    title: "Tiền NT PT", minWidth: 120, dataType: "float", dataIndx: "thanhtienntpt", align: "right", hidden: true,
                    render: function (ui) {
                ...
                {
                    title: "Tỷ giá PTr", minWidth: 120, dataType: "integer", dataIndx: "tygiaptr", align: "right", hidden: true,
                    render: function (ui) {
                ...
                {
                    title: "Tiền NT PTr", minWidth: 120, dataType: "float", dataIndx: "thanhtienntptr", align: "right", hidden: true,
                    render: function (ui) {
                ...
                {
                    title: "Mã TK", minWidth: 100, dataType: "string", dataIndx: "matk",align: "center",
                    validations: [
            ...
            dataModel: {
                ...
                postData: {xemchitiet: "1",loaitien:"",tkcongno:'131'},
        ...
        function calculateSummary() {
            ...
            $TongNTPhaiThu = 0;
            $TongNTPhaiTra = 0;
            data = $("#grid_editing_soduno_kh_dauky").pqGrid('option', 'dataModel.data');
            try {
                data.forEach(row => {
                    $TongPhaiThu+= parseFloat(row['sdkno']);
                    $TongPhaiTra+= parseFloat(row['sdkco']);
                    $TongNTPhaiThu+= parseFloat(row['thanhtienntpt']);
                    $TongNTPhaiTra+= parseFloat(row['thanhtienntptr']);
                })
            }catch (e) {
        ...
        objmakhdk.cellSave = function (evt, ui) {
...
<div id="dialog-soduno_khachhang" title="SỐ DƯ NỢ KHÁCH HÀNG ĐẦU KỲ - VIỆT NAM ĐỒNG (F8: XÓA )">
    <!-- dialog -->
