...
<div id="dialog-bangke_hanghoa_banra" title="Bảng kê phụ lục giảm thuế GTGT">
    ...
    <form method="post" id="Form-chinh" enctype="multipart/form-data">
        ...
        <fieldset style="background-color: #afd9ee">
            ...
            <table border="0" style="width: 100%;">
                <tr>
                    <td width="75px;"><b style="font-size: 14px;">Chi nhánh</b></td>
                    <td>
...
///-------------------Kết thúc--------------------------------
            ...
            if (valid) {
                $.confirm({
                    ...
                    content: function () {
                        ...
                        // Lệnh Ajax đầu tiên
                        return $.ajax({
                            url: $dir_module_baocaothue + "laythongtin_phuluc_giamthue_gtgt.php",
                            dataType: 'json',
                            method: 'get',
                            data: {
                                tungay: $tungay,
                            ...
                            success: function (response) {
                                //console.log("Kết quả từ lệnh đầu tiên:", response);
                            },
                            error: function (xhr, status, error) {
                                //console.error("Lỗi khi gọi lệnh đầu tiên:", xhr.responseText);
                            }
                        }).then(function (response1) {
                            // Kiểm tra dữ liệu từ lệnh đầu tiên
                            //console.log('Dữ liệu trả về từ lệnh đầu tiên:', response1);

                            if (response1 && response1.data1) {
                                // Gọi Ajax thứ hai
                                return $.ajax({
                                    url: $dir_module_baocaothue + "xuatexcel_phuluc_giamthue_gtgt.php",
                                    dataType: 'json',
                                    method: 'post',
                                    data: {
                                    },
                                    success: function (response2) {
                                        self.setContentAppend('Excel đã tạo thành công. Nhấp vào [Tải Excel] để tải về.');
                                        //console.log("Kết quả từ lệnh thứ hai:", response2);
                                    },
                                    error: function (xhr, status, error) {
                                        //console.error("Lỗi khi gọi lệnh thứ hai:", xhr.responseText);
                                    }
                                });
                            } else {
                                //console.error('Dữ liệu không hợp lệ từ lệnh đầu tiên');
                                return $.Deferred().reject('Dữ liệu không hợp lệ');
                            }
                        }).then(function (response2) {
                            //this.setContentAppend('Tập tin excel đã tạo thành công. Nhấp vào [Tải Excel] để tải về.');
                            //console.log("Lệnh thứ hai hoàn tất:", response2);
                        });
                    },
                    buttons: {
                        "Tải Excel": {
                            ...
                            btnClass: 'btn-blue',
                            action: function () {
                                window.open("datafile/<?php echo $_SESSION['MST']; ?>/<?php echo $_SESSION['NienDo']; ?>/Bang_Ke_01_GiamThue_GTGT_NQ142_GTGT_TT80.xls");
                            }
                        },
                        "Đóng": {
                            keys: ['N'],
        ...
        function xoadialog_bangketoankho() {// đóng form
            reset_dialog(".dialog-bangke_hanghoa_banra");
            reset_dialog(".dialog_main_bangke_hanghoa_banra");
        }
        dialog = $("#dialog-bangke_hanghoa_banra").dialog({
            autoOpen: false,
            height: "auto",
            width: $width,
            modal: true,
