...
<div id="dialog-bangke_hanghoa_banra" title="Bảng kê hóa đơn, dịch vụ, hàng hóa bán ra...">
    ...
    <form method="post" id="Form-chinh" enctype="multipart/form-data">
                ...
                <tr>
                    ...
                    <td>
                        <select name="sapxeptheohoadon" id="sapxeptheohoadon" style="width:100%;height:25px;">
                            ...
                            <option value="seri,sct">Ký hiệu, Số hóa đơn</option>
        ...
        <fieldset style="background-color: #afd9ee">
            <legend>Trực tiếp trên doanh thu</legend>
            <table>
                <tr>
                    <td><input type="checkbox" name="tructieptrendoanhthu" id="tructieptrendoanhthu"></td>
                    <td>&nbsp;Trực tiếp trên doanh thu</td>
                </tr>
            </table>
        </fieldset>
    </form>
...
///-------------------Kết thúc--------------------------------
        ...
        function ChucNang_ThuChi() {// Xử lý khi nhấp button đồng ý
			...
			$tructieptrendoanhthu = false;
			if ($("#tructieptrendoanhthu").is(":checked")) {
				// Checkbox đang được chọn
				$tructieptrendoanhthu = true;
			} else {
				// Checkbox không được chọn
				$tructieptrendoanhthu = false;
			}
            if (valid) {
                $('.dialog_main_thongbao').load("form/frm_insolieu_bangke_banra.php?tungay=" + $tungay + "&denngay=" + $denngay + "&intheothuesuat=" + $Intheothuesuat + "&intheochungtu=" + $Intheochungtu+ "&sapxeptheohoadon=" + $sapxeptheohoadon+ "&loaingoaite=" + $LoaiNgoaiTe+ "&kieuin=" + $kieuin+"&loaibangke="+$loaibangke+"&chinhanh="+$chinhanh+"&tructieptrendoanhthu="+$tructieptrendoanhthu);
            }
