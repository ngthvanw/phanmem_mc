...
<div id="dialog-baocao_sudung_hoadon" title="BẢNG BÁO CÁO TÌNH HÌNH SỬ DỤNG HÓA ĐƠN...">
    ...
    <form method="post" id="Form-chinh" enctype="multipart/form-data">
        <fieldset style="background-color: #afd9ee">
            ...
            <table border="0" style="width: 100%;">
                <tr style="display:block;">
                    ...
                    <td>&nbsp; Từ quý &nbsp;</td>
                    <td>
                        <select name="tuQuy" id="tuQuy" style="height: 20px;">
                    ...
                    <td>Đến quý &nbsp;</td>
                    <td>
                        <select name="denQuy" id="denQuy" style="height: 20px;">
                            <option value="1">Quý I</option>
                            <option value="2">Quý II</option>
                            <option value="3">Quý III</option>
                            <option value="4">Quý IV</option>
        ...
        <fieldset style="background-color: #afd9ee;display:block">
            <legend>Thiết lập số dư hóa đơn</legend>
            <table border="0" style="width: 100%;">
                <tr>
                    <td>
                        <button id="thietlapsoduhoadon" type="button" style="width:100%;border:1px solid #990" class="ui-button ui-corner-all ui-widget">Thiết lập số dư hóa đơn</button>
                    </td>
                </tr>
            </table>
        </fieldset>
        <fieldset style="background-color: #afd9ee;display:block">
            <legend>Số hóa đơn mua vào trong kỳ</legend>
            <table border="0" style="width: 100%;">
                <tr>
                    <td>
                        <button id="sohoadonmuavaotrongnam" type="button" style="width:100%;border:1px solid #990" class="ui-button ui-corner-all ui-widget">Số hóa đơn mua vào trong kỳ</button>
                    </td>
                </tr>
            </table>
        </fieldset>
        <fieldset style="background-color: #afd9ee;display:block">
            <legend>Đánh dấu hóa đơn xóa, mất, đã sử dụng hoặc trả lại</legend>
            <table border="0" style="width: 100%;">
                <tr>
                    <td>
                        <button id="sohoadonmattralai" type="button" style="width:100%;border:1px solid #990" class="ui-button ui-corner-all ui-widget">Số đánh dấu xóa, mất, đã sử dụng hoặc trả lại</button>
                    </td>
                </tr>
            </table>
        </fieldset>
        <fieldset style="background-color: #afd9ee;display:block">
            ...
            <table border="0" style="width: 100%;">
                ...
                <tr>
                    <td><input style="margin:6px;" name="bangdoisoat" type="checkbox" value="" id="bangdoisoat" />Đối soát hoá đơn xoá trong kỳ</td>
                </tr>
                <tr>
                    <td><button id="thietlapbangdoisoat" type="button" style="width:100%;border:1px solid #990" class="ui-button ui-corner-all ui-widget">Thiết lập đối soát hoá đơn xoá trong kỳ</button></td>
                </tr>
            </table>
        </fieldset>
        ...
        <fieldset style="background-color: #afd9ee;display:block">
            ...
            <table border="0" style="width: 100%;">
                <tr>
                    <td><input style="margin:6px;" name="tonghopcanam" type="checkbox" value="" id="tonghopcanam" />Tổng hợp báo cáo cả năm</td>
                </tr>
            </table>
        </fieldset>
    </form>
...
<script>
    ...
    $dir_module_phieuthuchi = "modules/baocaothue/";//----------------Lưới
...
///-------------------------Di chuyễn các phần tử bằng enter----------------
        ...
        $("#thietlapbangdoisoat").click(function (event) {// Gọi table mã nội dung để chọn
            $('.dialog_main_sodu_hoadon').load('form/frm_dm_doisoat_xoa_trongky.php');
        })
///-------------------Kết thúc--------------------------------
        ...
        $("#tuQuy").change(function (event) {// Gọi table mã nội dung để chọn
            $tuQuy = $("#tuQuy").val();

            $("#denQuy option[value=" + $tuQuy + "]").attr('selected', 'selected');
        })
        function ChucNang_ThuChi() {// Xử lý khi nhấp button đồng ý
            ...
            $tungay = $("#tuQuy").val();
            $denngay = $("#denQuy").val();
			$xoakhiphathien = $("#xoakhiphathien").prop("checked");
			$tonghopcanam = $("#tonghopcanam").prop("checked");
			$bangdoisoat = $("#bangdoisoat").prop("checked");

            if (valid) {
                for ($i = $tungay; $i <= $denngay; $i++) {
                    $.ajax({// Chuyển tồn kho cho năm mới
                        url: $dir_module_baocaothue + "tinhtinhhing_sudunghoadon.php",
                        async: false,
                        data: {thangtk: $i,xoakhiphathien:$xoakhiphathien,tonghopcanam:$tonghopcanam},
                ...
                if($bangdoisoat){
                    $('.dialog_main_thongbao').load("form/frm_insolieu_baocao_hoadon_doisoat.php?tungay=" + $tungay + "&denngay=" + $denngay+"&tonghopcanam="+$tonghopcanam );
                }else{
                    $('.dialog_main_thongbao').load("form/frm_insolieu_baocao_hoadon.php?tungay=" + $tungay + "&denngay=" + $denngay+"&tonghopcanam="+$tonghopcanam );
                }
            }
