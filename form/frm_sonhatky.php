...
<div id="dialog-sonhatky" title="Sổ nhật ký...">
    ...
    <form method="post" id="Form-chinh" enctype="multipart/form-data">
        ...
        <fieldset style="background-color: #afd9ee">
            <legend>In sổ nhật ký chung</legend>
            <table border="0" style="width: 100%;">
                <tr>
                    <td colspan="2"><select name="Intheothuesuat" style="width:100%;height:25px;" id="Intheothuesuat">
                      <option value="1">Không sử dụng sổ nhật ký thu chi</option>
                      <option value="2">Có sử dụng sổ nhật ký thu chi</option>
                      <option value="3">Sổ nhật ký bút toán do chương trình tạo</option>
                    </select></td>
                </tr>
                <tr style="display:none">
                    <td><b>Chứng từ :
...
///-------------------Kết thúc--------------------------------
        ...
        function ChucNang_ThuChi() {// Xử lý khi nhấp button đồng ý
            ...
            if (valid) {
                $('.dialog_main_thongbao').load("form/frm_insolieu_sonhatky.php?tungay=" + $tungay + "&denngay=" + $denngay + "&intheothuesuat=" + $Intheothuesuat + "&intheochungtu=" + $Intheochungtu+ "&sapxeptheohoadon=" + $sapxeptheohoadon+ "&theothongtu=" + $theothongtu+ "&kieuin=" + $kieuin);
            }
        ...
        function xoadialog_bangketoankho() {// đóng form
            reset_dialog(".dialog-sonhatky");
            reset_dialog(".dialog_main_sonhatky");
        }

        dialog = $("#dialog-sonhatky").dialog({
            autoOpen: false,
            height: "auto",
            width: $width,
            modal: true,
