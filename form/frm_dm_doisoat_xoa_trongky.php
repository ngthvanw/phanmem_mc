...
<script>
    ...
    $(function () {
        ...
        $("#dialog-sodu_hoadon").keydown(function (event) {//--------------Các phím tắt
            ...
            if (event.keyCode == Keys.F8 || event.keyCode == Keys.INSERT) {
            ...
            if ($('div').hasClass('jconfirm') == false) {
			 //if (event.keyCode == Keys.F4) {
              ...
              if (event.keyCode == Keys.F7) { // copy
                    $Quy = $("#denQuy").val();
                    $confirm = confirm("Bạn có muốn sao chép tình hình sử dụng hoá đơn không?\nHệ thống sẽ xoá dữ liệu hiện tại nếu thực hiện lệnh sao chép này.\nChọn [OK] để tiếp tục, [HUỶ] để huỷ bỏ lệnh.");
                    if($confirm){
                        $.confirm({
                            title: 'Sao chép dữ liệu thành công. ',
                            type: 'green',
                            autoClose: 'OK|1000',
                            content: function () {
                                var self = this;
                                return $.ajax({
                                    url: $dir_module_sudu_hoadon_select + "saochep_bcthsd_hoadon.php",
                                    async: false,
                                    data: {
                                        quy:$Quy,
                                    },
                            ...
                            buttons: {
                                "OK": {
                                    keys: ['Y'], action: function () {
                                        $("#grid_editing_sodu_hoadon").pqGrid("refreshDataAndView");
                                    }
                                }
                            }
                        });
		...
		function addRow(rowIndx,$name='mavt',$grid,$obj_addrow="") {
            ...
            var rowData = {loaiphieu: "1", soquyen: "", kyhieu: "", tuso: "", denso: "", soluong: "", mauso: "",quy:"1",loaiphieu: "XOA"};
        ...
        //--------------------------------Khai báo lưới---------------------------------------.
 		var obj = {
					...
					if (type == 'update') {                        
                        ...
                        if (valid) {
                            if (rowData[recIndx] == null) {
                                ...
                                url = $dir_module_sudu_hoadon_select+"edit_doisoat_hoadon_xoa_trongky.php";
                            }
                        }
                    }
                if (valid) {
            ...
            colModel: [//-------------------------------------------Khai báo các cột trong lưới-----------------------
                ...
                {
                    title: "Loại hóa đơn", dataType: "string", dataIndx: "loaphieu", width: 150, sortable: true,editable: false,
                    filter: {
                        type: 'textbox',
                        condition: 'begin',
                        listeners: ['keyup']
                ...
                {title: "Mẫu số", width: 100, dataType: "string", align: "right", dataIndx: "mauso",editable: false,
                    validations: [
                ...
                {title: "Hoá đơn", width: 150, dataType: "integer", align: "right", dataIndx: "hoadon",hidden: false,
                    filter: {
                        type: 'textbox',
                        condition: 'begin',
                        listeners: ['keyup']
                ...
                {title: "Hợp lệ", width: 120, dataType: "string", align: "center", dataIndx: "hople",hidden: false,
                    editor: {type: "select",options: function (ui) {
                            //remote validation
                            var parsedJson = [ { 0:"KHÔNG CHỌN"},{ 2:"KHÔNG HỢP LỆ"}, {1: "HỢP LỆ"}] ;
                            return parsedJson;
                        }},
                    render: function (ui) {
                        var value = ui.rowData.hople;
                        if (value == 1) {
                ...
                {title: "Mất liên đỏ", width: 120, dataType: "string", align: "center", dataIndx: "liendo",
                    editor: {type: "select",options: function (ui) {
                            //remote validation
                            var parsedJson = [ { 0:"KHÔNG CHỌN"},{ 2:"KHÔNG HỢP LỆ"}, {1: "HỢP LỆ"}] ;
                            return parsedJson;
                ...
                {title: "Thay thế", width: 150, dataType: "integer", align: "right", dataIndx: "thaythe",
                    filter: {
                        type: 'textbox',
                        condition: 'begin',
                        listeners: ['keyup']
                ...
                {title: "Ghi chú", width: 200, dataType: "string", align: "left",editable:true, dataIndx: "ghichu",
            ...
            dataModel: {
                ...
                location: "remote",
                recIndx: "sott",
                url: $dir_module_sudu_hoadon_select + "list_doisoat_hoadon_xoa_trongky.php",//-- Load danh sách lên lưới
                postData: {quy: $("#denQuy").val()},
...
<div id="dialog-sodu_hoadon"
     title="THIẾT LẬP ĐỐI SOÁT XOÁ TRONG KỲ (F7: SAO CHÉP DỮ LIỆU TỪ BCTHSD HOÁ ĐƠN , F8: XOÁ ,ESC : THOÁT)"><!-- dialog -->
