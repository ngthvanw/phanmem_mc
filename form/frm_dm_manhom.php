...
<script>
    ...
    $(function () {
        ...
        function xoadialog_manhomvt() { // ----------------------đóng form
            reset_dialog(".dialog-manhom_vatu");
            reset_dialog(".dialog_main_manhomvt");
        }

        $("#dialog-manhom_vatu").dialog({ // ------------------Gọi dialog
            resizable: false,
            height: $height,
            width: $width,
            modal: true
        ...
        $("#dialog-manhom_vatu").keydown(function (event) {//--------------Các phím tắt
            var $grid_pb = $("#grid_editing_manhom_vattu").closest('.pq-grid');//---- Lưới----------------
            ...
            var rowEditting = $("#grid_editing_manhom_vattu").pqGrid("getRowsByClass", {cls: 'pq-row-edit'});//---Lấy đối tượng đang sửa
            if ($('div').hasClass('jconfirm') == false) {
                if (event.keyCode == Keys.F4) {
                    rowIndx = 0;
                    var colM = $("#grid_editing_manhom_vattu").pqGrid("option", "colModel");
                    colM[1].editable = true;
                    $("#grid_editing_manhom_vattu").pqGrid("option", "colModel", colM);
        ...
        function addRow(rowIndx, $name='manhanvien', $grid, $obj_addrow="") {
            $ma = "";
            $.ajax({
                url: $dir_module_manhomvattu + "taoma.php",
                async: false,
                    ...
                    buttons: {
                        "Đồng ý": {
                            keys: ['Y'], action: function () {
                                ...
                                $.ajax($.extend({}, ajaxObj, {
                                    context: $grid,
                                    url: $dir_module_manhomvattu + "del.php",
                                    data: {id: sott, ma: ma},
        ...
        var objmanhomvt = {
                ...
                if (type == 'update') {
                    ...
                    if (valid) {
                        if (rowData[recIndx] == null) {
                            url = $dir_module_manhomvattu + "add.php";
                        }
                        else {
                            url = $dir_module_manhomvattu + "edit.php";
                        }
                    }
                }
                if (valid) {
            ...
            colModel: [//-------------------------------------------Khai báo các cột trong lưới-----------------------
...

                {
                        ...
                        {type: 'maxLen', value: 4, msg: "Mã nhóm phải có 4 ký tự !"},
                        {
                            type: function (ui) {
                                ...
                                if (isEdit) {
                                    ...
                                    //remote validation
                                    $.ajax({
                                        url: $dir_module_manhomvattu + "checkkey.php",
                    ...
                    render: function (ui) {
                        ...
                        if (rowData.manhom == 1100 || rowData.manhom == 1200) {//if change is negative.
                            rowData.pq_cellcls[dataIndx] = 'green';
                            return rowData.manhom;
                        }
                        else { //if change >= 0
                            return rowData.manhom;
                        }
                ...
                {
                    title: "Tên nhóm", width: 300, dataType: "string", dataIndx: "tennhom",
                    validations: [
                        {type: 'minLen', value: 1, msg: "Tên nhóm không được trống !"}
                    ],
                    editable: function (ui) {
                            $manhom = ui.rowData.manhom;
                        //console.log(ui);
                        if ($manhom == 1100 || $manhom == 1200) {
                            return false;
                        }
                        else {
                            return true;
            ...
            dataModel: {
                ...
                location: "remote",
                recIndx: "sott",
                url: $dir_module_manhomvattu + "list.php",//-- Load danh sách lên lưới
        ...
        var $grid = $("#grid_editing_manhom_vattu").pqGrid(objmanhomvt);

        //use refresh & refreshRow events to display jQueryUI buttons and bind events.
        function getRowSelect() { // --------------------Lấy dữ liệu theo dòng trên gird----------------------
            var arr = $("#grid_editing_manhom_vattu").pqGrid("selection", {
                type: 'cell',
                method: 'getSelection'
        ...
        function isEditCell(rowIndex, dataIndx) { // --------------------Lấy dữ liệu theo dòng trên gird----------------------
            var isEdit = false;
            isEdit = $("#grid_editing_manhom_vattu").pqGrid("isDirty"); //Lấy giá trị đang chọn
        ...
        //-----------------------------Hết lưới---------------------------------------------------------------------
        setTimeout(function () {
            $("#grid_editing_manhom_vattu .pq-search-hd-field").focus();
        }, 100);
    });
</script>
<div id="dialog-manhom_vatu"
     title="Danh sách nhóm vật tư, hàng hóa.... (ENTER : Sửa,Lưu , F4 : Thêm mới , F7: Sao chép , F8: Xóa , ESC: Thoát">
    <!-- dialog -->
    <div id="grid_editing_manhom_vattu" style="margin:5px auto;border: 0px !important;"></div>
</div>
