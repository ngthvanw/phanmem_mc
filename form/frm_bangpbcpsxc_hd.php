    ...
    $(function () {
        ...
        $("#bangphantichdoanhthu").click(function (event) {// Gọi table mã nội dung để chọn
            ...
            if ($loaisp == "CT") {
				...
				$('.dialog_main_dinhmuc_sanpham').load('form/frm_dm_bangphantich_doanhthu_thucte_hopdong.php');
			}else{
                ...
                if (confirm("Dữ liệu đã tồn tại. bạn có muốn ghi đè không ?")) {
                    $.confirm({
                        ...
                        content: function () {
                            var self = this;
                            return $.ajax({
                                url: $dir_module_dmsanpham + "themthongtin_phanbo_cpsxchung_hopdong.php",
                                dataType: 'json',
                                method: 'get',
                                data: {
                                    tungay: $tungay,
