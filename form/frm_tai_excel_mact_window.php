    ...
    if (isset($_FILES['excel_file'])) {
        ...
        if ($xlsx = SimpleXLSX::parse($file)) {
			...
			$rows = array_filter($rows, function($row) {
				// Kiểm tra xem tất cả các ô trong dòng có trống không
				foreach ($row as $cell) {
					if (trim($cell) !== '') {
            ...
            for ($i = 2; $i < $count_data; $i++) {
                ...
                $sql = "INSERT INTO masp ($columns) VALUES ($values);";

                // Thực hiện câu lệnh SQL				
				if ($BCT->re_query($sql)) {
					$BCT->re_query("UPDATE masp set sott=stt where sott=0;");
					$results[] = [
						'status' => 'Thành công',
						'data' => $row
					];
...
<div class="container">
    ...
    <a href="../tmp/tmp_masp.xlsx" download>Tải Excel mẫu</a>
</p>
</div>
