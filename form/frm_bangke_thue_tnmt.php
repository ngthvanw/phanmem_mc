...
<div id="dialog-bangke_thue_gtgt" title="Tờ khai phí bảo vệ môi trường ">
    ...
    <form method="post" id="Form-chinh" enctype="multipart/form-data">
        ...
        <fieldset style="background-color: #afd9ee;">
            ...
            <table style="font-size: 14px" border="1" width="780px">
                <tr>
                    <th style="text-align: center" rowspan="2">STT</th>
                    <th style="text-align: center" rowspan="2">Loại khoán sản</th>
                    <th style="text-align: center" colspan="2">Số lượng khoán sản</th>
                    <th style="text-align: center" rowspan="2"> Mức phí</th>
                    <th style="text-align: center" rowspan="2"> Hệ số tính phí</th>
                    <th style="text-align: center" rowspan="2">Số phí phải nộp trong kỳ</th>
                </tr>
                ...
                <tr>
                    ...
                    <td style="text-align: center;width: 100px;" >(6)</td>
                    <td style="text-align: center;width: 70px;" >(7)</td>
                    <td style="text-align: center;width: 70px;" >(8)</td>
                    <td style="text-align: center;width: 150px;" >(9)=(6)X(7)x(8)</td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td style="text-align: center;" ><input style="width: 100%;text-align: right" type="text" onkeyup="return format_munber(this.value,'#hesotinhphi')" name="hesotinhphi" id="hesotinhphi"></td>
                    <td style="text-align: center;" ><input style="width: 100%;text-align: right" type="text" readonly="true" name="thanhtientukhaithac" id="thanhtientukhaithac"></td>
                </tr>
...
///-------------------------Di chuyễn các phần tử bằng enter----------------
        $("#soluongtukhaithac, #mucphitukhaithac, #hesotinhphi").keyup(function (event) {
			var $soluong = $("#soluongtukhaithac").val().trim().replace(/,/gi, "");
			var $dongia = $("#mucphitukhaithac").val().trim().replace(/,/gi, "");
			var $hesotinhphi = $("#hesotinhphi").val().trim().replace(/,/gi, "");

			var $thanhtien = Math.round($soluong * $dongia * $hesotinhphi);

			$("#thanhtientukhaithac").val($.number($thanhtien, 0, ".", ","));
		});
...
///-------------------Kết thúc--------------------------------
        ...
        $("#thangtinhthue,#loaitokhai").change(function () {
            ...
            if ($loaitokhai == 1) {//// Tờ khai chính thức
                ...
                $data = "";
                $.ajax({// Thêm dữ liệu vào tokhaithue
                    url: $dir_module_baocaothue + "laythongtininphieukekhaithue_bvmt_json.php?thangtinhthue=" + $thangtinhthue + "&loaitokhai=" + $loaitokhai+"&namtinhthue="+$namtinhthue,
                    async: false,
                    success: function (response) {
                        if (response != 0) {
							...
							$("#hesotinhphi").val(FormatNumber($data.I.hesotinhphi));
        ...
        function ChucNang_ThuChi() {// Xử lý khi nhấp button đồng ý
            ...
            if (valid) {
                ...
                $.ajax({// Chuyển tồn kho cho năm mới
                    url: $dir_module_baocaothue + "kiemtracotontaidulieukhaithue_bvmt.php",
                    ...
                    if (retVal == true) {
                        $.confirm({
                            ...
                            content: function(){
                                var self = this;
                                return $.ajax({
                                    url: $dir_module_baocaothue + "themtokhaithue_bvmt.php?loaiks=" + $loaiks + "&dvt=" + $dvt + "&tenloai=" + $tenloai + "&soluong=" + $soluong + "&mucphi=" + $mucphi+ "&hesotinhphi=" + $hesotinhphi + "&thanhtien=" + $thanhtien + "&thangtinhthue=" + $thangtinhthue + "&loaitokhai="+$loaitokhai+ "&maloaiks="+$maloaiks+ "&tendvt="+$tendvt+ "&namtinhthue="+$namtinhthue,
                                    async: false,
                                });
                            },
                            buttons: {
                                "OK": {
                                    keys: ['Y'], action: function () {
                                        $('.dialog_main_thongbao').load("form/frm_insolieu_bangke_thuetnmt.php?thangtinhthue=" + $thangtinhthue+"&loaitokhai="+$loaitokhai+ "&namtinhthue="+$namtinhthue);
                                    }
                                }
                            }
                        });
