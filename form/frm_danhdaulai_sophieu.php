...
<div id="dialog-phuchoi" title="ĐÁNH LẠI THỨ TỰ PHIẾU...">
	...
	<form method="post" id="Form-Danhsothutu">
			...
			<legend>Chọn tháng</legend>
				<table border="0" style="width: 100%;">
					<tr>
						<td>Từ</td>
						<td>
							<select name="tuthang" style="height:30px;width:100%;" id="tuthang">
						...
						<td>Đến</td>
						<td>
							<select name="denthang" style="height:30px;width:100%;" id="denthang">
		...
		<fieldset style="background-color: #afd9ee; border: 1px solid #09F">
			...
			<table border="0" width="100%">
				<tr>
					<td width="50%"><input type="checkbox" name="PhieuNhap" class="PhieuNhap" value=""/> Phiếu nhập kho</td>
					<td width="50%"><input type="checkbox" name="PhieuThu" class="PhieuThu" value=""/> Phiếu Chi</td>
				</tr>
				<tr>
					<td><input type="checkbox" name="PhieuGhiCo" class="PhieuGhiCo" value=""/> Phiếu ghi có</td>
					<td><input type="checkbox" name="PhieuNganHangChi" class="PhieuNganHangChi" value=""/> Phiếu ngân hàng rút ra</td>
				</tr>
				<tr>
					<td></td>
					<td style="padding-left: 17px;">
						<select class='TKNganHangChi' id='TKNganHangChi' style="height:30px;">
						<option value="">--CHỌN TÀI KHOẢN NH--</option>
						<option value="ALL">TẤT CẢ TÀI KHOẢN</option>
						<?php
							$TKNH_Chi = 112101;
							for($i=6;$i<=64;$i=$i+2){
								if($i<36 || $i>54){
									echo "<option value='".$i."'>----Ngân hàng: ".$TKNH_Chi."</option>";
									$TKNH_Chi++;
								}
							}
						?>
		...
		<fieldset style="background-color: #afd9ee; border: 1px solid #09F">
			...
			<table border="0" width="100%">
				<tr>
					<td width="50%"><input type="checkbox" name="PhieuXuatSX" class="PhieuXuatSX" value=""/> Phiếu xuất kho SX</td>
					<td width="50%"></td>
				</tr>
				<tr>
				...
				<tr>
					<td><input type="checkbox" name="PhieuGhiNo" class="PhieuGhiNo" value=""/> Phiếu ghi Nợ</td>
					 <td><input type="checkbox" name="PhieuNganHangThu" class="PhieuNganHangThu" value=""/> Phiếu ngân hàng gửi vào</td>
				</tr>
				<tr>
					<td></td>
					<td style="padding-left: 17px;">
						<select class='TKNganHangThu' id='TKNganHangThu' style="height:30px;">
						<option value="">--CHỌN TÀI KHOẢN NH--</option>
						<option value="ALL">TẤT CẢ TÀI KHOẢN</option>
						<?php
							$TKNH_Thu = 112101;
							for($i=5;$i<=63;$i=$i+2){
								if($i<35 || $i>53){
									echo "<option value='".$i."'>----Ngân hàng: ".$TKNH_Thu."</option>";
									$TKNH_Thu++;
								}
							}
						?>
		...
		<fieldset style="background-color: #afd9ee; border: 1px solid #09F">
			...
			<table border="0" width="100%">
				<tr>
					<td><select style="height:30px;width: 100%;" id="loaidanhdau">
...
<script>
    ...
    $(function () {
        ...
        function ChucNang_DanhDauLai() {
			...
			valid = valid && checkNull($("#tuthang"), " Từ tháng đánh số ");
			valid = valid && checkNull($("#denthang"), " Đến tháng đánh số ");
            ...
            $PhieuNganHangChi = $(".PhieuNganHangChi").prop("checked");

            $TuNgay = $("#tuthang").val();
            $DenThang = $("#denthang").val();
            $loaidanhdau = $("#loaidanhdau").val();
			
			 $TKNganHangChi = $(".TKNganHangChi").val();
			 $TKNganHangThu = $(".TKNganHangThu").val();
            ...
            if ($PhieuNganHangThu) {
				...
				valid = valid && checkNull($("#TKNganHangThu"), " TK ngân hàng gửi vào không được trống!");
            ...
            if ($PhieuNganHangChi) {
                $str_phieunganhangchi = 1;
				valid = valid && checkNull($("#TKNganHangChi"), " TK ngân hàng rút ra không được trống!");
            }
                        ...
                        "Đồng ý": {
                            keys: ['Y'], action: function () {
                                $.confirm({
                                    ...
                                    content: 'url:' + $dir_module_psvattu + 'danhdaulai_phieu.php?phieunganhangthu=' + $str_phieunganhangthu + '&phieunhap=' + $str_phieunhap + '&phieuthu=' + $str_phieuthu + '&phieughico=' + $str_phieughico +'&phieunganhangchi=' + $str_phieunganhangchi + '&phieuxuat=' + $str_phieuxuat + '&phieuxuatsx=' + $str_phieuxuatsx + '&phieuchi=' + $str_phieuchi + '&phieughino=' + $str_phieughino +'&tuthang=' + $TuNgay + '&denthang=' + $DenThang+ '&loaidanhdau=' + $loaidanhdau+ '&tknganhangthu=' + $TKNganHangThu+ '&tknganhangchi=' + $TKNganHangChi,
                                    contentLoaded: function () {
                                    },
                                    buttons: {
                                        "Thoát": {
                                            keys: ['Y'], btnClass: 'btn-green', action: function () {
                                                $("#Form-Danhsothutu")[0].reset();
                                            }
                                        }
                                    }
                                });
        ...
        form = dialog.find("#Form-Danhsothutu").on("submit", function (event) {
