    ...
    $(function () {
        ...
        $("#grid_editing_nhapchitiet_nhapkho").keydown(function (event) {//--------------Các phím tắt
            ...
            if (event.keyCode == Keys.F7) { // Hủy bỏ hàng đang xóa
                $res = confirm("CẢNH BÁO! \n\n BẠN ĐANG CHUẨN BỊ SAO CHÉP DANH SÁCH VẬT TƯ HÀNG HOÁ. CHỨC NĂNG NÀY SẼ LÀM THAY ĐỔI DỮ LIỆU PHIẾU NHẬP HIỆN TẠI. \n\n BẠN CÓ MUỐN TIẾP TỤC KHÔNG ?");
                if($res){
				var $loaiphieu = prompt("NHẬP LOẠI PHIẾU ĐỂ SAO CHÉP DANH SÁCH VẬT TƯ, HÀNG HOÁ \n 1. Nhập Kho \n 2. Xuất kho \n 3. Xuất kho SX \n 4. Định mức SP", "4");
				if ($loaiphieu == 1 || $loaiphieu == 2 || $loaiphieu == 3 || $loaiphieu == 4) {
					if($loaiphieu == 4){
						var $MaSP = prompt("NHẬP MÃ SẢN PHẨM ĐỂ SAO CHÉP DANH SÁCH VẬT TƯ, HÀNG HOÁ", "");
						if ($MaSP != null) {
							var SLDuTru = prompt("Nhập vào số lượng cần sản xuất sản phẩm ?", "");
							if((isNaN(SLDuTru) == false || SLDuTru === "")&&SLDuTru != null)
							$sophieuhientai = $("#sophieu").val().trim(); /// Là số phiếu
							$.ajax({// Lấy tổng chiết khấu
								url: $dir_module_nhapkho + "saochepdanhsach_dinhmucsp.php",// Lấy tồng thuế NK,TTĐB,VC,BX
								data: {MaSP: $MaSP,sophieuhientai: $sophieuhientai,SLDuTru:SLDuTru},
								async: false,
								success: function (response) {
									...
									if($data!=""){
										...
										$("#dialog-bangchitiet_nhapkho").load("form/gird_bangchitiet_xuatkho_sanxuat.php?list=0&sophieu=<?php echo $SoPhieu; ?>&thuegtgt=<?php echo $cothuegtgt; ?>&cochietkhau=<?php echo $cochietkhau; ?>&baogomthue=<?php echo $baogomthue; ?>&ngayghiso=<?php echo $ngayghiso; ?>&makho=<?php echo $makho; ?>");
									}
								}
							});
							...
							if (sophieu != null) {
								...
								$.ajax({// Lấy tổng chiết khấu
									...
									data: {mapsktsaochep: sophieu,sophieuhientai: $sophieuhientai,loaiphieu:$loaiphieu,loaiphieuhientai:3},
									async: false,
									success: function (response) {
										$data = response.trim();
										if($data!=""){
...
//change edit button to update button and delete to cancel.
        ...
        var objxuatkhosx = {
            ...
            colModel: [//-------------------------------------------Khai báo các cột trong lưới-----------------------
                ...
                {
                    title: "Mã VT", dataType: "string", dataIndx: "mavt", width: 80, sortable: true, editable: true,
                    filter: {
                        type: 'textbox',
                        condition: 'begin',
                        listeners: ['keyup']
                ...
                },
                {
                    ...
                    render: function (ui) {// Hiện số tiền tính toán lên lưới
                        ...
                        var val = ui.rowData.thang<?php echo $thang_ngayghiso ?>;
						return  DinhDangSo(val);
                    }
                },
                {
        ...
        objxuatkhosx.render = function (evt, ui) {
            $summary = $("<div class='pq-grid-summary'  ></div>")
                .prependTo($(".pq-grid-bottom", this));
            calculateSummary();
        }

        objxuatkhosx.cellSave = function (evt, ui) {
            objxuatkhosx.refresh.call(this);
        }
        objxuatkhosx.refresh = function (evt, ui) {
            calculateSummary();
            var data = [totalData]; //2 dimensional array
            var objxuatkhosx = { data: data, $cont: $summary }
            $(this).pqGrid("createTable", objxuatkhosx);
        }
...
<div id="dialog-bangchitiet_nhapkho"
     title="Nhập đơn giá, số lượng hàng hóa vật tư.... (k: hàng hóa không chịu thuế) (ESC: Thoát , F4: Thêm vật tư, INSERT: Lấy danh sách tồn kho, F2: Sửa, F9: Lưu , END : Hủy dòng đang sửa)">
    <!-- dialog -->
