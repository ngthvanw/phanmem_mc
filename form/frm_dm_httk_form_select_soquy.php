    ...
    $(function () {
		...
		var obj = {
            ...
            dataModel: {
                ...
                location: "remote",
                recIndx: "sott",
                url: $dir_module_httk_select+"list_soquy.php",//-- Load danh sách lên lưới
        ...
        $grid.one("pqgridload", function (evt, ui) {
            ...
            $find = $("#grid_editing_httk_select .pq-grid-table td:nth-child(1):contains('<?php echo $ma; ?>')");
            try{
			if(typeof($find[0])=="undefined")
            {
                $find = $(".pq-grid-table td:nth-child(1)");
                $find[5].focus();
                $( "#grid_editing_httk_select").pqGrid( "setSelection", {rowIndx: 0,colIndx: 1} );
            }else{
                $find[0].focus();
                $colIndx = $("#grid_editing_httk_select .pq-grid-table td:nth-child(1):contains('<?php echo $ma; ?>')").attr('pq-col-indx');
                $rowIndx = $("#grid_editing_httk_select .pq-grid-table td:nth-child(1):contains('<?php echo $ma; ?>')").parent().attr('pq-row-indx');
                $( "#grid_editing_httk_select" ).pqGrid( "setSelection", {rowIndx: $rowIndx,colIndx: $colIndx} );
            }
							}
				catch(err) {
