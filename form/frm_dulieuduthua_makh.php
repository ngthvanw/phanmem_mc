...
<div id="dialog-bangchitiet_hanghoa" title="DỮ LIỆU MÃ KHÁCH HÀNG DƯ THỪA...">
    ...
    <form method="post" id="Form-chinh" enctype="multipart/form-data">
        ...
        <fieldset style="background-color: #afd9ee">
            <legend>Chọn mã khách hàng cần xoá <b STYLE="color: red;">(XỬ LÝ BẢNG TỔNG HỢP CÔNG NỢ TRƯỚC KHI THỰC HIỆN XOÁ DỮ LIỆU)</b></legend>
            <table border="0" style="width: 100%;">
                <tr>
                    <td colspan="2">
        ...
        <fieldset style="background-color: #afd9ee;">
            ...
            <table>
                <tr>
                    ...
                    <td>&nbsp;Tự động xoá dữ liệu không cần thiết</td>
                </tr>
            </table>
        </fieldset>
    </form>
    ...
    $(function () {
        ...
        var obj = {
            ...
            colModel: [//-------------------------------------------Khai báo các cột trong lưới-----------------------
                ...
                { title: "Mã KH", dataType: "string", dataIndx: "makh", width: 150,sortable: true,
                ...
                { title: "Tên KH", width: 400, dataType: "string", dataIndx: "tenkh",
                    validations: [
                ...
                { title: "Mã KH cha", dataType: "string", dataIndx: "makhcha", width: 120,sortable: true,
		...
		{ title: "", dataIndx: "state", width: 5, align: "center", type:'checkBoxSelection', cls: 'ui-state-default', resizable: false, sortable:false,
            render: function (ui) {
                ...
                if (rowData.choxoa!=null) {//if change is negative.
                    rowData.pq_cellcls[dataIndx] = 'choxoa';
                }
            }
        }
            ],//,
            ...
            dataModel: {
                ...
                location: "remote",
                recIndx: "sott",
                url: $dir_module_makhachhang+"list_makhthua.php",//-- Load danh sách lên lưới
        ...
        $grid.one("pqgridload", function (evt, ui) {
            try {
                $(".choxoa>input[type='checkbox']").attr("disabled", true);
            }catch(err){
...
///-------------------Kết thúc--------------------------------
        ...
        function ChucNang_ThuChi() {// Xử lý khi nhấp button đồng ý
            ...
            if (valid) {
                if($InTatCaVatTu){
                    $res = confirm("Bạn có muốn xoá tất cả mã khách hàng không ?");
                    if($res){
                        $.confirm({
                            title: 'Xoá dữ liệu thành công',
                            type: 'green',
                            autoClose: 'KẾT THÚC|1000',
                            content: function(){
                                var self = this;
                                return $.ajax({
                                    url: $dir_module_makhachhang + "xoadulieu_makh_duthua.php",
                                    dataType: 'json',
                                    method: 'get',
                                    data: {
                                        mavt:$mavt,
                                        xoatatca:$InTatCaVatTu
                                    },
                                });
                            },
                            buttons: {
                    ...
                    $res = confirm("Bạn có muốn xoá mã khách hàng "+$mavt+" này không ?");
                    if($res){
                        $.confirm({
                            title: 'Xoá dữ liệu thành công',
                            type: 'green',
