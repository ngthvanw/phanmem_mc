    ...
    $(function () {
        ...
        var objcphddk = {
            ...
            colModel: [//-------------------------------------------Khai báo các cột trong lưới-----------------------
                ...
                {
                    title: "Tên HĐ", width: 350, dataType: "string", editable: false, dataIndx: "tensp",
                    filter: { type: 'textbox', condition: 'begin', listeners: ['keyup'] }
                },
                {
                    title: "Mã HĐ cha",
            ...
            dataModel: {
                ...
                location: "remote",
                recIndx: "sott",
                url: $dir_module_dmsanpham + "list_cpdodang_hd_dk.php",//-- Load danh sách lên lưới
                getData: function (response) {
                    return {data: response.data};
                }
            }
        };
        function calculateSummaryHD() {
            arrayData = tongtiendaukyHD();
            $tongduno = arrayData.data.soduno;
            $tongduco = arrayData.data.soduco;
            if($tongduno!=$tongduco){
                //alert("Tổng dư nợ và tổng dư có không bằng nhau !");
        ...
        objcphddk.render = function (evt, ui) {
            $summarySP = $("<div class='pq-grid-summary'  ></div>")
                .prependTo($(".pq-grid-bottom", this));
            calculateSummaryHD();
        }

        objcphddk.cellSave = function (evt, ui) {
            calculateSummaryHD();
            objcphddk.refresh.call(this);
        }

        objcphddk.refresh = function (evt, ui) {
            var data = [totalDataSP]; //JSON (array of objects)
            var obj = { data: data, $cont: $summarySP }
            $(this).pqGrid("createTable", obj);
        }
        var $grid = $("#grid_editing_cphddodang_dk").pqGrid(objcphddk);

        /////////////////////////////Kết thúc lưới công trình//////////////////////////////////////////////////

        //use refresh & refreshRow events to display jQueryUI buttons and bind events.
        function getRowSelect() { // --------------------Lấy dữ liệu theo dòng trên gird----------------------
            var arr = $("#grid_editing_cphddodang_dk").pqGrid("selection", {
                type: 'cell',
                method: 'getSelection'
            }); //Lấy giá trị đang chọn
            if (arr && arr.length > 0) {
        ...
        $("#tab_bangđoangspdk").click(function () {
            $("#grid_editing_cphddodang_dk").pqGrid( "refreshDataAndView" );
        });
        ...
        function tongtiendaukyHD() {
            $data=""
            $.ajax({// Kiểm tra xem STT có tồn tại hay không
                url: $dir_module_dmsanpham + "tongtien_spdodang_hd_dauky.php",
                async: false,
                success: function (response) {
                    $data = $.parseJSON(response);
                }
...
<div id="dialog-sodu_cpdodang_dk"
    ...
    <div id="tabs" style="padding: 0px;">
        <ul>
            <li><a href="#tabs-0" >HỢP ĐỒNG</a></li>
        </ul>
        <div id="tabs-0">
            <div id="grid_editing_cphddodang_dk" style="margin:0px;border: 0px !important;"></div>
        </div>
    </div>
    <!-- dialog -->
</div>