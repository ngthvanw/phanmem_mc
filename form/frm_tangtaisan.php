...
<div id="dialog-tangtaisan" title="TĂNG TÀI SẢN CỐ ĐỊNH (F8: XOÁ PHIẾU)">
...
/// bắt dầu mã bộ phận
        ...
        function laythongtinphieudanhap() {
            if("<?php echo $sottphieukhac; ?>"!=""){
                $data = "";
                $.ajax({
                    url: $dir_module_taisan + "laythongtintaisanps_theothamchieu.php",
...
///-------------------Kết thúc--------------------------------
       function layngayghiso($loaiphieu){
        ...
        $(btnstt).click(function (event) {// Gọi table khách hàng để chọn
            $('.dialog_main_ds_nhapkho').load("form/frm_ds_tanggiam_taisan.php?idstyle=Form-chinh&idinput=STT&idfocus=STT&loaiphieu=1");
        });
        ...
        function ChucNang_nhapkho() {// Xử lý khi nhấp button đồng ý
            var valid = true;
                ...
                valid = valid && checkNull($("#thoigiansudung"), " Thời gian sử dụng ");
				valid = valid && checkNull($("#giatriconlai"), " Giá trị còn lại ");
            ...
            if($("#chonloaisp").val()==0 && ($("#tkno").val()==621 || $("#tkno").val()== 622 || $("#tkno").val()==623 || $("#tkno").val()==627)){
