...
<script>
    ...
    $(function () {
        ...
        function xoadialog_makho() { // ----------------------đóng form
        ...
        $("#dialog-khohang").keydown(function (event) {//--------------Các phím tắt
            var $grid_pb = $("#grid_editing_makho").closest('.pq-grid');//---- Lưới----------------
            ...
            var rowEditting = $("#grid_editing_makho").pqGrid("getRowsByClass", {cls: 'pq-row-edit'});//---Lấy đối tượng đang sửa
            if ($('div').hasClass('jconfirm') == false) {
                if (event.keyCode == Keys.F4) {
                    rowIndx = 0;
				var colM=$( "#grid_editing_makho" ).pqGrid( "option" , "colModel" );
				 colM[1].editable = true;
				 $( "#grid_editing_makho" ).pqGrid( "option", "colModel", colM);
                ...
                if (event.keyCode == Keys.F7) { // copy
				 ...
				 $( "#grid_editing_makho" ).pqGrid( "option", "colModel", colM);
                    ...
                    addRow(rowIndx, 'makho', $grid_pb, _dataRow);
                }
        ...
        function addRow(rowIndx, $name='manhanvien', $grid, $obj_addrow="") {
            $ma="";
            $.ajax({
                url: $dir_module_makho+"taoma.php",
                async: false,
            ...
            if ($obj_addrow != "") {
                ...
                var rowData = { makho: "0", tenkho: "", diachi: "", ghichu: "" };
            }
            rowData.makho = $ma;
        ...
        var obj = {
            ...
            colModel: [//-------------------------------------------Khai báo các cột trong lưới-----------------------
                ...
                { title: "Mã kho", dataType: "string", dataIndx: "makho", width: 80,sortable: true,editable:false,
                    validations: [
        ...
        var $grid = $("#grid_editing_makho").pqGrid(obj);

        //use refresh & refreshRow events to display jQueryUI buttons and bind events.
        function getRowSelect() { // --------------------Lấy dữ liệu theo dòng trên gird----------------------
            var arr = $("#grid_editing_makho").pqGrid("selection", {
                type: 'cell',
                method: 'getSelection'
        ...
        function isEditCell(rowIndex, dataIndx) { // --------------------Lấy dữ liệu theo dòng trên gird----------------------
            var isEdit = false;
            isEdit = $("#grid_editing_makho").pqGrid("isDirty"); //Lấy giá trị đang chọn
        ...
        //-----------------------------Hết lưới---------------------------------------------------------------------
        setTimeout(function () {
            $("#grid_editing_makho .pq-search-hd-field").focus();
        }, 100);
    });
</script>
<div id="dialog-khohang"
     title="Danh sách kho hàng (ENTER : Sửa,Lưu ,F7: Sao chép , F8: Xóa , ESC: Thoát)"><!-- dialog -->
    <div id="grid_editing_makho" style="margin:5px auto;border: 0px !important;"></div>
</div>