                ...
                <tr>
                    ...
                    <td width="75px" >
                        <input type="text" name="sohoadon" value="" id="sohoadon" required="require"
                               placeholder="Số hóa đơn" maxlength="12" class="text ui-widget-content ui-corner-all"/>
                    </td>
                    <td width="55px" >
                ...
                <tr>
                    ...
                    <td colspan="1" align="right"><span style="padding: 2px">
                            <select style="width:90%;<?php echo (($_SESSION['phanloaihanghoadauvao'] == '1') ? '' : 'display:none'); ?>" id="loaihanghoadichvu">
                                ...
                                <?php
                                    foreach ($cb_ThueSuat_HKD as $key_TS_HKD => $val_TS_HKD) {
                                        echo "<option value='" . $key_TS_HKD . "'>" . $val_TS_HKD . "</option>";
                                    }
                                ?>
                      </select>
                        </span>
                ...
                if('<?php echo $_SESSION['nhacungcaphddt']; ?>'=='viettel'){
                    if (valid && $LoaiPhieu == 2 && ($loaict == '1' || $loaict == '2') && $hoadondientu != 0 && $hoadondientu != 9 && $loaitokhai == '1' && "<?php echo $_SESSION['thietlaphddt'] ?>" == "1" && "<?php echo $_SESSION['txt_hddt_tendangnhap'] ?>" != "" && "<?php echo $_SESSION['txt_hddt_matkhau'] ?>" != "" && "<?php echo $_SESSION['txt_hddt_duongdan'] ?>" != "") {// Nếu là phiếu chi và loại chứng từ là thuế GTGT thì thực hiện lập hoá đơn điện tử
                        ...
                        for (var i = 0; i < chitiethanghoa.length; i++) {
							...
							var $MaHangHoa = chitiethanghoa[i].mavt;
                            objchitiet.lineNumber = (i + 1);
							if($MaHangHoa=="CH.KHAU" || $MaHangHoa=="CH.KHAU1" || $MaHangHoa=="CH.KHAU2" || $MaHangHoa=="CH.KHAU3" || $MaHangHoa=="CH.KHAU4" || $MaHangHoa=="CH.KHAU5"){
								objchitiet.selection = 3;
								//objchitiet.isIncreaseItem = false;
							}else{
								objchitiet.selection = 1;
								//objchitiet.isIncreaseItem = true;
							}
                            objchitiet.itemCode = $MaHangHoa;
                            objchitiet.itemName = chitiethanghoa[i].tenvt.split("&#039;").join("'");
                            objchitiet.unitName = $donvitinh;
                       ...
                       const GioLap_HoaDon = new Date().toTimeString().slice(0, 8);

                        var newDate = $NgayHoaDon[1] + "," + $NgayHoaDon[2] + "," + $NgayHoaDon[0] + " " + GioLap_HoaDon;
                        var newDateNVB = $NgayLapVanBan[1] + "," + $NgayLapVanBan[2] + "," + $NgayLapVanBan[0] + " " + GioLap_HoaDon;
                    ...
                    if (valid && $LoaiPhieu == 2 && ($loaict == '1' || $loaict == '2') && $hoadondientu != 0 && $loaitokhai == '1' && "<?php echo $_SESSION['thietlaphddt'] ?>" == "1" && "<?php echo $_SESSION['txt_hddt_tendangnhap'] ?>" != "" && "<?php echo $_SESSION['txt_hddt_matkhau'] ?>" != "" && "<?php echo $_SESSION['txt_hddt_duongdan'] ?>" != "") {// Nếu là phiếu chi và loại chứng từ là thuế GTGT thì thực hiện lập hoá đơn điện tử
                        ...
                        var chitiethanghoa = $("#grid_editing_chitiet_nhapkho").pqGrid("getData", {dataIndx: ['sott', 'mavt', 'tenvt', 'tenvt', 'dvt', 'soluongnhap', 'donggianhap','thanhtienchuack', 'thanhtien', 'thue', 'tienchietkhau', 'thuesuat','chietkhau']});
                        var arrobjchitiet = [];
                        $phantramthue = 0;
                        for (var i = 0; i < chitiethanghoa.length; i++) {
                            var objchitiet = new Object();
							
							$MaHangHoa = chitiethanghoa[i].mavt;
                            $phantramthue = chitiethanghoa[i].thuesuat;
                            if($loaict==1){
                                if (($phantramthue == 'K' || $phantramthue == 'k')) {
                                    $phantramthue = 4;
                            ...
                            objchitiet.Amount = Math.abs(chitiethanghoa[i].thanhtienchuack);
                            objchitiet.DiscountRate = Math.abs(chitiethanghoa[i].chietkhau);
                            objchitiet.DiscountAmount = Math.abs(chitiethanghoa[i].tienchietkhau);
                            objchitiet.TaxRateID = $phantramthue;
                            objchitiet.TaxAmount = Math.abs(chitiethanghoa[i].thue);
							if($MaHangHoa=="CH.KHAU" || $MaHangHoa=="CH.KHAU1" || $MaHangHoa=="CH.KHAU2" || $MaHangHoa=="CH.KHAU3" || $MaHangHoa=="CH.KHAU4" || $MaHangHoa=="CH.KHAU5"){
								objchitiet.IsDiscount = true;
							}else{
                    ...
                    if (valid && $LoaiPhieu == 2 && ($loaict == '1' || $loaict == '2') && $hoadondientu != 0 && $hoadondientu != 9 && $loaitokhai == '1' && "<?php echo $_SESSION['thietlaphddt'] ?>" == "1" && "<?php echo $_SESSION['txt_hddt_tendangnhap'] ?>" != "" && "<?php echo $_SESSION['txt_hddt_matkhau'] ?>" != "" && "<?php echo $_SESSION['txt_hddt_duongdan'] ?>" != "") {// Nếu là phiếu chi và loại chứng từ là thuế GTGT thì thực hiện lập hoá đơn điện tử
                        ...
                        for (var i = 0; i < chitiethanghoa.length; i++) {
                            ...
                            if ($soluongnhap == "") {
                                ...
                                objchitiet.InventoryItemCode = $MaHangHoa;
                            ...
                            objchitiet.AmountOC = Math.abs(chitiethanghoa[i].thanhtienchuack);// Thành tiền chưa chiết khấu
                            objchitiet.Amount = Math.abs(chitiethanghoa[i].thanhtienchuack);// TỔng tiền chưa chiết khấu
                            objchitiet.DiscountRate = Math.abs(chitiethanghoa[i].chietkhau);
                            objchitiet.DiscountAmountOC = Math.abs(chitiethanghoa[i].tienchietkhau);
                            objchitiet.DiscountAmount =  Math.abs(chitiethanghoa[i].tienchietkhau);
                            ...
                            if($MaHangHoa=="CH.KHAU" || $MaHangHoa=="CH.KHAU1" || $MaHangHoa=="CH.KHAU2" || $MaHangHoa=="CH.KHAU3" || $MaHangHoa=="CH.KHAU4" || $MaHangHoa=="CH.KHAU5"){
								objchitiet.InventoryItemType = 4;
							}else{
                            ...
                            var $data = [
                                {
                                    ...
                                    "TotalDiscountAmount": Math.abs($("#tienchietkhau").val().toString().split(",").join("")),
                                    "TotalDiscountAmountOC": Math.abs($("#tienchietkhau").val().toString().split(",").join("")),
                                    "EInvoiceStatus": 1,
