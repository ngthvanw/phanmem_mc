...
<script>
    ...
    $width = getWidth() - 50;
    $(function () {
        var $dir_module_ps_k_ghino = "";
        $dir_module_ps_k_ghino = "modules/pskghino/";//--------------------------------------------Thay đổi khi copy
        $dir_module_makh = "modules/makhachhang/";
        $dir_module_httk = "modules/httk/";
        function xoadialog_ps_k_ghino() { // ----------------------đóng form
            reset_dialog(".dialog-ps_k_ghino");
            reset_dialog(".dialog_main_psk_ghino");
        }

        $("#dialog-ps_k_ghino").dialog({ // ------------------Gọi dialog
            resizable: false,
            height: $height,
            width: $width,
            modal: true
        ...
        $("#dialog-ps_k_ghino").keydown(function (event) {//--------------Các phím tắt
            var $grid_pb = $("#grid_editing_ps_k_ghino").closest('.pq-grid');//---- Lưới----------------
            ...
            var rowEditting = $("#grid_editing_ps_k_ghino").pqGrid("getRowsByClass", {cls: 'pq-row-edit'});//---Lấy đối tượng đang sửa
            if ($('div').hasClass('jconfirm') == false) {
                ...
                if (event.keyCode == Keys.INSERT) { // Lưu và thêm chi tiết phát sinh kế toán
                    if (isEditing($grid_pb)) {
                        ...
                        $('.dialog_main_chitiet_pskt').load("form/frm_chitiet_ps_k_ghino.php?sott=" + mapskt);
                    }else{
                        if (rowSelect != false) {
                            var rowData = rowSelect[0].rowData;
                            var mapskt = rowData.mapskt;
       ...
       /* $.contextMenu('destroy');
        $.contextMenu({// Menu chuột phải
            selector: '#grid_editing_ps_k_ghino',
            build: function ($trigger, e) {
                return {
                    callback: function (key, options) {
                        var $grid_pb = $("#grid_editing_ps_k_ghino").closest('.pq-grid');
        ...
        function change_data_quit_ps_k_ghino() { // Cảnh báo thoát khi thay đổi dữ liệu////////////////////////////////////////////////////
            var $grid_pb = $("#grid_editing_ps_k_ghino").closest('.pq-grid');//---- Lưới----------------
        ...
        var manoidung_select = function (ui) {
            ...
            var $inp = $("<input type='text'  name='" + dataIndx + "' class='" + cls + " pq-cell-editor pg-cel-define' readonly />")
                .appendTo($cell)
                .val(dc).keypress(function (e) {
                    $('.dialog_main_manoidung').load("form/frm_dm_manoidung_select.php?idstyle=grid_editing_ps_k_ghino");
                });
        }
                    ...
                    buttons: {
                        "Đồng ý": {
                            keys: ['Y'], action: function () {
                                ...
                                $.ajax($.extend({}, ajaxObj, {
                                    context: $grid,
                                    url: $dir_module_ps_k_ghino + "del.php",
                                    data: {id: sott, ma: ma},
        ...
        function update(rowIndx, $grid) {
            ...
            if (isDirty) {
                ...
                if (rowData[recIndx] == null) {
                    //url to add records.
                    url = $dir_module_ps_k_ghino + "add.php";
                }
                else {
                    //url to  update records.
                    url = $dir_module_ps_k_ghino + "edit.php";
                }
                var sott = "";
                $.ajax($.extend({}, ajaxObj, {
                    context: $grid,
        ...
        var makh_select = function (ui) {
            ...
            var $inp = $("<input type='text'  name='" + dataIndx + "' class='" + cls + " pq-cell-editor pg-cel-define' readonly />")
                ...
                .val(dc).keypress(function () {
                    $('.dialog_main_makh').load("form/frm_dm_makh_select.php?idstyle=grid_editing_ps_k_ghino");
                });
        }
        ...
        var obj = {
                            ...
                            "click": function (evt) {
                                ...
                                if (rowSelect == false) {
                                    ...
                                    $.ajax({// Lấy thông tin phiếu và lưu vào session
                                        url: $dir_module_ps_k_ghino + "laythongtinphieuthu.php",
                                        data: {
                                            mapskt: $mapskt,
                                        },
                                        async: false,
                                    ...
                                    $('.dialog_main_print').load("form/print_phieu_ghino.php");
                                }
                            }
                        }]
                    }
            ...
            colModel: [//-------------------------------------------Khai báo các cột trong lưới-----------------------
                    ...
                    validations: [
                        ...
                        },
                        {
                            type: function (ui) {
                                ...
                                //remote validation
                                $.ajax({
                                    url: $dir_module_ps_k_ghino + "checkkey.php",
                ...
                },
                {
                    ...
                    editor: {
                        type: "select", options: function (ui) {
                            ...
                            var parsedJson = "";
                            $.ajax({
                                url: $dir_module_ps_k_ghino + "cb_loaiphieu.php",
                                data: {},
                                async: false,
                                success: function (response) {
                    ...
                    render: function (ui) {
                        ...
                        var $TenLoai = "";
                        $.ajax({
                            url: $dir_module_ps_k_ghino + "ten_loaiphieu.php",
                            data: {ma: $maloaict},
                            async: false,
                            success: function (response) {
                                $TenLoai = response;
                ...
                {
                    title: "TK nợ", minWidth: 50, dataType: "integer", dataIndx: "tkno",
                    validations: [
                        {type: 'minLen', value: 1, msg: "Tài khoản nợ không được trống !"},
                        {
                            type: function (ui) {
                                ...
                                if (_found) {
                                    ...
                                    $('.dialog_main_httk').load("form/frm_dm_httk_select.php?idstyle=grid_editing_ps_k_ghino");
                                    return false;
                                }
                            }
                        }
                ...
                },
                {
                    ...
                    render: function (ui) {
                        ...
                        var $Ten = "";
                        $.ajax({
                            url: $dir_module_ps_k_ghino + "load_ctpskt.php",
                            data: {mapskt: $mapskt},
                            async: false,
                            success: function (response) {
            ...
            dataModel: {
                ...
                location: "remote",
                recIndx: "sott",
                url: $dir_module_ps_k_ghino + "list.php",//-- Load danh sách lên lưới
        ...
        var $grid = $("#grid_editing_ps_k_ghino").pqGrid(obj);
        ...
        function getRowSelect() { // --------------------Lấy dữ liệu theo dòng trên gird----------------------
            var arr = $("#grid_editing_ps_k_ghino").pqGrid("selection", {
                type: 'cell',
                method: 'getSelection'
...
<div id="dialog-ps_k_ghino"
     title="Phát sinh phiếu ghi nợ... (F2: Sửa , F7: Sao chép , F8: Xóa , F9: Lưu , END : Hủy dòng đang sửa,INSERT: Thêm chi tiết phiếu thu)">
    <!-- dialog -->
    <div id="grid_editing_ps_k_ghino" style="margin:5px auto;border: 0px !important;"></div>
</div>