    ...
    $(function () {
        ...
        $("#dialog-ds_nhapkho").keydown(function (event) {//--------------Các phím tắt
			...
			if (event.keyCode == Keys.INSERT || event.keyCode == Keys.ENTER) {
                    $ma = rowSelect[0].rowData.mapsvoncsh;
                    $data = new Array($ma)
                    <?php
                    $i=0;
        ...
        function addRow($grid, $obj_addrow="") {
            ...
            var mapskt =1;
            $.ajax({
                url: $dir_module_vonchusohuu+"taomapskt.php",// tao mã phiếu pskt
                async: false,
                    ...
                    buttons: {
                        "Đồng ý": {
                            keys: ['Y'], action: function () {
                                ...
                                $.ajax($.extend({}, ajaxObj, {
                                    context: $grid,
                                    url: $dir_module_vonchusohuu + "del.php",
                                    data: {id: sott, ma: ma},
        ...
        var obj = {
            ...
            colModel: [//-------------------------------------------Khai báo các cột trong lưới-----------------------
                ...
                {
                    title: "Số TT", dataType: "integer", dataIndx: "mapsvoncsh", editable: true, width: 0, hidden: false,// Mã cửa pkst
                    filter: {
                        type: 'textbox',
                        condition: 'begin',
                        listeners: ['keyup']
        ...
        //-----------------------------Hết lưới---------------------------------------------------------------------
		 setTimeout(function(){
		$("#grid_editing_ds_tangtaisan .pq-search-hd-field").focus();
	},1000);
    });
</script>
<div id="dialog-ds_nhapkho"
     title="DANH SÁCH TĂNG,GIẢM VỐN CHỦ SỞ HỮU... (INSERT: chọn mã, ESC: Thoát)">
    <!-- dialog -->
