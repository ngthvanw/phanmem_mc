...
<div id="dialog-capnhat_matkhau" title="THAY ĐỔI THÔNG TIN HIỂN THỊ ">
    ...
    <form method="post" id="Form-CapNhap_MatKhau" enctype="multipart/form-data">
        <fieldset>
            <table class="table-dialog">
                <tr>
                    <td colspan="2">
                        <label for="name" style="font-weight: bold;">Mã số thuế</label><input type="text" name="TenDangNhap" id="MaSoThue" required="" readonly  value="<?php echo $_SESSION['MST']; ?>" class="text ui-widget-content ui-corner-all" style="width:65%" />
                    </td>

                </tr>
                <tr>
                    <td colspan="2">
                        <label for="name" style="font-weight: bold;">Tên công ty</label><input type="text" name="MatKhauCu" id="TenCongTy" required=""   value="<?php echo $_SESSION['TenCongTy']; ?>" class="text ui-widget-content ui-corner-all" style="width:65%" />
                    </td>
                </tr>
                <tr>
                    <td colspan="2">
                        <label for="name" style="font-weight: bold;">Địa chỉ</label><input type="text" name="MatKhauCu" id="DiaChi" required=""   value="<?php echo $_SESSION['DiaChi']; ?>" class="text ui-widget-content ui-corner-all" style="width:65%" />
                    </td>
                </tr>
             </table>
...
function xoadialog(){
 ...
 function ChucNang_CapNhat_MatKhau(){
    ...
    allFields.removeClass("ui-state-error"); // kiem tra du lieu
            if (valid) {
            $.ajax({
                url: $dir_module + "edit_thaydoi_thongtin.php", // Thay dổi file
                type: "GET", // chọn phương thức gửi là get
                dateType: "text", // dữ liệu trả về dạng text
                data: { // Danh sách các thuộc tính sẽ gửi đi
                    tendn:$("#TenCongTy").val().trim(),
                    mst:$("#MaSoThue").val().trim(),
