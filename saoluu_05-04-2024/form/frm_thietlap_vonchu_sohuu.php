...
<style>
    ...
    <div id="dialog-tangtaisan" title="THIẾT LẬP ĐẦU KỲ VỐN CHỦ SỞ HỮU">
    ...
    <form method="post" id="Form-chinh" enctype="multipart/form-data">
            ...
            <table class="table-dialog" style="vertical-align: middle;width: 100%" border="0">
                <tr>
                    ...
                    <td width="27%"><span style="padding: 2px">
                      <input type="hidden" name="ngayghiso" value="<?php echo date("Y-m-d") ?>" style="width:97%" id="ngayghiso"
                               required="require"
            ...
            <table class="table-dialog" style="vertical-align: middle;width: 100%" border="0">
                <tr style="display:none;">
                  ...
                  <td style="padding: 2px"><input type="hidden" name="btnnoidung" id="btnnoidung"
                                                                class="button" value="..."/>
                    <input type="text" name="manoidung" value="_ATS01" list="listmanoidung" id="manoidung" required="required" style="width: 20%"
                               placeholder="Mã nội dung" class="text ui-widget-content ui-corner-all" />
                               <datalist id="listmanoidung"></datalist>
                    <input name="noidung" type="text" value="Tăng do mua mới hoặc bổ sung" disabled="disabled" required="required" class="text ui-widget-content ui-corner-all" id="noidung"
                ...
                <tr>
                    <td style="padding: 2px">
                        <label for="name">Bộ Phận</label></td>
                    <td style="padding: 2px">
                        <input type="hidden" name="btnmakho" id="btnmakho"
                                                                class="button" value="..."/>
                        <input type="text" name="makho" value="0001" list="listmabophan" id="makho" required style="width: 20%"
      ...
      <fieldset style="background-color: #afd9ee">
        <legend>Thông tin chủ sở hữu

          </legend><table width="100%" border="0">
             
               <tr>
                  <td width="11%"><label for="name11">Tên :</label></td>
                  <td colspan="5"><input name="makh" type="text"
                               class="text ui-widget-content ui-corner-all" list="listmakhachhang" id="makh" style="width: 20%" value=""/>
                  <input name="tenkh" type="text"
                               class="text ui-widget-content ui-corner-all" id="tenkh" style="width: 90%" value=""/>
                               <datalist id="listmakhachhang"></datalist>
                  </td>
                  <td width="22%">&nbsp;</td>
              </tr>
              <tr>
                  <td><label for="name2"> Vốn điều lệ:</label></td>
                  <td colspan="2"><input type="text" name="vondieule" onkeyup="return format_munber(this.value,'#vondieule')" id="vondieule" value="" style="width: 100%;text-align:right;"
                               class="text ui-widget-content ui-corner-all"/></td>
                  <td colspan="2" align="right"><label for="name14">Vốn đã góp:</label></td>
                <td colspan="2"><input type="text" name="vondagop" onkeyup="return format_munber(this.value,'#vondagop')" id="vondagop" value="" style="width: 100%;text-align:right;"
                               class="text ui-widget-content ui-corner-all"/></td>
              </tr>
      ...
      <fieldset style="background-color: #afd9ee">
         <legend>Thực hiện định khoản</legend>
        <table class="table-dialog" style="vertical-align: middle;width: 100%" border="0">
                <tr>
...
<script>
    $height = 300;
    $width = 700;
    //$dir_module_vonchusohuu = "modules/mabp/";////////////////Khai báo đường dẫn vào mudole
    ...
    $(function () {
        ...
        $.ajax({// Load danh sách mã khách hàng
            url: $dir_module_mact + "listall.php",
            async: false,
            dataType: "json",
            success: function (response) {
                $array = (response);
                for (var i = 0; i < $array.length; i++) {
                    //alert($array[i].makh);
                    $listmabophan+='<option value='+$array[i].mact+'>'+bodauTiengViet($array[i].tenct)+'</option>';
                }
            }
        });
...
/////////////////////////////////////////////Kết thúc Danh sách Autocomplex
        ...
        function checkSoHoaDonTrung() {// Check key khi nhấn submit
            ...
            $.ajax({
                url: $dir_module_vonchusohuu + "kiemtrasohoadon.php",
        ...
        $("#Form-chinh").keydown(function (event) {// Gọi table mã nội dung để chọn
            if (event.keyCode == Keys.ESCAPE) { // copy
				...
				if($them=="add"){
					var r = confirm("Dữ liệu chưa được lưu ? Bạn có muốn thoát không ?");
					if(r==true){
						$('.dialog_main_thongbao').load('form/frm_thongbao_tang_vonsohuu_thoat.php');
					}else{
        ...
        $("#makho").keydown(function (event) {// Gọi table mã nội dung để chọn
         ...
         $("#vondieule").keydown(function (event) {// Gọi table mã nội dung để chọn
            if (event.keyCode == Keys.ENTER) { // copy
                $("#vondagop").focus();
            }
        })
       $("#vondagop").keydown(function (event) {// Gọi table mã nội dung để chọn
            if (event.keyCode == Keys.ENTER) { // copy
                $("#tkno").focus();
            }
        })
...
///-------------------Kết thúc--------------------------------
       function layngayghiso($loaiphieu){
            ...
            $LoaiPhieu=$loaiphieu;
            $.ajax({
                url: $dir_module_vonchusohuu + "layngayghiso.php",
		...
		$(btnstt).click(function (event) {// Gọi table khách hàng để chọn
            $('.dialog_main_ds_nhapkho').load("form/frm_ds_tanggiam_vondieule.php?idstyle=Form-chinh&idinput=STT&idfocus=STT&loaiphieu=0");
        });
        //---------------------End maKH--------------
        //////////Begin ma kh////////////////////////////
        ...
        $(btnmakho).click(function (event) {// Gọi table khách hàng để chọn
            $('.dialog_main_mact').load("form/frm_dm_mact_form_select.php?idstyle=Form-chinh&idinput=makho***tenkho&idfocus=cothuegtgt");
        });
        ...
        function goitablemakho($vale) {
            if ($vale == "") {// nếu tk nợ trống thì bật tra bảng
                ...
                $data = "";
                $.ajax({
                    url: $dir_module_mact + "laythongtin.php",
                    data: {'ma': $vale},
                    async: false,
                    success: function (response) {
                        $data = $.parseJSON(response);
        ...
        function goitablemakhachhang($vale) {
            if ($vale == "") {// nếu tk nợ trống thì bật tra bảng
                $('.dialog_main_makh').load("form/frm_dm_makh_form_select.php?idstyle=Form-chinh&idinput=makh***tenkh***diachi***masothue***makhachhang2***tenkhachhang2***diachi2&idfocus=vondieule&ma="+$("#makh").val());
            } else {// nếu tk có không trống
                $data = "";
                $.ajax({
                ...
                if ($data != null) { // Nếu tk có tòn tại trong hệ thống tài khoản thì lấy giá trị và gán vào textbox
                    ...
                    $("#vondieule").focus();
                                 ...
                                 $('.dialog_main_makh').load("form/frm_dm_makh_form_select.php?idstyle=Form-chinh&idinput=makhachhang***tenkhachhang***diachi***masothue***makhachhang2***tenkhachhang2***diachi2&idfocus=vondieule&ma="+$("#makhachhang").val());
                    $('.dialog_main_makh').load("form/frm_dm_makh_form_select.php?idstyle=Form-chinh&idinput=makhachhang***tenkhachhang***diachi***masothue***makhachhang2***tenkhachhang2***diachi2&idfocus=vondieule&ma="+$("#makh").val());
                }
            }
        }
		...
		$("#btntimtaisan").click(function(){
			$('.dialog_main_danhmuc_cptratruoc').load("form/frm_danhsach_macptratruoc_form_select.php");
		});
		...
		function checkMaTaiSan($val) {// Check key khi nhấn enter
            ...
            if ($mataisan == "") {//Nếu số thứ tự null sẽ tạo số mới tự động tăng theo mã
                ...
                $data = "";
                    $.ajax({
                        url: $dir_module_vonchusohuu + "laythongtintaisan.php",
                        data: {ma: $mataisan},
                        async: false,
                        success: function (response) {
        ...
        function checkSTT(STT) {// Check key khi nhấn enter
            ...
            if ($STT == "") {//Nếu số thứ tự null sẽ tạo số mới tự động tăng theo mã
                var $data;
                $.ajax({
                    url: $dir_module_vonchusohuu + "taomapsts.php",// Tạo số thứ tự của phiếu
                    async: false,
                    data: {tanggiam: 0},
                    success: function (response) {
                        $data = response;
                ...
                $data = "";
                    $.ajax({
                        url: $dir_module_vonchusohuu + "laythongtintaisanps.php",
                ...
                if ($checkphieuthuchi == 0) {
                    ...
                    $("#vondieule").val(FormatNumber($data.vondieule.toString()));
                    $("#vondagop").val(FormatNumber($data.vongop.toString()));
        ...
        function laydinhkhoan_chitietpsvt(){
            ...
            $.ajax({// Kiểm tra xem STT có tồn tại hay không
                url: $dir_module_vonchusohuu + "laythongtintabledinhkhoan.php",
        ...
        function ChucNang_nhapkho() {// Xử lý khi nhấp button đồng ý
            var valid = true;
                ...
                valid = valid && checkNull($("#makh"), " Tên ngưới gốp vốn ");
                valid = valid && checkNull(manoidung, " Mã nội dung ");
                valid = valid && checkNull(makho, " Mã kho ");
				 valid = valid && checkNull($("#tkno"), " TK nợ ");
				  valid = valid && checkNull($("#tkco"), " TK có ");
                //valid = valid && checkSoHoaDonTrung();

                if (valid) {
                    $.ajax({
                        url: $dir_module_vonchusohuu + "addpsts.php", // Bao gồm cả add và edit
                        ...
                        data: { // Danh sách các thuộc tính sẽ gửi đi
                            ...
                            vondieule: $("#vondieule").val().trim(),
                            vondagop: $("#vondagop").val().trim(),
                        ...
                        success: function (result) {
                           $('.dialog_main_thongbao').load('form/frm_thongbao_tang_vonsohuu.php?sottphieukhac=<?php echo $sottphieukhac ?>');
                        }
                    });
                }
        ...
        dialog = $("#dialog-tangtaisan").dialog({
            ...
            buttons: {
                ...
                "Kết thúc": function () {
                    $('.dialog_main_thongbao').load('form/frm_thongbao_tang_vonsohuu_thoat.php?sottphieukhac=<?php echo $sottphieukhac ?>');
                }
            }
        });
        dialog.dialog("open");
