...
<div id="dialog-thietminh_baocao_taichinh" title="Thiết minh báo cáo tài chính...">
        ...
        <fieldset style="background-color: #afd9ee;">
            ...
            <table border="0" style="width: 100%;">
                <tr>
                    ...
                    <td>Từ ngày</td>
                    <td align="right"><input type="date" name="thuyetminhtungay" style="width:130px;" id="thuyetminhtungay"
                                             class="text ui-widget-content ui-corner-all"
                                             value="<?php if($_SESSION['TuNgay']!=""){ echo $_SESSION['TuNgay'];}else{echo $_SESSION['NienDo'] . "-" . date("m-d");} ?>"/></td>
                    <td>Đến ngày</td>
                    <td align="right">
                        <input type="date" name="thuyetminhdenngay" id="thuyetminhdenngay" style="width:130px;"
                               class="text ui-widget-content ui-corner-all"
...
///-------------------Kết thúc--------------------------------
        ...
        function ChucNang_ThuChi() {// Xử lý khi nhấp button đồng ý
            ...
            $tungay = $("#thuyetminhtungay").val();
            $denngay = $("#thuyetminhdenngay").val();

            if (valid) {
                $tontai = true;
                if ($tontai) {
                    var result = confirm("Đã tồn tại bảng thuyết minh tài chính ! Bạn có muốn ghi đè ?");
                    if (result) {
                        $.confirm({
                            title: 'Thành công',
                            type: 'green',
                            ...
                            content: function(){
                                var self = this;
                                return $.ajax({
                                    url: $dir_module_ketoantonghop + "laythongtin_bangthuyetminh_taichinh.php",
                                    dataType: 'json',
                                    method: 'get',
                                    data: {
                                        tungay:$tungay,
                            ...
                            buttons: {
                                "OK": {
                                    keys: ['Y'], action: function () {
                                        $('.dialog_main_thongbao').load("form/frm_insolieu_thuyetminh_baocao_taichinh.php?tungay="+$tungay+"&denngay="+$denngay);
                                    }
                                }
                            }
                        });
        ...
        function xoadialog_bangketoankho() {// đóng form
            reset_dialog(".dialog-thietminh_baocao_taichinh");
            reset_dialog(".dialog_main_thietminh_baocao_taichinh");
        }

        dialog = $("#dialog-thietminh_baocao_taichinh").dialog({
            autoOpen: false,
            height: "auto",
            width: $width,
            modal: true,
