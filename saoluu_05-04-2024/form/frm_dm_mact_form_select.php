...
<script>
    ...
    $(function() {
        ...
        function xoadialog_mact() { // ----------------------đóng form 
		 ...
		 if (typeof $(".dialog-macongtrinh").html() == "undefined") {
          ...
          if ( $('div').hasClass('jconfirm')==false) {
              if (event.keyCode == Keys.F4) {
                  ...
                  if(rowSelect==false){
                      ...
                      var _dataRow = {mact : rowData.mact,tenct :rowData.tenct,mactcha :rowData.mact,makh :rowData.makh,diachi :rowData.diachi,so_hd :rowData.so_hd,ngayhd :rowData.ngayhd,ngay_kc :rowData.ngay_kc,ngay_ht :rowData.ngay_ht,giatri_hd :rowData.giatri_hd,vatlieu :rowData.vatlieu,nhancong :rowData.nhancong,may :rowData.may};
                      addRow(rowIndx+1,'mact',$grid_pb,_dataRow);
                  }
              }
			 ...
			 if (event.keyCode == Keys.INSERT) {
                        ...
                        $ma = rowSelect[0].rowData.mact;
                        $ten = rowSelect[0].rowData.tenct;
    ...
    var formart_num = function (ui) {
        ...
        function addRow(rowIndx,$name='mavt',$grid,$obj_addrow="") {
            ...
            if($obj_addrow!=""){
                ...
                var rowData = {mact:"",tenct:"",mactcha:"0",makh:"",diachi :"",so_hd :"",ngayhd :"0000-00-00",ngay_kc :"0000-00-00",ngay_ht :"0000-00-00",giatri_hd :"",vatlieu :"",nhancong :"",may :""}; //empty row template
            }
            rowData.mact = $ma;
       ...
       var obj = {
            ...
            colModel: [//-------------------------------------------Khai báo các cột trong lưới-----------------------
                ...
                { title: "Mã BP", dataType: "string", dataIndx: "mact", minWidth: 80,sortable: true,editable:false,
                    validations: [
                        { type: 'minLen', value: 1, msg: "Mã bộ phận không được trống !" },
                        { type: 'maxLen', value: 14, msg: "Mã bộ phận không lớn hơn 14 ký tự !" },
                        { type: function (ui) {
                            var value = ui.value,
                ...
                { title: "Mã BP Cha", minWidth: 80, dataType: "string", align: "left", dataIndx: "mactcha",hidden:false,
                    validations: [
                ...
                { title: "Chi phí máy", minWidth: 150, dataType: "string", align: "left", dataIndx: "may",hidden:true,
                    editor: {
                        type: formart_num
                    },
                    validations: [
        ...
        $grid.one("pqgridload", function (evt, ui) {
                $find = $("#grid_editing_mact .pq-grid-table td:nth-child(1):contains('<?php echo $ma; ?>')");
				try{
                if(typeof($find[0])=="undefined")
                {
                    $find = $("#grid_editing_mact .pq-grid-table td:nth-child(1)");
                    $find[5].focus();
                    $( "#grid_editing_mact").pqGrid( "setSelection", {rowIndx: 0,colIndx: 1} );
                }else{
                    $find[0].focus();
                    $colIndx = $("#grid_editing_mact .pq-grid-table td:nth-child(1):contains('<?php echo $ma; ?>')").attr('pq-col-indx');
                    $rowIndx = $("#grid_editing_mact .pq-grid-table td:nth-child(1):contains('<?php echo $ma; ?>')").parent().attr('pq-row-indx');
                    $( "#grid_editing_mact" ).pqGrid( "setSelection", {rowIndx: $rowIndx,colIndx: $colIndx} );
                }
								}
				catch(err) {
        ...
        function getRowSelect() { // --------------------Lấy dữ liệu theo dòng trên gird----------------------                 
		 ...
		 function isEditCell(rowIndex,dataIndx) { // --------------------Lấy dữ liệu theo dòng trên gird----------------------                 
             var isEdit = false;
			  isEdit = $("#grid_editing_mact").pqGrid("isDirty"); //Lấy giá trị đang chọn
             
             return isEdit;
         }

		 setTimeout(function(){
		//$("#grid_editing_mact .pq-search-hd-field").focus();
	},100);
 //-----------------------------Hết lưới---------------------------------------------------------------------       
