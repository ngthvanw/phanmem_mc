...
<head>
    <meta charset="utf-8"/>
    <title>BẢNG TỔNG HỢP DANH SÁCH TRÌNH DUYỆT</title>
    ...
    $(function () {
        ...
        var obj = {
            ...
            toolbar: {
                items: [
                    {
                        type: "<span id='thongkedanhsach' style='color:red'></span>"
                    }
                ]
            },
            historyModel: {
            ...
            colModel: [//-------------------------------------------Khai báo các cột trong lưới-----------------------
                ...
                {
                    title: "Người gửi", minWidth: 70, dataType: "string", dataIndx: "nguoigui",editable:false,
                    filter: { type: 'textbox', condition: 'contain', listeners: ['keyup'] }
                },
                {
                    title: "Ngày gửi", minWidth: 120, dataType: "string", align: "center", dataIndx: "ngaytrinhduyet",editable:false,
                    filter: { type: 'date', condition: "begin", listeners: ['keyup'] },

                },
                {
                    title: "Trạng thái", minWidth: 40, dataType: "integer", align: "center", dataIndx: "trangthai",editable:false,
                    render: function (ui) {
                        var value = ui.rowData.trangthai;
                        var mst = ui.rowData.masothue;
                        var tencongty = ui.rowData.tencongty;
                        var tendatabase = ui.rowData.tendatabase;
                        if (value == 'CD') {
                            return "<a href=\"frm_duyet_bangcandoi_taikhoan.php?mst="+mst+"&tencongty="+tencongty+"&tendatabase="+tendatabase+"\"><img src='../icon/choduyet.jpg' alt='Đang chờ duyệt' width='60px'/></a>";
                        } else if (value == 'DD') {
                            return "<a href=\"frm_duyet_bangcandoi_taikhoan.php?mst="+mst+"&tencongty="+tencongty+"&tendatabase="+tendatabase+"\"><img src='../icon/daduyet.jpg' alt='Đã duyệt' width='60px' /></a>";
                        }else if (value == 'TN'){
                            return "<a href=\"frm_duyet_bangcandoi_taikhoan.php?mst="+mst+"&tencongty="+tencongty+"&tendatabase="+tendatabase+"\"><img src='../icon/tamnop.jpg' alt='Tạm nộp' width='60px' /></a>";
                        }else{
                            return "<a href=\"frm_duyet_bangcandoi_taikhoan.php?mst="+mst+"&tencongty="+tencongty+"&tendatabase="+tendatabase+"\"><img src='../icon/tralai.jpg' alt='Trả lại' width='60px' /></a>";
                        }
                    },
                    filter: { type: "select",
                        condition: 'equal',
                        ...
                        options: function (ui) {
                            //remote validation
                            var parsedJson = [{'CD':'CHỜ DUYỆT'}, {'DD':'ĐÃ DUYỆT'}, {'1':'PHIẾU THU'},{'TN':'TẠM NỘP'},{'TL':'TRẢ LẠI'}] ;
                            return parsedJson;
                        },
                        listeners: ['change']
                    },
            ...
            dataModel: {
                ...
                url: $dir_module_user + "list_dsct.php",//-- Load danh sách lên lưới
        ...
        $grid.one("pqgridload", function (evt, ui) {
            $("#grid_editing_thongke_phanmem .pq-search-txt").focus();
            ThongKeDanhSach();
        });
        ...
        function ThongKeDanhSach() {
            $.ajax({// Kiểm tra xem STT có tồn tại hay không
                url: $dir_module_user + "thongke_danhsach_duyet.php",
                data: {nam: <?php echo $_SESSION['NienDo']; ?>},
                async: false,
                success: function (response) {
                    $("#thongkedanhsach").html(response);
                }
            });
        }
...
<div id="dialog-makhachhang"
     title="BẢNG TỔNG HỢP DANH SÁCH TRÌNH DUYỆT"><!-- dialog -->
