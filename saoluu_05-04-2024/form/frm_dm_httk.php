...
<script>
    ...
    $(function() {
        ...
        function xoadialog_httk_select() { // ----------------------đóng form 
            reset_dialog(".dialog-httk");
            reset_dialog(".dialog_main_httk");
        }
		//reset_dialog(".dialog-httk");
        //if(typeof $(".dialog-httk").html()=="undefined"){// Chỉ gọi dialo khi chưa hiện
            $("#dialog-httk").dialog({ // ------------------Gọi dialog 
                resizable: false,
                height: $height,
                width: $width,
                modal: true
        ...
        //}
     $("#dialog-httk").keydown(function(event) {//--------------Các phím tắt
        var $grid_pb = $("#grid_editing_httk").closest('.pq-grid');//---- Lưới----------------
        ...
        var rowEditting = $( "#grid_editing_httk" ).pqGrid( "getRowsByClass", { cls : 'pq-row-edit' } );//---Lấy đối tượng đang sửa
 ...
 function addRow(rowIndx,$name='mavt',$grid,$obj_addrow="") {
		...
		var obj = {
            ...
            colModel: [//-------------------------------------------Khai báo các cột trong lưới-----------------------
                ...
                { title: "Tên TK tiếng Anh", minWidth: 200, dataType: "string", dataIndx: "tentk_en"},
                { title: "Tên TK tiếng Trung", minWidth: 200, dataType: "string", dataIndx: "tentk_cn"},
                    { title: "Mã TK cha", minWidth: 80, dataType: "string", align: "left", dataIndx: "matkcha",
                            validations: [
                    ...
                    { title: "Mã TS trên bảng CĐKT", minWWidth: 120, dataType: "string", align: "right", dataIndx: "mats",
                        //validations: [
                            //{ type: 'minLen', value: 1, msg: "Loại tài khoản không được trống !" }
                        //]
                    },
                    { title: "Mã Ng.Vốn trên bảng CĐKT", minWidth: 120, dataType: "string", align: "right", dataIndx: "mangv",
                        //validations: [
                            //{ type: 'minLen', value: 1, msg: "Loại tài khoản không được trống !" }
                        //]
                    },
        ...
        var $grid = $("#grid_editing_httk").pqGrid(obj);
        ...
        function getRowSelect() { // --------------------Lấy dữ liệu theo dòng trên gird----------------------                 
             var arr = $("#grid_editing_httk").pqGrid("selection", {
                 type: 'cell',
                 method: 'getSelection'
		 ...
		 function isEditCell(rowIndex,dataIndx) { // --------------------Lấy dữ liệu theo dòng trên gird----------------------                 
             var isEdit = false;
			  isEdit = $("#grid_editing_httk").pqGrid("isDirty"); //Lấy giá trị đang chọn
             
             return isEdit;
         }
 //-----------------------------Hết lưới---------------------------------------------------------------------       
    setTimeout(function(){
		$("#grid_editing_httk .pq-search-hd-field").focus();
	},100);
    });     
</script>
<div id="dialog-httk" title="Chọn mã tài khoản (4: Thêm , F7: Sao chép , F8: Xóa )"><!-- dialog -->
  <div id="grid_editing_httk" style="margin:5px auto;border: 0px !important;"></div>
</div>