...
<div id="dialog-bangchitiet_socai" title="Sổ quỹ và báo cáo quỹ...">
    ...
    <form method="post" id="Form-chinh" enctype="multipart/form-data">
        ...
        <fieldset style="background-color: #afd9ee">
            ...
            <table border="0" style="width: 100%;">
				...
				<tr>
                    <td><b style="font-size: 14px;">Loại tiền:</b></td>
                    <td><select name="loaitien" id="loaitien" style="width:100%;height:25px;">
                ...
                <tr>
                    ...
                    <td><select name="ChiNhanhCongTy" id="ChiNhanhCongTy" class="text ui-widget-content ui-corner-all" style="width:100%;height:25px;">
    ...
    $(function () {
		...
		{ title: "", dataIndx: "state", width: 5, align: "center", type:'checkBoxSelection', cls: 'ui-state-default', resizable: false, sortable:false }
            ...
            dataModel: {
                ...
                location: "remote",
                recIndx: "sott",
                url: $dir_module_matk+"list_soquy.php",//-- Load danh sách lên lưới
...
///-------------------------Di chuyễn các phần tử bằng enter----------------
        ...
        $("#loaitien").change(function (event) {// Gọi table mã nội dung để chọn
            $tuthang = $("#congdontuthang").val();
            $loaitien = $("#loaitien").val();
            if ($loaitien == 'NT') {
                $("#congdonsocai").val("0");
                $("#congdonsocai").prop("disabled",true);
            }else{
                $("#congdonsocai").prop("disabled",false);
            }
        })
...
///-------------------Kết thúc--------------------------------
        ...
        function ChucNang_ThuChi() {// Xử lý khi nhấp button đồng ý
            ...
            if (valid) {
				if($loaitien=="VND"){
					$('.dialog_main_thongbao').load("form/frm_insolieu_bangchitiet_soquy.php?tungay=" + $tungay + "&denngay=" + $denngay + "&sapxep=" + $sapxep + "&matk=" + $mavt + "&theobophan=" + $theobophan + "&theonoidung=" + $noidung + "&congdonsocai=" + $congdonsocai + "&loctheospct=" + $loctheospct+"&theongay="+$theongay+"&theotkdoiung="+$theotkdoiung+"&loaitien=" + $loaitien);

				}else{
                    $.confirm({
                        title: 'Nạp tỷ giá thành công . ',
                        type: 'green',
                        autoClose: 'OK|1000',
                        content: function () {
                            var self = this;
                            return $.ajax({
                                url: $dir_module_ketoantonghop + "themtinsocai_ngoaite.php",
                                async: false,
                                data: {
                                    tungay:$tungay,
                                    denngay:$denngay,
                        ...
                        buttons: {
                            "OK": {
                                keys: ['Y'], action: function () {
                                    $('.dialog_main_thongbao').load("form/frm_insolieu_bangchitiet_soquy_ngoaite.php?tungay=" + $tungay + "&denngay=" + $denngay + "&sapxep=" + $sapxep+"&matk=" + $mavt+"&theobophan=" + $theobophan+"&theonoidung=" + $noidung+"&congdonsocai=" + $congdonsocai+"&loaitien=" + $loaitien);
                                }
                            }
                        }
                    });
