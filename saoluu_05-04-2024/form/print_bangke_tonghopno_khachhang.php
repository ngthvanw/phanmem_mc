...
<div style="width: 500px;;" id="dialog_insolieu_nokhachhang" title="In số liệu">
    <iframe id="fred" style="border:1px solid #666CCC" title="" src="tcpdf/baocao/inbang_tonghop_nokhachhang.php"
            frameborder="1" scrolling="auto" height="100%" width="100%"></iframe>
</div>
<script>
    $(function () {
        function xoadialog_print_insolieu_thuchi() { // đóng form
            reset_dialog(".dialog_insolieu_nokhachhang");
            reset_dialog(".dialog_main_print");
        }

        $("#dialog_insolieu_nokhachhang").keydown(function (event) {//--------------Các phím tắt
            if (event.keyCode == Keys.ESCAPE) {
                xoadialog_print_insolieu_thuchi();
            }
        });
        dialog = $("#dialog_insolieu_nokhachhang").dialog({
            autoOpen: false,
            height: getHeight(),
            width: getWidth(),
            modal: true,
