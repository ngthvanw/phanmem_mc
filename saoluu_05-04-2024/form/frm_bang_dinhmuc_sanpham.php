...
<div id="dialog-tangtaisan" title="NHẬP ĐỊNH MỨC SẢN PHẨM">
                ...
                <td width="60%" valign="top">
                    ...
                    <fieldset style="background-color: #afd9ee;border: 0px solid">
                        <table style="vertical-align: middle;width: 100%;" border="0">
                            <tr>
                                <td style="width:20%"><label for="name11">Sản phẩm:</label></td>
                                <td style="width:20%">
                            ...
                            <tr>
                                ...
                                <td style="width:20%">
                                    <select id="loaidinhmucsp">
                                        ...
                                        <option value="NC">Nhân công</option>
                                        <option value="CM">Ca máy</option>
                                        <option value="SXC">Chi phí SX chung</option>
                                    </select>
                                </td>
                                <td style="width:60%">
                                    <table width="100%" border="0">
                                        <tr>
                                            <td width="30%"><label>Số lượng sản phẩm:</label></td>
                                            <td width=40%"><input name="soluonguocluong" type="text" style="width:98%;text-align: right;"
                                                       ...
                                                       value="1"/></td>
                                                       <td width="30%" style="text-align:left;"><label id="donvitinh"></label></td>
                                        </tr>
                                    </table>
    ...
    $(function () {
        ...
        $("#dialog-tangtaisan").keydown(function(event) {//--------------Các phím tắt
            ...
            if (event.keyCode == Keys.F7) { // Hủy bỏ hàng đang xóa
                $res = confirm("CẢNH BÁO! \n\n BẠN ĐANG CHUẨN BỊ SAO CHÉP DANH SÁCH ĐỊNH MỨC TỪ SẢN PHẨM KHÁC. CHỨC NĂNG NÀY SẼ LÀM THAY ĐỔI DỮ LIỆU ĐỊNH MỨC HIỆN TẠI. \n\n BẠN CÓ MUỐN TIẾP TỤC KHÔNG ?");
                if($res){
                    var sophieu = prompt("NHẬP MÃ SẢN PHẨM CẦN SAO CHÉP", "");
                    if (sophieu != null) {
                        $sophieuhientai = $("#masp").val().trim(); /// Là số phiếu
                        $.ajax({// Lấy tổng chiết khấu
                            url: $dir_module_nhapkho + "saochepdanhsachdinhmucsp.php",// Lấy tồng thuế NK,TTĐB,VC,BX
                            data: {maspsaochep: sophieu,masphientai: $sophieuhientai},
                            async: false,
                            success: function (response) {
                                ...
                                if($data!=""){
                                    ...
                                    $("#gird_chitiet_dinhmuc").load("form/bang_gird_chitiet_dinhmuc_all.php?masp="+$sophieuhientai+"&list=0&loaidinhmucsp=ALL&soluongspuocluong=1");
                                }
                            }
                        });
                    }
        ...
        var obj = {
            ...
            title: "Danh sách sản phẩm (INSERT:Lấy DS vật tư,F8: Xoá dữ liệu thừa)",
            colModel: [//-------------------------------------------Khai báo các cột trong lưới-----------------------
                {title: "SoTT", dataType: "integer", dataIndx: "sott", editable: false, width: 0, hidden: true,},
                {
                    title: "Mã SP", dataType: "string", dataIndx: "masp", minWidth: 100, sortable: true, editable: false,
                    validations: [
                ...
                {
                    title: "Tên SP", minWidth: 200, dataType: "string", dataIndx: "tensp", editable: false,
                    validations: [
				...
				{
                    title: "Mã SP Cha", minWidth: 80, dataType: "string", dataIndx: "maspcha", editable: false,
                    validations: [
                        {type: 'minLen', value: 1, msg: "Đơn vị tính không được trống !"}
                    ],
                    filter: {
                ...
                {
                    title: "SL NL", minWidth: 30, dataType: "string", dataIndx: "slnguyenlieu", editable: false
                }
            ],//,
        ...
        obj.rowSelect = function (evt, ui) {
            if (ui.rowData) {
                ...
                $("#tensp").val(tensp);
                $("#donvitinh").text(dvt);
                $("#gird_chitiet_dinhmuc").load("form/bang_gird_chitiet_dinhmuc_all.php?masp='" + masp + "'&list=0&loaidinhmucsp='ALL'&soluongspuocluong='"+$soluongspuocluong+"'");// Lấy danh sách chưa có nhập
            }
        }
               ...
               // $("#grid_editing_chitiet_nhapkho").pqGrid( "refreshDataAndView" );
                ...
                if(masp==""){
                    ...
                    $loaidinhmucsp =  $("#loaidinhmucsp").val();
                        ...
                        if($loaidinhmucsp=="")
                            $("#gird_chitiet_dinhmuc").load("form/bang_gird_chitiet_dinhmuc.php?masp='" + masp + "'&list=1&loaidinhmucsp='"+$loaidinhmucsp+"'&soluongspuocluong='"+$soluongspuocluong+"'");// Lấy danh sách chưa có nhập
                        else if($loaidinhmucsp=="NC")
                            $("#gird_chitiet_dinhmuc").load("form/bang_gird_chitiet_dinhmuc_nc.php?masp='" + masp + "'&list=1&loaidinhmucsp='"+$loaidinhmucsp+"'&soluongspuocluong='"+$soluongspuocluong+"'");// Lấy danh sách chưa có nhập
                        else if($loaidinhmucsp=="CM")
                            $("#gird_chitiet_dinhmuc").load("form/bang_gird_chitiet_dinhmuc_cm.php?masp='" + masp + "'&list=1&loaidinhmucsp='"+$loaidinhmucsp+"'&soluongspuocluong='"+$soluongspuocluong+"'");// Lấy danh sách chưa có nhập
                        else if($loaidinhmucsp=="SXC")
                            $("#gird_chitiet_dinhmuc").load("form/bang_gird_chitiet_dinhmuc_sxc.php?masp='" + masp + "'&list=1&loaidinhmucsp='"+$loaidinhmucsp+"'&soluongspuocluong='"+$soluongspuocluong+"'");// Lấy danh sách chưa có nhập
                        else if($loaidinhmucsp=="ALL")
                            $("#gird_chitiet_dinhmuc").load("form/bang_gird_chitiet_dinhmuc_all.php?masp='" + masp + "'&list=1&loaidinhmucsp='"+$loaidinhmucsp+"'&soluongspuocluong='"+$soluongspuocluong+"'");// Lấy danh sách chưa có nhập
