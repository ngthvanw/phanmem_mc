...
<script>
    ...
    $(function () {
        ...
        var obj = {
            ...
            groupModel:{
                dataIndx: ["duyetcpduoctru"],
                collapsed: [ false, false],
                title: ["<b style='font-weight:bold;'>{0} ({1} hàng)</b>","{0} - {1}"],
                dir: ["up","down"]
            },
            filterModel: {
                on: true,
            ...
            change: function (evt, ui) {// Khi dữ liệu thay đổi
                ...
                if (type == 'update') {
                    ...
                    if (valid) {
                        if (rowData[recIndx] == null) {
                            url = $dir_module_dsnhapkho + "edit_cpkhongduoctru.php";
                        }
                        else {
                            url = $dir_module_dsnhapkho + "edit_cpkhongduoctru.php";
                        }
                ...
                if (valid) {
                    rowData.tiencpduocduyet = rowData.tienhang;
                    $.ajax({
                        url: url,
                        data: rowData,
                        dataType: "json",
            ...
            colModel: [//-------------------------------------------Khai báo các cột trong lưới-----------------------
                {
                    title: "Số TT", dataType: "textbox", dataIndx: "sott", editable: false, width: 0, hidden: true
                },
                {
                    title: "Số TT", dataType: "textbox", dataIndx: "nhapxuat", editable: false, width: 0, hidden: true
                },
                {
                    title: "Số phiếu", dataType: "textbox", dataIndx: "mapskt", editable: true, width: 80, hidden: false,align:"center",// Mã cửa pkst
                    filter: {
                        type: 'textbox',
                        condition: 'begin',
                        listeners: ['keyup']
                ...
                {
                    title: "Loại phiếu", dataType: "textbox", dataIndx: "loaiphieu", editable: true, width: 120, hidden: false,align:"left",// Mã cửa pkst
                    filter: { type: "select",
                        condition: 'equal',
                        prepend: { '': '----Tất cả----' },
                        options: function (ui) {
                    ...
                    render: function (ui) {
                        ...
                        if($nhapxuat==2){
                            if($loaiphieu==1){
                                ...
                                return "PHIẾU GHI CÓ";
                            }else if($loaiphieu>=5 && $loaiphieu<=64 && $loaiphieu%2!=0 ){
                                return "PHIẾU THU NH";
                            }else if($loaiphieu>=5 && $loaiphieu<=65 && $loaiphieu%2==0 ){
                                return "PHIẾU CHI NH";
                            }else{
                ...
                },
                {
                    ...
                    summary: {
                        type: ["sum"],
                        title: ["<b style='font-weight:bold;'>Tổng cộng:</b> {0}"]
                    }
                },
                {
                    title: "Trạng thái", minWidth: 100, dataType: "integer", align: "center", dataIndx: "duyetcpduoctru",
                    <?php echo $khoadong; ?>
                    filter: { type: "select",
                        condition: 'equal',
                        prepend: { '': '----Tất cả----' },
                        options: function (ui) {
                            var parsedJson = [ { 0:"KHÔNG DUYỆT"}, {1: "DUYỆT"}] ;
                            return parsedJson;
                        },
                        listeners: ['change']
                    },
                    render: function (ui) {
                        var value = ui.rowData.duyetcpduoctru;
                        if (value == "1") {
                ...
                {
                    title: "Tiền hàng duyệt ",
                    minWidth: 100,
                    dataType: "string",
                    align: "right",
                    dataIndx: "tiencpduocduyet",
                    editable: false,
                    render: function (ui) {
                        var value = ui.rowData.tiencpduocduyet;
                        return $.number(value,0,",",",");
                    }
                 },
                {
            ...
            dataModel: {
                ...
                location: "remote",
                recIndx: "sott",
                url: $dir_module_dsnhapkho + "list_khongcpkhongduoctru.php",//-- Load danh sách lên lưới
        ...
        //-----------------------------Hết lưới---------------------------------------------------------------------
		 setTimeout(function(){
		$("#grid_editing_ds_hoadon_khongchungtu .pq-search-hd-field").focus();
	},1000);
    });
</script>
<div id="dialog-ds_hoadon_khongchungtu"
     title="DANH SÁCH HÓA ĐƠN CHI PHÍ KHÔNG ĐƯỢC TRỪ... (ESC: Thoát)">
    <!-- dialog -->
