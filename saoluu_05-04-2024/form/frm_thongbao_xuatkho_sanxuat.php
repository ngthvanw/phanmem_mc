    ...
    $(function () {
        ...
        function xoaform_phieuthuchi($mangsang) {//------------------------------------------------------------------------------------
            if($mangsang==1) {
                ...
                $("#manoidung").val("_XKHSX");
                $("#noidung").val("Xuất kho sản xuất");
        ...
        $("#InPhieuHoaDon").click(function () {
            $('.dialog_main_thongtin_nhapxuatkho').load('form/frm_insolieu_inhoadon.php?<?php echo $ChuoiTruyVan; ?>');
        });

        $("#inphieuthuchi_hoadondientu").click(function () {
            $('.dialog_main_insolieu_thuchi').load('form/frm_insolieu_hoadondientu.php?<?php echo $ChuoiTruyVan; ?>&loaihddt=7');
        });
        $("#inphieuthuchi_hoadondientu_chuyendoi").click(function () {
            <?php
            if($_SESSION['nhacungcaphddt']=='viettel'){
               ?>
            $('.dialog_main_insolieu_thuchi').load('form/frm_insolieu_hoadondientu.php?<?php echo $ChuoiTruyVan; ?>&loaihddt=9');
            <?php
            }else  if($_SESSION['nhacungcaphddt']=='bkav'){
                ?>
                    $('.dialog_main_insolieu_thuchi').load('form/frm_insolieu_hoadondientu.php?<?php echo $ChuoiTruyVan; ?>&loaihddt=20');
                <?php
            }
            ?>
        });

        $("#InPhieuBienNhan").click(function () {
            $('.dialog_main_thongtin_nhapxuatkho').load('form/frm_insolieu_biennhan.php?<?php echo $ChuoiTruyVan; ?>');
            ...
            <td style="width: 50%;">
                <fieldset>
                    ...
                    <button id="inphieuthuchi_hoadondientu"
                            class="ui-button ui-widget ui-corner-all" style="width: 100%;margin-bottom: 2px;font-size: 16px;"><span
                        >In hóa đơn ĐT...</span></button>
                    <button id="inphieuthuchi_hoadondientu_chuyendoi"
                            class="ui-button ui-widget ui-corner-all" style="width: 100%;margin-bottom: 2px;font-size: 16px;"><span
                        >In hóa đơn CĐ...</span></button>
                    <button id="InPhieuHoaDon"
                            class="ui-button ui-widget ui-corner-all" style="width: 100%;margin-bottom: 2px;font-size: 16px;"><span
                                >In hóa đơn...</span></button>
                    <button id="InPhieuNhapKho"
                            class="ui-button ui-widget ui-corner-all" style="width: 100%;margin-bottom: 2px;font-size: 16px;"><span
                                >In phiếu xuất kho...</span></button>
