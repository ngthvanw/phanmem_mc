...
<div id="dialog-bangchitiet_hanghoa" title="DỮ LIỆU MÃ VẬT TƯ DƯ THỪA...">
    ...
    <form method="post" id="Form-chinh" enctype="multipart/form-data">
        ...
        <fieldset style="background-color: #afd9ee">
            <legend>Chọn mã hàng cần xoá</legend>
            <table border="0" style="width: 100%;">
                <tr>
                    <td colspan="2">
    ...
    $(function () {
		...
		{ title: "", dataIndx: "state", width: 5, align: "center", type:'checkBoxSelection', cls: 'ui-state-default', resizable: false, sortable:false }
            ...
            dataModel: {
                ...
                location: "remote",
                recIndx: "sott",
                url: $dir_module_mavattu+"list_mavtthua.php",//-- Load danh sách lên lưới
...
///-------------------Kết thúc--------------------------------
        ...
        function ChucNang_ThuChi() {// Xử lý khi nhấp button đồng ý
            ...
            if (valid) {
                if($InTatCaVatTu){
                    $res = confirm("Bạn có muốn xoá tất cả mã vật tư không ?");
                    if($res){
                        $.confirm({
                            ...
                            content: function(){
                                var self = this;
                                return $.ajax({
                                    url: $dir_module_mavattu + "xoadulieu_mavt_duthua.php",
                                    dataType: 'json',
                                    method: 'get',
                                    data: {
                                        mavt:$mavt,
                    ...
                    $res = confirm("Bạn có muốn xoá mã vật tư "+$mavt+" này không ?");
                    if($res){
                        $.confirm({
                            title: 'Xoá dữ liệu thành công',
                            type: 'green',
