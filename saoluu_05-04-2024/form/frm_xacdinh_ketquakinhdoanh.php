...
<div id="dialog-xacdinh_ketqua_kd" title="Xác định kết quả kinh doanh...">
    ...
    <form method="post" id="Form-chinh" enctype="multipart/form-data">
        ...
        <fieldset style="background-color: #afd9ee">
            <legend>Tài khoản ngoài bảng CĐKT</legend>
            <table border="0" style="width: 100%;">
                <tr>
                    <td><select name="Intheothuesuat" style="width:100%;height:25px;display:none;" id="Intheothuesuat">
                  ...
                  <button onclick="$('.dialog_main_thietlap_buttoan_phatsinh').load('form/frm_thietlap_buttoan_phatsinh.php');" type="button" style="width:100%;border:1px solid #990" class="ui-button ui-corner-all ui-widget">Các bút toán được thiết lập</button>
                  </td>
                </tr>
                <tr style="display:none">
                    <td>&nbsp;<b>Sắp xếp khi in</b></td>
                </tr>
                <tr  style="display:block;">
                    <td>
                        <table style="width: 100%">
                            <tr>
                                <td><input name="xacdinhthuetndn" type="checkbox" id="xacdinhthuetndn"
                                           checked="checked"></td>
                                <td>Xác định thuế TNDN phải nộp</td>
                                <td>&nbsp;</td>
                                <td>&nbsp;</td>
                                <td>&nbsp;</td>
                                <td>&nbsp;</td>
                            ...
                            <tr>
                              <td><input name="xoabuttoanps" type="checkbox" id="xoabuttoanps"
                                            /></td>
...
<script>
    ...
    $dir_module_makhachhang = "modules/ketoantonghop/";////////////////Khai báo đường dẫn vào mudole
...
///-------------------Kết thúc--------------------------------
        ...
        function ChucNang_ThuChi() {// Xử lý khi nhấp button đồng ý
            ...
            $tungay = $("#tungay").val();
            $denngay = $("#denngay").val();
            $xacdinhthuetndn = $("#xacdinhthuetndn").val();
            $xacdinhthuetndn=0;
            ...
            $xoabuttoanps=0;
            if($("#xoabuttoanps").prop("checked") == true){
                $xoabuttoanps=1;
            }
            $tienbtps = 0;
            $.ajax({
                url: $dir_module_makhachhang+"checkbuttoanphatsinh.php",
                async: false,
            ...
            if($xoabuttoanps==1){
                $.confirm({// Chay lai bảng cân đối tài khoản
                    ...
                    content: function () {
                        var self = this;
                        return $.ajax({
                            url: $dir_module_makhachhang + "xoabuttoanps.php",
                            dataType: 'json',
                            method: 'get',
                            data: {
                                tungay: $tungay,
                ...
                if (parseInt($tienbtps) == 0) {
                    $.confirm({// Chay lai bảng cân đối tài khoản
                        ...
                        content: function () {
                            var self = this;
                            return $.ajax({
                                url: $dir_module_makhachhang + "laythongtininphieu_candoi_tk_va_ptps.php",
                                dataType: 'json',
                                method: 'get',
                                data: {
                                    tungay: $tungay,
                        ...
                        buttons: {
                            "OK": {
                                keys: ['Y'], action: function () {
                                    $('.dialog_main_tokhai_thuetndn').load('form/frm_bangke_xacdinhthue_tndn.php');
                                }
                            }
                        }
                    });
                } else {
                    $('.dialog_main_thongbao').load("form/frm_insolieu_buttoan_phatsinh.php");
                }
            }
        ...
        function xoadialog_bangketoankho() {// đóng form
            reset_dialog(".dialog-xacdinh_ketqua_kd");
            reset_dialog(".dialog_main_xacdinh_kqkd");
        }

        dialog = $("#dialog-xacdinh_ketqua_kd").dialog({
            autoOpen: false,
            height: "auto",
            width: $width,
            modal: true,
