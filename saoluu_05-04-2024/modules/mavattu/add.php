<?php
    ...
    $OBJ->set_SoTT($sott);
    $mavt = check_data(strtoupper($_GET['mavt']));
    $OBJ->set_MaVT($mavt);
    $OBJ->setMaVTDoi($_GET['mavttt']);
    $OBJ->setPBChiPhi(check_data($_GET['pbchiphi']));
    ...
    $OBJ->setTKDoanhThu(check_data($_GET['tkdoanhthu']));
    ...
    $OBJ->setHeSo(check_data(str_replace(",","",$_GET['heso'])));

    $OBJ->set_ChuThich(check_data($_GET['ghichu']));

    $OBJ->setLoaiVL(check_data($_GET['loaivl']));
