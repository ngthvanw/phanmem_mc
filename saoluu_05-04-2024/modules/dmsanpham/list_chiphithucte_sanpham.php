...
database::re_query(" insert into bangdoanhthuthucte(mact,gtcongtrinh,tyle,doanhthuthucte) select masp,0,0,0 from masp where loaisp='SP' and masp not in (select mact from bangdoanhthuthucte) ");
database::re_query(" DELETE FROM bangdoanhthuthucte WHERE mact not in (select masp FROM masp) ");
...
if($filterQuery==""){
    $filterQuery.=" loaisp='SP'";
}else{
    $filterQuery.=" and loaisp='SP'";
}
$OBJ->set_orderby($filterQuery);
if(isset($_GET['pq_filter'])){
    $result = $OBJ->loadListMaCT_DOANHTHUTHUCTE_W();
}else{
    $result = $OBJ->loadListMaSP_DOANHTHUTHUCTE_DK();
}
