...
database::re_query(" insert into cpdodangdk(mact,soduno,soduco) select masp,0,0 from masp where loaisp in('CT','SP') and masp not in (select mact from cpdodangdk ) ");

database::re_query(" DELETE FROM cpdodangdk WHERE mact not in (select masp FROM masp) ");
...
if(isset($_GET['pq_filter'])){
    ...
    $result = $OBJ->loadListMaCT_DK(0,3);
}
