...
$dodangdauky = $OBJCT->load_danhsach_dodang_dk("CT");// Lấy đầu kỳ dỡ dang
...
if ($phuongphapphanbo == "doanhthuthuchien") {
    ...
    $TongDoanhThuThuan_CT = 0;
    foreach ($dataDoanhThuThuan as $itemTongDT) {
        if ($itemTongDT['loaisp'] == "CT") {
            $TongDoanhThuThuan_CT += ($itemTongDT['tienco'] - $itemTongDT['tienno']);
        }
    }
	...
	$TongDoanhThuThuan['CT'] = $TongDoanhThuThuan_CT;
...
if ($phuongphapphanbo == "doanhthuthuchien") {
    ...
    $matk = $matk_511;
    $dataDoanhThuThuanSP = $OBJCT->load_danhsach_no_co_doanhthuthuan_theocongtrinh($matk, $theonoidung, $sapxep);// lấy tất cả thu chi
...
foreach ($dataCPSXCTong as $itemTongCPSXCTong) {
    ...
    if ($itemTongCPSXCTong['loaisp'] == "SP") {
        ...
        $TongCPSXCTong_SPTheoTK627[$itemTongCPSXCTong['tk']] += ($itemTongCPSXCTong['tienno']-$itemTongCPSXCTong['tienco']);
        if($itemTongCPSXCTong['mabp']=='0001'){
            $TongCPSXCSPTong_001+=($itemTongCPSXCTong['tienno']-$itemTongCPSXCTong['tienco']);
        }
    }
}
...
foreach ($dataCPSXCTong_0001_CTCAP1 as $itemTongCPSXCTong) {
    if ($itemTongCPSXCTong['loaisp'] == "CT"){
        $TongCPSXCTong_CTTheoTK627[$itemTongCPSXCTong['tk']] += ($itemTongCPSXCTong['tienno']-$itemTongCPSXCTong['tienco']);
    }
}
$matk = $matk_623;
...
foreach ($dataCPMayTong_0001_CTCAP1 as $itemTongCPMayTong) {
    if ($itemTongCPMayTong['loaisp'] == "CT"){
        $TongCPNCCTTong_CTTheoTK623[$itemTongCPMayTong['tk']] += ($itemTongCPMayTong['tienno']-$itemTongCPMayTong['tienco']);
    }
}
...
$TongCPNC622CTTong_CT = 0;
$TongCPNCP622SPTong = 0;
$TongCPNC622SPTong_001 = 0;

foreach ($dataCPNC622Tong as $itemTongCPNC622Tong) {
    if ($itemTongCPNC622Tong['loaisp'] == "CT")
        $TongCPNC622CTTong_CT += ($itemTongCPNC622Tong['tienno']-$itemTongCPNC622Tong['tienco']);
    if ($itemTongCPNC622Tong['loaisp'] == "SP") {
        $TongCPNCP622SPTong += ($itemTongCPNC622Tong['tienno']-$itemTongCPNC622Tong['tienco']);// Tổng nhân công cho toán bộ sản phẩm
        if($TongCPNCP622SPTong['mabp']=='0001'){
            $TongCPNC622SPTong_001+=($itemTongCPNC622Tong['tienno']-$itemTongCPNC622Tong['tienco']);// Tổng nhân công cho toán bộ sản phẩm
        }
    }
}
$TongCPNC622CTTong['CT'] = $TongCPNC622CTTong_CT;
...
foreach ($SPCAP1 as $KMASP => $item) {// Duyệt tất cả sản phẩm cấp cao nhất
    ...
    $TongTienCPSXChungCap1 = ($dataCPSXC[$KMASP]['tienno']- $dataCPSXC[$KMASP]['tienco']);
    ...
    foreach ($SPCAP2 as $KMASP2 => $item2) {
        ...
        $TongTienCPSXChungCap2 = ($dataCPSXC[$KMASP2]['tienno']-$dataCPSXC[$KMASP2]['tienco']);
...
foreach ($CTCAP1 as $KMACT => $item) {// Duyệt tất cả sản phẩm cấp cao nhất
    ...
    $TongDoanhThuThuanCap1 = 0;/// Là Tổng NVL Hoặc NC tùy theo lựa chọn phân bổ SXC
    $TongTienCPSXChungCTCap1 = ($dataCPSXC[$KMACT]['tienno']-$dataCPSXC[$KMACT]['tienco']);
...
foreach ($CTCAP1 as $KMACT => $item) {// Duyệt tất cả sản phẩm cấp cao nhất
    ...
    $DSCTCONCAP1_ARRAY = explode(",", $DSCTCONCAP1);
    $TongDoanhThuThuanCap1 = 0;/// Là Tổng NVL Hoặc NC tùy theo lựa chọn phân bổ SXC
    $TongTienCPMayCTCap1 = ($dataMay[$KMACT]['tienno']-$dataMay[$KMACT]['tienco']);
    foreach ($DSCTCONCAP1_ARRAY as $ItemCap1) {
        $TongDoanhThuThuanCap1 += $dataDoanhThuThuanSP[$ItemCap1]['tienco'];
    }
...
foreach ($CTCAP1 as $KMACT => $item) {// Duyệt tất cả sản phẩm cấp cao nhất
    ...
    $DSCTCONCAP1_ARRAY = explode(",", $DSCTCONCAP1);
    $TongDoanhThuThuanCap1 = 0;/// Là Tổng NVL Hoặc NC tùy theo lựa chọn phân bổ SXC
    $TongTienCPNCCTCap1 = ($dataNhancong[$KMACT]['tienno']-$dataNhancong[$KMACT]['tienco']);
    foreach ($DSCTCONCAP1_ARRAY as $ItemCap1) {
        $TongDoanhThuThuanCap1 += $dataDoanhThuThuanSP[$ItemCap1]['tienco'];
    }
...
//foreach ($TinhCPChuaPhanCap as $iTT){
//$tong1111+= $iTT['sotiennc622pb'];
//}
//debug($TinhCPChuaPhanCap);
//echo $tong1111;
//// Thêm bút toán phát sinh tự động
$sql_emp_pskt = "delete from pskt where loaiphieu ='77' and btps='2'";
database::re_query($sql_emp_pskt);

$sql_emp_chitiet_pskt = "delete from chitiet_pskt where loaiphieu ='77' and btps='2'";
...
foreach ($TinhCPChuaPhanCap as $kmact => $itemChuaPhanCap) {
    $LoaiPhieu = 77;
    $value_pskt .= "('" . ($sophieu) . "',1,'" . $denngay . "','154','{$LoaiPhieu}','" . abs($itemChuaPhanCap['sotiennc622pb']) . "','2'),";
    $value_chitiet_pskt .= "(1,'" . $denngay . "','" . abs($itemChuaPhanCap['sotiennc622pb']) . "','','" . $kmact . "','" . $itemChuaPhanCap['tenct'] . "','100094','Phân bổ chi phí nhân công Mã CT/SP:{$kmact}','','','622','','" . abs($itemChuaPhanCap['sotiennc622pb']) . "','4','{$LoaiPhieu}','" . ($sophieu) . "','2','" . $itemChuaPhanCap['loaisp'] . "'),";
    if ($itemChuaPhanCap['loaisp'] == "CT") {
        ...
        foreach ($TongCPNCCTTong_CTTheoTK623 as $kmatk => $item_CTTheoTK623) {
            if ($item_CTTheoTK623 != 0) {
                ...
                $sophieu++;
                $value_pskt .= "('" . $sophieu . "'," . 2 . ",'" . $denngay . "','154','77','" . abs($tongtencamay) . "','2'),";
                $value_chitiet_pskt .= "(" . 2 . ",'" . $denngay . "','" . abs($tongtencamay) . "','','" . $kmact . "','" . $itemChuaPhanCap['tenct'] . "','100094','Phân bổ chi phí ca máy Mã CT/SP:{$kmact}','','','" . $kmatk . "','','" . abs($tongtencamay) . "','4','77','" . $sophieu . "','2','" . $itemChuaPhanCap['loaisp'] . "'),";
            }
        }
        ...
        foreach ($TongCPSXCTong_CTTheoTK627 as $kmatk => $item_CTTheoTK627) {
            if ($item_CTTheoTK627 != 0) {
                ...
                $sophieu++;
                $value_pskt .= "('" . $sophieu . "'," . 3 . ",'" . $denngay . "','154','77','" . abs($tongtenxpsxc) . "','2'),";
                $value_chitiet_pskt .= "(" . 3 . ",'" . $denngay . "','" . abs($tongtenxpsxc) . "','','" . $kmact . "','" . $itemChuaPhanCap['tenct'] . "','100094','Phân bổ chi phí sản xuất chung Mã CT/SP:{$kmact}','','','" . $kmatk . "','','" . abs($tongtenxpsxc) . "','4','77','" . $sophieu . "','2','" . $itemChuaPhanCap['loaisp'] . "'),";
            }
        }
    }
...
database::re_query("ALTER TABLE `bangphanbo_chiphi_sxchung` ADD `sotienpbcpsxccap1` BIGINT NOT NULL, ADD `sotienpbcpsxccap2` BIGINT NOT NULL AFTER `sotienpbcpsxccap1`, ADD `sotienpbnccap1` BIGINT NOT NULL AFTER `sotienpbcpsxccap2`, ADD `sotienpbnccap2` BIGINT NOT NULL AFTER `sotienpbnccap1`;");
database::re_query("ALTER TABLE `bangphanbo_chiphi_sxchung` ADD `loaipb` CHAR(2) NOT NULL;");
database::re_query("delete from bangphanbo_chiphi_sxchung where loaipb = 'CT'");
database::re_query("delete from bangphanbo_chiphi_sxchung where loaipb = ''");
database::re_query("insert into bangphanbo_chiphi_sxchung(sottxuat,mact,tenct,dodangdk,sotiennl,sotiennc,sotienmay,sotiencpsxc,mactcha,doanhthuthuan,tongcong,sotiencpsxcpb,tylenl,tylenc,tylemay,tylecpsxc,tylecpsxcpb,giathanh,lailo,dodangck,dodangck_,doanhthuhopdong,tylethucte,sotienncpb,sotiennc622pb,sotienpbcpsxccap1,sotienpbcpsxccap2,loaipb) VALUES " . substr($valins, 0, -1));
