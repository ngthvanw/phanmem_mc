...
database::re_query("delete from bangtonghop_danhthu_chiphi_giathanhct where `loaipb`='HD'");
...
if($TuDongNapGia=="true"){
    foreach ($TinhCPChuaPhanCap as $k=>$ItemNG){
        $TongGiaThanh = 0;
        $TongSoLuong = 0;
        $TongThanhTien = 0;
        if($ItemNG['loaisp']=='HD'){
            ...
            while ($data_dinhmuc = $OBJCT->re_fetch($query_dinhmuc)){
                ...
                $TongThanhTien+=$data_dinhmuc['tylehaohoc'];
                $data1_dinhmuc[] = $data_dinhmuc;
            }
            foreach ($data1_dinhmuc as $Item_giathanh){
                $SoLuongDinhMuc     = $Item_giathanh['dinhmuc'];
                $ThanhTienDinhMuc   = $Item_giathanh['tylehaohoc'];
                $DonGiaNhap = round(((($ThanhTienDinhMuc/$TongThanhTien)*$TongGiaThanh)/$SoLuongDinhMuc ),3);
    ...
    $data = $OBJPSCTMAVT->laydanhsachphieunhapkhosanxuathd_();// Lấy danh sách phiếu nhập kho thành phẩm từ hợp đồng
