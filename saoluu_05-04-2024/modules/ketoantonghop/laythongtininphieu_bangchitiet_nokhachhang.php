...
$query_makh = database::re_query($sql_makh);
$data_kh = database::re_fetch($query_makh);
database::re_query("UPDATE pskt SET makh_nh = '' WHERE (loaiphieu!=3 and loaiphieu!=4) or tenkh_nh='';");

$sql_matk = "select * from matk WHERE matk='" . $matk . "'";
$query_matk = database::re_query($sql_matk);
$data_tk = database::re_fetch($query_matk);

$sql_nodk = "select * from sdcn WHERE makh in ('" . $string_makh_da_thaythe . "') and matk='" . $matk . "'";

$query_nodk = database::re_query($sql_nodk);
$data_nodk = database::re_fetch_all($query_nodk);
$str_w = "";
$str_w2 = "";
