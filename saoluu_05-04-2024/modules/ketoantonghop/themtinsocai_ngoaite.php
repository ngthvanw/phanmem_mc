...
foreach ($danhsach_data as $k_matk => $itemTK) {// Duyet vao tk
        ...
        foreach ($itemTHANG as $itemCT) {
            ...
            if ($itemCT["tienco"] != 0 || $itemCT["tienntco"] != 0) {// CÓ NGUYÊN TỆ
                ...
                if (substr($k_matk, 0, 4) == '1122') {
                    //echo $sqlupdate;
                    database::re_query($sqlupdate);
                    database::re_query($sqlupdate2);
                }
    ...
    database::re_query("ALTER TABLE `bangtygianganhang` ADD `sotien` BIGINT NOT NULL AFTER `sotiennt`;");
    database::re_query("ALTER TABLE `bangtygianganhang` CHANGE `tygia` `tygia` DOUBLE(10,3) NOT NULL;");
    database::re_query("delete from bangtygianganhang WHERE matk='{$k_matk}'");
