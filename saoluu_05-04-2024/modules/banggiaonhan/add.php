...
$sql_sott = "SELECT `AUTO_INCREMENT` as sott FROM  INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'phpmyadmin' AND   TABLE_NAME   = 'danhsach_giaonhan_chungtu_{$noiluu_phanmem}';";
$query_sott = database::re_query($sql_sott);
$res_sott = database::re_fetch($query_sott);

$sott = $res_sott['sott'];

database::re_query("ALTER TABLE phpmyadmin.danhsach_giaonhan_chungtu_{$noiluu_phanmem} AUTO_INCREMENT={($sott+1)};");
...
$sql_ins = "insert into phpmyadmin.danhsach_giaonhan_chungtu_{$noiluu_phanmem}(sott,masothue,tencongty,nguoigiao,hotennguoigiao,ngaygiao,nguoinhan,hotennguoinhan,noidung,nguoigiaoky,ngaynguoigiaoky,nguoinhanky,ngaynguoinhanky,ghichu,tenkd,bosung)
