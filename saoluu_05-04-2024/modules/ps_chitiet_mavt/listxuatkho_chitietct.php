...
$check=$OBJ->kiemtraphieuvt_ctct(); // Kiểm tra xem phiếu này có nhâp vật tư chưa
$OBJ->set_orderby($filterQuery);
$OBJ->setCothueGTGT($cothuegtgt);
$OBJ->setCoChietKhau($cochietkhau);
...
if($LoadUnion==1){
    ...
    $result = $OBJ->loadListMaVTXuat_CTCT_Union($mact);
}else {
    if ($check) {
        //$OBJ->set_orderby(" sophieu = " . $_GET['sophieu']);
        $result = $OBJ->loadListMaVT_CTCT($mact);
    } else {
        $result = $OBJ->loadListMaVTXuat_CTCT_Notjon($mact);
    }
}
