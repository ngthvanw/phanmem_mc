    ...
    public static function deSerializeFilter($pq_filter)        
    {
        ...
        {            
            ...
            if ($condition == "contain")
            {
                ...
                //$fc[] = " MATCH(".$dataIndx.") AGAINST('{$text}' IN NATURAL LANGUAGE MODE)";
            }
...
if($LoadUnion==1){
    $result = $OBJ->loadListMaVTUnion($thangngayghiso,$makho);
}else {
    if ($check) {
        $result = $OBJ->loadListMaVT($thangngayghiso,$makho);
    } else {
        $result = $OBJ->loadListMaVTNotjon($thangngayghiso,$makho);
    }
}
