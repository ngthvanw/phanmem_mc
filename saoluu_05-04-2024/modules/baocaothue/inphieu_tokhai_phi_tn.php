...
class PDF extends tFPDF
{
    ...
    function ImprovedTable($header, $data,$nhomnganh)
    {
        ...
        $this->Cell(234,3,"",'0',0,'L');// LTRB là border c?a cell
        $this->SetFont('timesi', '', 8);
        $this->Cell(50,3,"Đơn vị tiền: Đồng Việt Nam",'0',0,'R');
        $this->Ln(9);

        $w = array(0=>0,1=>10,2=>70,3=>15,4=>20,5=>25,6=>15,7=>32,8=>32,9=>32,10=>32);
        $this->SetFont('timesb', '', 10);
        ...
        $this->Cell($w[3]+$w[4],5,"Sản lượng tài nguyên",'LTB',0,'C');
        $this->Cell($w[5],5,"",'LTR',0,'C');
        $this->Cell($w[6],5,"",'LTR',0,'C');
        $this->Cell($w[7],5,"",'LTR',0,'C');
        $this->Cell($w[8],5,"",'LTR',0,'C');
        $this->Cell($w[9],5,"",'LTR',0,'C');
        $this->Cell($w[10],5,"",'LTR',0,'C');
        $this->Ln();
        $this->Cell($w[1],5,"STT",'L',0,'C');// LTRB là border c?a cell
        $this->Cell($w[2],5,"Tên loại tài nguyên ",'L',0,'C');// LTRB là border c?a cell
        $this->Cell($w[3],5,"Đơn vị",'L',0,'C');
        $this->Cell($w[4],5,"Sản lượng",'L',0,'C');
        $this->Cell($w[5],5,"Giá tính thuế",'L',0,'C');
        $this->Cell($w[6],5,"Thuế",'L',0,'C');
        $this->Cell($w[7],5,"Mức thuế TN",'L',0,'C');
        $this->Cell($w[8],5,"Thuế TN phát",'L',0,'C');
        $this->Cell($w[9],5,"Thuế TN dự kiến",'L',0,'C');
        $this->Cell($w[10],5,"Số phí phải nộp",'LR',0,'C');
        $this->Ln();
        ...
        $this->Cell($w[5],5,"đv tài nguyên",'LRB',0,'C');
        $this->Cell($w[6],5,"suất",'LRB',0,'C');
        $this->Cell($w[7],5,"ấn định",'LRB',0,'C');
        $this->Cell($w[8],5,"sinh trong kỳ",'LRB',0,'C');
        $this->Cell($w[9],5,"được miễn trong kỳ",'LRB',0,'C');
        $this->Cell($w[10],5,"trong kỳ",'LRB',0,'C');
        $this->Ln();
        ...
        $this->Cell($w[6], 5,"(5)", 'LRB', 0, 'C');
        $this->Cell($w[7], 5,"(5)", 'LRB', 0, 'C');
        $this->Cell($w[8], 5,"(5)", 'LRB', 0, 'C');
        $this->Cell($w[9], 5,"(5)", 'LRB', 0, 'C');
        $this->Cell($w[10], 5,"(6)=(4)x(5)", 'LRB', 0, 'C');
        $this->Ln();

        $this->SetFont('timesb', '', 9);
        $this->SetLineWidth(0.03);
        $this->Cell($w[1], 5,"I", 'LB', 0, 'C');// LTRB là border c?a cell
        $this->Cell($w[2], 5,"Tài nguyên khai thác:", 'LRB', 0, 'L');// LTRB là border c?a cell
        $this->Cell($w[3], 5, "", 'LRB', 0, 'C');
        $this->Cell($w[4], 5,"", 'LRB', 0, 'R');
        $this->Cell($w[5], 5,"", 'LRB', 0, 'C');
        $this->Cell($w[6], 5,"", 'LRB', 0, 'C');
        $this->Cell($w[7], 5,"", 'LRB', 0, 'C');
        $this->Cell($w[8], 5,"", 'LRB', 0, 'C');
        $this->Cell($w[9], 5,"", 'LRB', 0, 'C');
        $this->Cell($w[10], 5,"", 'LRB', 0, 'R');
        $this->Ln();
        ...
        $this->Cell($w[6], 5,(number_format($data['I']['thuesuat']) == 0) ? "" : number_format($data['I']['thuesuat'],0,",","."), 'LRB', 0, 'C');
        $this->Cell($w[7], 5,(number_format($data['I']['thueandinh']) == 0) ? "" : number_format($data['I']['thueandinh'],0,",","."), 'LRB', 0, 'C');
        $this->Cell($w[8], 5,(number_format($data['I']['thuephatsinh']) == 0) ? "" : number_format($data['I']['thuephatsinh'],0,",","."), 'LRB', 0, 'C');
        $this->Cell($w[9], 5,(number_format($data['I']['thuegiam']) == 0) ? "" : number_format($data['I']['thuegiam'],0,",","."), 'LRB', 0, 'C');
        $this->Cell($w[10], 5,(number_format($data['I']['thanhtien']) == 0) ? "" : number_format($data['I']['thanhtien'],0,",","."), 'LRB', 0, 'R');
        $this->Ln();

        $this->SetFont('timesb', '', 9);
        $this->SetLineWidth(0.03);
        $this->Cell($w[1], 5,"II", 'LB', 0, 'C');// LTRB là border c?a cell
        $this->Cell($w[2], 5,"Tài nguyên thu mua gom:", 'LRB', 0, 'L');// LTRB là border c?a cell
        $this->Cell($w[3], 5, "", 'LRB', 0, 'C');
        $this->Cell($w[4], 5,"", 'LRB', 0, 'R');
        $this->Cell($w[5], 5,"", 'LRB', 0, 'C');
        $this->Cell($w[6], 5,"", 'LRB', 0, 'C');
        ...
        $this->Cell($w[8], 5,"", 'LRB', 0, 'C');
        $this->Cell($w[9], 5,"", 'LRB', 0, 'C');
        $this->Cell($w[10], 5,(number_format($thuenhom2) == 0) ? "" : number_format($thuenhom2,0,",","."), 'LRB', 0, 'R');
        $this->Ln();

        $this->SetFont('timesb', '', 9);
        $this->SetLineWidth(0.03);
        $this->Cell($w[1], 5,"III", 'LB', 0, 'C');// LTRB là border c?a cell
        $this->Cell($w[2], 5,"Tài nguyên tịch thu, giao bán:", 'LRB', 0, 'L');// LTRB là border c?a cell
        $this->Cell($w[3], 5, "", 'LRB', 0, 'C');
        $this->Cell($w[4], 5,"", 'LRB', 0, 'R');
        $this->Cell($w[5], 5,"", 'LRB', 0, 'C');
        $this->Cell($w[6], 5,"", 'LRB', 0, 'C');
        ...
        $this->SetFont('timesb', '', 9);
        $this->SetLineWidth(0.03);
        $this->Cell($w[1]+$w[2]+$w[3]+$w[4]+$w[5]+$w[6]+$w[7]+$w[8]+$w[9], 5,"Tổng cộng:", 'LRB', 0, 'C');// LTRB là border c?a cell
        $this->Cell($w[10], 5,(number_format($data['I']['thanhtien']) == 0) ? "" : number_format($data['I']['thanhtien'],0,",","."), 'LRB', 0, 'R');
        $this->Ln();
        $this->Ln();
        ...
        $this->Cell(190, 4,"NHÂN VIÊN ĐẠI LÝ THUẾ", '0', 0, 'L');// LTRB là border c?a cell
        $this->SetFont('times', '', 9);
        ...
        $this->SetFont('times', '', 9);
        $this->Cell(150, 4, $_SESSION['txt_hovatendaily'], 0, 0, L);
        $this->SetFont('timesb', '', 9);
        ...
        $this->SetFont('times', '', 9);
        $this->Cell(150, 4, $_SESSION['txt_chungchindaily'], 0, 0, L);
        $this->SetFont('timesb', '', 9);
