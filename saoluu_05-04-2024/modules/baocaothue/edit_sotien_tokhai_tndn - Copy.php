...
foreach ($mangsotien as $itemSoTien){
    ...
    database::re_query(" update tokhaitndn set sotien=".$$sotien." where machitieu='".$itemSoTien['machitieu']."' and loaitokhai='TNDN'");
}
...
if($TongTienTruocThue>=0){
    ...
    $select = database::re_query("select tiencock from buttoanps WHERE maso='KCNSDN'");
    $data =  database::re_fetch($select);
    $tiendoanhnghiepdanop = $data['tiencock'];
    $thuednphainop = $TienThue - $tiendoanhnghiepdanop;
    if($thuednphainop <= 0){
        database::re_query(" UPDATE buttoanps SET `sotien` = '".abs($thuednphainop)."' WHERE maso = 'KCNSDN';");
        database::re_query(" update pskt t1 
                                        join chitiet_pskt t2 on t1.sophieu=t2.sophieu
        ...
        database::re_query(" UPDATE buttoanps SET `sotien` = '0' WHERE maso = 'KCNSDN';");
        database::re_query(" update pskt t1 
                                        join chitiet_pskt t2 on t1.sophieu=t2.sophieu
                                        set t1.tongcong='0',
                                            t2.gtvnd1='0',
