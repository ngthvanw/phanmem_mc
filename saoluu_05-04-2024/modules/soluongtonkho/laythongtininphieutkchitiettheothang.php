        ...
        if ($tutaobuttoanphatsinh == "true") {
            ...
            foreach ($data as $sophieu => $iTem) {
                foreach ($iTem as $iTemMaVT) {
                    ...
                    database::re_query("update chitiet_psvt set donggianhap='" . $dongia . "',thanhtienchuack='" . $thanhtien . "',thanhtien='" . $thanhtien . "' where sott='" . $iTemMaVT['sott'] . "'");
                }
            }
            foreach ($dinhkhoan as $ksophieu => $itemDK) {
                foreach ($itemDK as $kmatk => $iTemSoTien) {
                    database::re_query("update dinhkhoan_psvt set sotien='" . round($iTemSoTien) . "' where sophieu='" . $ksophieu . "' and tkno='" . $kmatk . "'");
                }
            }
