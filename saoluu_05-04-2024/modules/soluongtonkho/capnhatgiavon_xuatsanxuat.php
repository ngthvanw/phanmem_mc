...
if ($PhuongPhapTinhGiaVon == 2) {
    ...
    foreach ($dataPXK as $itemPXK) {
        foreach ($dataGiaVonLienHoan[$itemPXK['sophieu']] as $itemPhieuXuat) {
            if ($itemPXK['loaiphieu'] == 2) {
                ...
                database::re_query("update chitiet_psvt INNER JOIN tkthang on chitiet_psvt.sophieu=tkthang.sophieu_nxk set chitiet_psvt.donggianhap=tkthang.dongia,chitiet_psvt.thanhtienchuack=tkthang.thanhtienxuat,chitiet_psvt.thanhtien=tkthang.thanhtienxuat,chitiet_psvt.thue=0 where sophieu='" . $itemPhieuXuat['sophieu_nxk'] . "' and chitiet_psvt.mavt=tkthang.mavt ");
                //database::re_query("update chitiet_psvt set donggianhap='" . $itemPhieuXuat['dongiabinhquan'] . "',thanhtienchuack='" . $itemPhieuXuat['thanhtienxuat'] . "',thanhtien='" . $itemPhieuXuat['thanhtienxuat'] . "' where sophieu='" . $itemPhieuXuat['sophieu'] . "' and mavt='" . $itemPhieuXuat['mavt'] . "' and soluong='".$itemPhieuXuat['soluongxuat']."'");
            }
        }
    }
    ...
    foreach ($data as $sophieu => $iTem) {
        foreach ($iTem as $iTemMaVT) {
            ...
            database::re_query("update chitiet_psvt set donggianhap='" . $dongia . "',thanhtienchuack='" . $thanhtien . "',thanhtien='" . $thanhtien . "',thue=0 where sott='" . $iTemMaVT['sott'] . "'");
        }
    }

    foreach ($dinhkhoan as $ksophieu => $itemDK) {
        foreach ($itemDK as $kmatk => $iTemSoTien) {
            database::re_query("update dinhkhoan_psvt set sotien='" . round($iTemSoTien) . "' where sophieu='" . $ksophieu . "' and tkco='" . $kmatk . "'");
            database::re_query("update dinhkhoan_psvt set sotien='0' where sophieu='" . $ksophieu . "' and tkco='33311'");
        }
    }
        ...
        foreach ($datatkthang as $k => $itemtkthang) {
            ...
            if ($tmptinh != 0 && $tmptinh < 100000 && $tmptinh > (-100000)) {// Nếu tổng tiền tồn kho cuối kỳ bị lệch so với tồn kho từng tháng
                ...
                if ($rows_update == 0) {
                    ...
                    database::re_query("update chitiet_psvt set thanhtienchuack=(thanhtienchuack+{$tmptinh}),thanhtien=(thanhtien+{$tmptinh}) where sophieu='" . $datasophieu['sophieu'] . "' order by soluongnhap LIMIT 1");
                    database::re_query("update dinhkhoan_psvt set sotien=(sotien+{$tmptinh}) where sophieu='" . $datasophieu['sophieu'] . "' and tkco='" . $k . "' LIMIT 1");
                }
            }
        }
    }
    database::re_query($sql_pskt1);
    database::re_query($sql_chitiet_pskt1);
}
