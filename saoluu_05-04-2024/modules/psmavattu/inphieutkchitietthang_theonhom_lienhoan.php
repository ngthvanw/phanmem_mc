        ...
        $(document).ready(function () {
            $('#xuatexcel').click(function () {
                $(".dt-print-view").table2excel({
                    ...
                    filename: "tklienhoan_" + new Date().toISOString().replace(/[\-\:\.]/g, "") + ".xls",
                    fileext: ".xls",
                    exclude_img: true,
                    exclude_links: true,
                    exclude_inputs: true,
...
<table class="no-print" style="background-color: #00c6ff;" width="100%" border="0">
    <tr>
        <td class="no-print"><input class="no-print" type="button"
                                    style="color: #0000CC;font-weight: bold;border: 2px solid red;" value="Xuất excel"
                                    id="xuatexcel"></td>
        <td></td>
    </tr>
</table>
...
<thead>
  <tr>
    ...
    <th STYLE="border:0.5px solid #000;"  rowspan="2">Ngày ghi sổ</th>
    <th STYLE="border:0.5px solid #000;"  rowspan="2">Số phiếu</th>
    ...
    <th STYLE="border:0.5px solid #000;"  rowspan="2" >Đơn giá</th>
    ...
    <th STYLE="border:0.5px solid #000;"  colspan="2">Số tồn cuối kỳ</th>
  </tr>
  <tr>
  ...
  ';
    ...
    foreach ($dataCT as $key => $tiemG) {
        ...
        foreach ($tiemG as $tiemCT) {
            {
                ...
                $soluongg = 0;
                $demall++;
                if ($tiemCT['soluong'] != 0 || $tiemCT['soluongtonck'] != 0 || $tiemCT['soluongnhap'] != 0 || $tiemCT['soluongxuat'] != 0) {
                    $sott++;
                    $thanhtiendk = 0;
                    $soluongdk = 0;
                    if ($tiemCT['sottct'] == "") {
                        $soluongdk = $tiemCT['soluong'];
                        $thanhtiendk = $tiemCT['thanhtien'];
                    }
                    $dongiabinhquan = "";
                    if($tiemCT['soluongxuat']!=0){
                        $dongiabinhquan =$tiemCT['dongiabinhquan'];
                    }
                    $html_ct .= '
        <tr >
        <td class="td_first"  width="35px" align="center" >' . $sott . '</td>
        <td class="td_center" align="center" style="font-size: 12px" width="80px" >' . date("d-m", strtotime($tiemCT['ngs'])) . '</td>
        <td class="td_center" align="center" width="80px" >' . $tiemCT['sottct'] . '</td>
        ...
        <td class="td_center" align="center" width="40px" >' . number_format($dongiabinhquan, 3, ",", ".") . '</td>
        <td class="td_center" width="90px" align="right" >' . (($soluongdk == 0) ? "" : number_format($soluongdk, 3, ",", ".")) . '</td>
        <td align="right" class="td_end" width="100px" >';
                    $html_ct .= (($thanhtiendk == 0) ? "" : number_format($thanhtiendk, 0, ",", "."));
                    $html_ct .= '</td>
        ...
        <td align="right" class="td_end" width="100px" >';
                    $html_ct .= (($tiemCT['thanhtientonck'] == 0 || $tiemCT['soluongtonck'] == 0) ? "" : number_format($tiemCT['thanhtientonck'], 0, ",", "."));
                    $html_ct .= '</td>

        </tr>';
                }
                if ($tiemCT['sottct'] == "") {
                    $tongsldkg += $tiemCT['soluong'];
                    $tongthanhtiendkg += $tiemCT['thanhtien'];
                    $tongsldk += $tiemCT['soluong'];
                    $tongthanhtiendk += $tiemCT['thanhtien'];
                }

                $thanhtienck[$tiemCT['mavt']] = $tiemCT['thanhtientonck'];

                $tongthanhtiennhapps += $tiemCT['thanhtiennhap'];
                $tongthanhtienxuatps += $tiemCT['thanhtienxuat'];
                $slck[$tiemCT['mavt']] = $tiemCT['soluongtonck'];

                $tongslnhap += $tiemCT['soluongnhap'];
                $tongslxuat += $tiemCT['soluongxuat'];

                $thanhtienckg[$tiemCT['mavt']] = $tiemCT['thanhtientonck'];


                $slckg[$tiemCT['mavt']] = $tiemCT['soluongtonck'];
        ...
        $tongslckg = array_sum($slckg);
        $tongthanhtienckg = array_sum($thanhtienckg);
    ...
    $tongslck = array_sum($slck);
    $tongthanhtienck = array_sum($thanhtienck);
    $html = '
