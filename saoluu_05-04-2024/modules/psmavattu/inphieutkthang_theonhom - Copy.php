...
class PDF extends tFPDF
{
    ...
    {
        $w = array(0,8,24,86,8,13,20,20,26);
    ...
    foreach ($_SESSION["LISTTKTHANG"] as $key =>$tiemG) {
        ...
        $this->Cell($w[3], 5, mb_substr($tiemCT['tenvt'],0,57,'UTF-8'), 'LR', 0, 'L');// LTRB là border c?a cell
        $this->Cell($w[4], 5, $tiemCT['matk'], 'LR', 0, 'C');
        $this->Cell($w[5], 5, ucfirst($tiemCT['dvt']), 'LR', 0, 'C');
        ...
        $tongsoluong+=$tiemCT['soluongtonck'];
        ...
        $this->Cell($w[3], 5,$key, 'LR', 0, 'R');// LTRB là border c?a cell
        ...
        $this->Cell($w[6],5,number_format($tongsoluong,2,",","."),'LTRB',0,'R');
