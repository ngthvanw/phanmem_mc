...
database::re_query("UPDATE manhanvien set manhanvien=sott where manhanvien=0`;");
//// Bảng thu khách hàng từ ngày đến ngÀY
database::re_query("ALTER TABLE `bangluongnhanvien` ADD `phicongdoan` BIGINT NOT NULL AFTER `phucapchucvu`;");
database::re_query("ALTER TABLE `bangluongnhanvien` ADD `baohiemyt` BIGINT NOT NULL AFTER `baohiem`, ADD `baohiemtn` BIGINT NOT NULL AFTER `baohiemyt`, ADD `kinhphicongdoan` BIGINT NOT NULL AFTER `baohiemtn`, ADD `dn_baohiem` BIGINT NOT NULL AFTER `kinhphicongdoan`, ADD `dn_baohiemyt` BIGINT NOT NULL AFTER `dn_baohiem`, ADD `dn_baohiemtn` BIGINT NOT NULL AFTER `dn_baohiemyt`;");
...
if($_SESSION['theothongtu']=="tt200"){
   ...
   $value_tmpluong.="('{$itemListNhanVien['matk1']}','{$tkluong}','{$TONGLUONGCP1}','{$loaisp}'),
                   ...
                   ('{$tkluong}','3386','".abs( $itemListNhanVien['baohiemtn'])."',''),
                   ...
                   ('{$itemListNhanVien['matk1']}','3386','{$TONGDNBHTN1}','{$loaisp}'),
                   ('{$itemListNhanVien['matk2']}','3386','{$TONGDNBHTN2}','{$loaisp}'),";

}
$sql_insert_tmp = "insert into tmp_hachtoanluongthang(tkno,tkco1,sotien1,loaisp) VALUE " . substr($value_tmpluong, 0, -1);

database::re_query("delete from tmp_hachtoanluongthang");
database::re_query($sql_insert_tmp);
...
foreach ($dataListtmpButToan as $itemButToanTMP){
    if(substr($itemButToanTMP['tkco1'],0,3)=='338' || $itemButToanTMP['tkco1']=='3335'){
        ...
        }else if($itemButToanTMP['tkco1']=='3386' && substr($itemButToanTMP['tkno'],0,3)=='334') {
...
if($tinhluongtheo=="false"){
    ...
    database::re_query("delete from bangluongnhanvien WHERE thang='{$txttinhluongtheo}'");
    database::re_query($sql_in);
    ...
    database::re_query($sql_emp_pskt_83);
    database::re_query($sql_emp_chitiet_pskt_83);
    if($khongtaobuttoan!="true") {
        database::re_query($sql_pskt_83);
        database::re_query($sql_chitiet_pskt_83);
    }
}
