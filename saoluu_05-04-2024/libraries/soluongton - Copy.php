...
class soluongton extends database{
    ...
    function loadListMaVT($parentid =0){
        ...
        if($fill!="")
            $sql_w = " and ".$this->get_orderby() ;
        $sql = "SELECT tk.mavt,tk.tenvt,tk.matk,tk.quycach,tk.dvt,tk.gtvnck,tk.slck,tk.dgxvnd,tk.rate,tk.manhom,manhom.tennhom FROM tk left JOIN mavt on (tk.mavt = mavt.mavt) inner join manhom on (tk.manhom = manhom.manhom) where 0=0 $sql_w
                UNION ALL 
                SELECT mavt,tenvt,matk,quycach,dvt,giamua as gtvnck,sl as slck,sl as dgxvnd,rate,mavt.manhom,manhom.tennhom FROM mavt inner join manhom on (mavt.manhom = manhom.manhom) WHERE mavt not in(select mavt from tk) $sql_w ORDER BY tenvt " .$this->getLimit();
        $this->query($sql);
        $i=0;
        while($data=$this->fetch())
        {
    ...
    public function xoaMaVT(){
	 ...
	 function layTongTienTonKho(){
        ...
        if($fill!="")
            $sql_w = " and ".$this->get_orderby() ;
        $sql = "SELECT sum(dgxvnd) as thanhtien,sum(slck) as soluong FROM tk ";
        $this->query($sql);
        $i=0;
        while($data=$this->fetch())
        {
