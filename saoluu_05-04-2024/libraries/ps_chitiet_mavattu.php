...
class ps_chitiet_mavattu extends database
{
    ...
    {// Tính phát sinh nhập trong khoản thời gian nhất định
        $sql = "SELECT mavt,thang1,thang2,thang3,thang4,thang5,thang6,thang7,thang8,thang9,thang10,thang11,thang12 FROM tmp_tksphienthai";// Tồn đầu kỳ
        $query = $this->re_query($sql);
    ...
    function laydanhsachphieunhapkhosanxuathd_()//// Lấy thông tin nhập kho để sản xuất-------------
    {// Tính phát sinh nhập trong khoản thời gian nhất định
        $sql = "SELECT t1.sophieu,t3.matk,t1.mavt,t1.soluongnhap,t1.sott,t2.loaisp,t2.makho masp FROM chitiet_psvt t1 inner join psvt t2 on (t1.sophieu=t2.sophieu) INNER join mavt t3 on(t1.mavt=t3.mavt) where t2.loaiphieu=1 and t2.loaisp='HD'";// Tồn đầu kỳ
        $query = $this->re_query($sql);
        while ($data = $this->re_fetch($query)) {
            $row[$data['sophieu']][] = $data;
        }
