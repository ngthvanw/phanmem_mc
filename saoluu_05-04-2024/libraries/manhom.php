...
class manhom extends database{
    ...
    public function checkKeyTrung(){
		$sql="select * from manhom where manhom='".$this->get_MaNhom()."' and sott!=".$this->get_SoTT()."";
		$this->query($sql);
		if($this->num_rows() >= 1){
			return FALSE;
		}else{
	...
	/*function loadListMaKho($parentid =0){
     ...
     public function createSoTT(){
		$sql="select max(sott) as sott from manhom";
		$this->query($sql);
		if($this->num_rows()==1){
	       $data=$this->fetch();
           return $data['sott']+1;
	...
	public function loadListMaNhom(){
		...
		$sql="select * from manhom where 0=0 $sql_w  order by rank DESC" ;
        $this->query($sql);
			$i=0;
			while($data=$this->fetch()){
				$i++;
    ...
    public function CB_ListMaNhomVT(){
        $trees = array();
        $fill = $this->get_orderby();
        if($fill!="")
            $sql_w = " and ".$this->get_orderby();
        $sql="select manhom,tennhom from manhom where 0=0 $sql_w  order by manhom DESC" ;
        $this->query($sql);
        $i=0;
        while($data=$this->fetch()){
            $i++;
    ...
    public function CB_ListMaNhomVTHH(){
        $trees = array();
        $fill = $this->get_orderby();
        if($fill!="")
            $sql_w = " and ".$this->get_orderby();
	...
	public function getMaNhom(){
		$sql="select * from manhom where manhom='".$this->get_MaNhom()."'";
		$this->query($sql);
		if($this->num_rows() == 0){
			return 0;
		}else{
...
public function themMaNhom(){
       $sql="INSERT INTO manhom (sott,manhom,tennhom,ghichu) 
                       VALUES ('".$this->get_SoTT()."','".$this->get_MaNhom()."','".$this->get_TenNhom()."','".$this->get_ChuThich()."' );";
       $this->query($sql);
}

public function suaMaNhom(){
    $sql="update manhom set tennhom='".$this->get_TenNhom()."',ghichu='".$this->get_ChuThich()."'
                  WHERE sott = '".$this->get_SoTT()."'";
     $this->query($sql);  
	 
	 $sqltk="update tk set tennhom='".$this->get_TenNhom()."'
                  WHERE manhom = '".$this->get_MaNhom()."'";
     $this->query($sqltk);  
	 
}
public function xoaMaNhom(){
	   $sql="delete from manhom where manhom='".$this->get_MaNhom()."'";
	   $this->query($sql);
}
}
?>
