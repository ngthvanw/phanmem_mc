...
class makho extends database
{
    ...
    public function CB_loadListMaKho()
    {
        $trees = array();
        $fill = $this->get_orderby();
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
        $sql = "select makho,tenkho from makho where 0=0 $sql_w  order by makho";
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
            $i++;
    ...
    public function loadListMaKho_CoKeyLaMa()
    {
        $trees = array();
        $fill = $this->get_orderby();
        if ($fill != "")
        ...
        while ($data = $this->fetch()) {
            $i++;
            $data['STT'] = $i;
			$data['tenkho_'] = "Kho hàng: ".$data['tenkho'];
            $trees[$data['makho']] = $data;
        }
        return $trees;
    }
