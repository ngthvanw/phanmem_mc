...
class manhanvien extends database
{
    ...
    public function LoadBangLuongNhanVienTongHop()
    {
        ...
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
        $sql = "SELECT manv,tennv,chucvu,trinhdo,sum(luongcb) as luongcb,sum(phucapchucvu) as phucapchucvu,sum(baohiem) as baohiem,socmnd,sum(tienangiuaca) as tienangiuaca,sum(phucapkhongdungbhxh) as phucapkhongdungbhxh,sum(thuethunhap) as thuethunhap,matk1,phantramtk,matk2,loaibp,sapxep FROM bangluongnhanvien WHERE 0=0 GROUP BY manv";
        $this->query($sql);
        if ($this->num_rows() == 0) {
            return $trees;
        } else {
    ...
    {
        $sql = "select max(sott) as sott from manhanvien";
        $this->query($sql);
        if ($this->num_rows() == 1) {
            $data = $this->fetch();
            return $data['sott'] + 1;
