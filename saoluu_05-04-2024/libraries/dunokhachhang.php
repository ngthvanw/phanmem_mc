<?php
class dunokhachhang extends database{
    public $tringorder;
    ...
    function loadListDuNoKH($parentid =0){// Lấy danh sách khách hàng
        $trees = array();
        $fill = $this->get_orderby();
        if($fill!="")
            $sql_w = " and ".$this->get_orderby() ;
        $sql = "SELECT makh,tenkh,matk,sophaithu,sophaitra,diachi FROM sdcn as a where thang='0' $sql_w
                  UNION SELECT makh,tenkh,matk,sl as sophaithu,sl as sophaitra,diachi FROM makh WHERE makh not in(select makh from sdcn where thang='0') $sql_w";
        $this->query($sql);
        $i=0;
        while($data=$this->fetch())
        {
