   ...
   SELECT pskt.mapskt,pskt.loaiphieu,chitiet_pskt.tkno2 as tk,chitiet_pskt.mapskt as sophieu,noidung2 as noidung,pskt.ngayghiso,chitiet_pskt.sct,chitiet_pskt.ngayhoadon,chitiet_pskt.chuthich,0 as tienco,chitiet_pskt.gtvnd2 as tienno,0 as tienntco,chitiet_pskt.sotiennt1 as tienntno, tkco as tkdu,MONTH(ngayghiso) as thang,chitiet_pskt.sott FROM pskt INNER JOIN chitiet_pskt on(pskt.sophieu=chitiet_pskt.sophieu) where  pskt.loaiphieu in(4) and (makh!=makh_nh and makh_nh!='') and chitiet_pskt.tkno2 in ('" . $matk . "') $sql_mand1  " . $this->getStrOderby4() . "
    ...
    public function load_danhsach_congtrinh_dodang_chuaphancap($ListCT, $dodangdauky, $arrayNL, $arrayNC, $arrayMay, $arrayCPSXC, $dataDoanhThuThuan, $TongDoanhThuThuan, $TongCPSXCTong, $dataGiaThanh, $TongCPSXSPTong, $TongDoanhThuThuanSP, $dataDoanhThuThuanSP, $dataCPNCTong, $TongCPNCCTTong, $TongCPNCPSPTong, $TongCPNC622CTTong, $TongCPNCP622SPTong, $SoTienPhanBoCap1, $SoTienPhanBoCap2,$SoTienPhanBo627CTCap1,$SoTienPhanBo623CTCap1,$SoTienPhanBo622CTCap1)// khong sử dụng cho sổ cái
    {
        ...
        if ($mactluu != "") {
            ...
            }else if ($loaisp == "HD") {// Nếu là cong trình thì thực hiện theo doanh thu thực hiện
                $row[$mactluu]['sotiencpsxcpb'] = $row[$mactluu]['sotiencpsxcpb'] - ($tongtiensxcpb - $TongCPSXCTong);
                $row[$mactluu]['sotienncpb'] = $row[$mactluu]['sotienncpb'] - ($tongtienncpb - $TongCPNCCTTong);
    ...
    public function load_danhsach_congtrinh_dodang_chuaphancap_cocpsxpb($ListCT, $dodangdauky, $arrayNL, $arrayNC, $arrayMay, $arrayCPSXC, $dataDoanhThuThuan, $TongDoanhThuThuan, $TongCPSXCTong, $dataGiaThanh, $dataCPSXCPB, $dataGiaThanhTieuChuan, $HeSoTungSanPham, $DataNCTieuChuan, $TongNCThucTe, $dataSoLuongXuatkhoThanhPham, $DataNVLTieuChuan, $DataCPSXCTieuChuan, $DataMayTieuChuan, $dataSoLuongDuTruSanPham, $sophieu, $tutaobuttoan, $dataNCPB, $masp, $dataNC622PB, $dataTong154, $dataPhanBoPhanXuong,$dataMay_CTTheoTK,$dataCPSXC_CTTheoTK,$dataNC622PBSP,$dataTongCTKhoan,$denngay)// khong sử dụng cho sổ cái
    {
        ...
        foreach ($ListCT as $KMaCT => $iTiem) {
            ...
            if ($iTiem['loaisp'] == "SP") {
                ...
                $value_pskt .= "('" . $sophieu . "','1','" . $denngay . "','154','79','" . abs($sotiennltieuchuan + $sotiennctieuchuan) . "'),";
                ...
                foreach ($GiaTriKCQua_taikhoanchung_623_627['623'] as $K_taikhoanchung_623=>$Item_taikhoanchung_623){// Duyệt tài khoản tạm 623
                    ...
                    if($TongTienCPSXTieuChuan623>0){
                        ...
                        $sophieu++;
                        $value_pskt .= "('" . ($sophieu) . "','1','" . $denngay . "','154','79','" . $TienHoachToan_623 . "'),";
                        $value_chitiet_pskt .= "('" . 1 . "','" . $denngay . "','" . $TienHoachToan_623 . "','0','{$KMaCT}','{$KMaCT}','{$iTiem['loaisp']}','100093','Kết chuyển CP máy tiêu chuẩn - SP:{$KMaCT} ','100093','Kết chuyển CP sản xuất chung tiêu chuẩn - SP:{$KMaCT} ','".$K_taikhoanchung_623."','627','" . $TienHoachToan_623 . "','4','79','" . ($sophieu) . "'),";
                    }
                }
                $TongTienCPSXTieuChuan627 = abs($TienCPSXRiengTieuChuan+$TienCPSXChungTieuChuan);// Tổng Tiền 623 của sản phẩm
                foreach ($GiaTriKCQua_taikhoanchung_623_627['627'] as $K_taikhoanchung_627=>$Item_taikhoanchung_627){// Duyệt tài khoản tạm 623
                    ...
                    if($TongTienCPSXTieuChuan627>0){
                        ...
                        $sophieu++;
                        $value_pskt .= "('" . ($sophieu) . "','1','" . $denngay . "','154','79','" . $TienHoachToan_627 . "'),";
                        $value_chitiet_pskt .= "('" . 1 . "','" . $denngay . "','0','" . $TienHoachToan_627 . "','{$KMaCT}','{$KMaCT}','{$iTiem['loaisp']}','100093','Kết chuyển CP máy tiêu chuẩn - SP:{$KMaCT} ','100093','Kết chuyển CP sản xuất chung tiêu chuẩn - SP:{$KMaCT} ','','".$K_taikhoanchung_627."','" . $TienHoachToan_627 . "','4','79','" . ($sophieu) . "'),";
                    }
                }
                ...
                $value_pskt .= "('" . $sophieu . "','1','" . $denngay . "','154','79','" . abs($sotiennl + $sotiennc) . "'),";
                ...
                foreach ($dataMay_CTTheoTK[$KMaCT] as $key_matk=> $ItemMay_CTTheoTK){
                    $sophieu++;
                    $value_pskt .= "('" . ($sophieu) . "','1','" . $denngay . "','154','79','" . abs($ItemMay_CTTheoTK['tienno']) . "'),";
                    $value_chitiet_pskt .= "('" . 1 . "','" . $denngay . "','" . abs($ItemMay_CTTheoTK['tienno']) . "','" . abs(0) . "','{$KMaCT}','{$KMaCT}','{$iTiem['loaisp']}','100093','Kết chuyển CP máy - CT:{$KMaCT}','100093','Kết chuyển CP sản xuất chung - CT:{$KMaCT} ','".$key_matk."','','" . abs($ItemMay_CTTheoTK['tienno']) . "','4','79','" . ($sophieu) . "'),";
                }
                foreach ($dataCPSXC_CTTheoTK[$KMaCT] as $key_matk=> $ItemCPSXC_CTTheoTK){
                    $sophieu++;
                    $value_pskt .= "('" . ($sophieu) . "','1','" . $denngay . "','154','79','" . abs(($ItemCPSXC_CTTheoTK['tienno']-$ItemCPSXC_CTTheoTK['tienco'])) . "'),";
                    $value_chitiet_pskt .= "('" . 1 . "','" . $denngay . "','" . abs(0) . "','" . abs(($ItemCPSXC_CTTheoTK['tienno']-$ItemCPSXC_CTTheoTK['tienco'])) . "','{$KMaCT}','{$KMaCT}','{$iTiem['loaisp']}','100093','Kết chuyển CP máy - CT:{$KMaCT}','100093','Kết chuyển CP sản xuất chung - CT:{$KMaCT} ','','".$key_matk."','" . abs(($ItemCPSXC_CTTheoTK['tienno']-$ItemCPSXC_CTTheoTK['tienco'])) . "','4','79','" . ($sophieu) . "'),";
                }                $sophieu += 2;
...
///------------------------ Tổng hợp công nợ-------------------------------------------------
    ...
    function themcdkt($stt, $matsnv, $tentsnv, $matk, $maso, $matsnvcha, $loaitsnv,$tentsnv_en)
    {
        $sql = "insert into sdkt(sott,matk,matsnv,tentsnv,maso,matsnvcha,loaitsnv,tentsnv_en) values('" . $stt . "','" . $matk . "','" . $matsnv . "','" . $tentsnv . "','" . $maso . "','" . $matsnvcha . "','" . $loaitsnv . "','".$tentsnv_en."')";
        $this->query($sql);
    }

    function suacdkt($stt, $matsnv, $tentsnv, $matk, $maso, $matsnvcha, $loaitsnv, $loaitk,$tentsnv_en)
    {
        $sql = "update sdkt set
									matk='" . $matk . "',
									matsnv='" . $matsnv . "',
