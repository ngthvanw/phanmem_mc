...
class ps_chitiet_mavattu extends database
{
    ...
    {// Lấy danh sách nhập vật tư trong tồn kho khi nhấn INSERT
        ...
        UNION ALL 
		SELECT 0 as sott,mavt.mavt,mavt.tenvt,mavt.dvt,mavt.giaban as donggianhap,sl as soluongnhap,rate as thuesuat,sl as thanhtienchuack,sl as thanhtien,dp as chietkhau,sl as thue,sl as tienchietkhau,sl as slcu,sl as dgcu,rate as thuesuatcu,dp as chietkhaucu,0 as phivc,0 as phibx,0 as tygiant,0 as nguyentent,0 as thanhtiennt,tmp_tkhientai.slton as soluongton FROM mavt LEFT JOIN tmp_tkhientai on (mavt.mavt = tmp_tkhientai.mavt) WHERE 0=0 ".str_replace("mavt","mavt.mavt",$sql_w)." order by tenvt" . $this->getLimit();
        $this->query($sql);
        $i = 0;
        //$dataton = $this->LoadDanhSachTKHT();
        $datagiavon = $this->LoadDanhSachGiaVon($thang);
    ...
    function loadListDanhSachTKTheoThangChiTietCuoiKyTheoNhom($InSoAm,$loctheotk)
    {// Xuất tòn kho cuối kỳ theo từng tháng
        ...
        while ($datag = $this->re_fetch($queryg)) {
            ...
            if ($fill != "")
                $sql_w = " and " . $this->get_orderby();
            if ($InSoAm == 1)
                $sql = "select * from tkthang where thang=" . $this->getThang() . " and manhom=" . $manhom . " and ROUND(soluongtonck,1)!=0 {$sqlwtk} order by tenvt";
            if ($InSoAm == 2)
                $sql = "select * from tkthang where thang=" . $this->getThang() . " and manhom=" . $manhom . " and ROUND(soluongtonck,1) >0 {$sqlwtk} order by tenvt";
            if ($InSoAm == 3)
                $sql = "select * from tkthang where thang=" . $this->getThang() . " and manhom=" . $manhom . " and ROUND(soluongtonck,1) < 0 {$sqlwtk} order by tenvt";
            if ($InSoAm == 4)
                $sql = "select * from tkthang where thang=" . $this->getThang() . " and manhom=" . $manhom . " {$sqlwtk} order by tenvt ";
            $this->query($sql);
            $i = 0;
            while ($data = $this->fetch()) {
                $mavt = $data['mavt'];
    ...
    {// Tính ton kho tháng lấy từ file tk và ghi vào file tk_thang
        ...
        while ($data = $this->fetch()) {
            ...
            $data['loaict'] = 4;
			//$array_am = array();
            if ($soluongtonck < 0 && in_array ($data['mavt'],$array_am)) {
				$array_am[$data['mavt']]=$data['mavt'];
                echo $thongbaoam = $data['mavt'] . " - " . $data['tenvt'] . " - (" . $soluongtonck . " " . $data['dvt'] . ") \n ";
            }
    ...
    function loadListDanhSachTKTheoThangChiTietCuoiKyTheoNhom_CongDon($InSoAm,$tuthang,$denthang,$loctheotk)
    {// Xuất tòn kho cuối kỳ theo từng tháng
        $trees = array();
		...
		if($loctheotk!="ALL"){
            ...
            if ($fill != "")
                $sql_w = " and " . $this->get_orderby();
            if ($InSoAm == 1)
                $sql = "select * from tkthang where thang=" . $this->getThang() . " and ROUND(soluongtonck,1)!=0 {$sqlwtk} order by tenvt";
            if ($InSoAm == 2)
                $sql = "select * from tkthang where thang=" . $this->getThang() . " and ROUND(soluongtonck,1) >0 {$sqlwtk} order by tenvt";
            if ($InSoAm == 3)
                $sql = "select * from tkthang where thang=" . $this->getThang() . " and ROUND(soluongtonck,1) < 0 {$sqlwtk} order by tenvt";
            if ($InSoAm == 4)
                $sql = "select * from tkthang where thang=" . $this->getThang() . " {$sqlwtk} order by tenvt ";
            $this->query($sql);
            $i = 0;
            while ($data = $this->fetch()) {
                $mavt = $data['mavt'];
    ...
    {
        $sql_w = $this->get_orderby();
        $sql = "select sum(tongtiengiavon) as tongtiengiavon,matk from (select sum(gtvnd1) as tongtiengiavon,tkno1 as matk from chitiet_pskt where MONTH(ngayhoadon) ='{$thang}' and loaiphieu='65' GROUP BY tkno1 UNION ALL select sum(sotien) as tongtiengiavon,dinhkhoan_psvt.tkco as matk from psvt INNER JOIN dinhkhoan_psvt on (psvt.sophieu = dinhkhoan_psvt.sophieu) WHERE month(ngayghiso)='{$thang}' and loaiphieu='3' GROUP BY dinhkhoan_psvt.tkco) t GROUP BY matk";
        $this->query($sql);
        while ($data = $this->fetch()) {
            $row[$data['matk']] = $data;
        }
    ...
    {// Tính phát sinh nhập trong khoản thời gian nhất định
        $sql = "SELECT t1.sophieu,t3.matk,t1.mavt,t1.soluongnhap,t1.sott FROM chitiet_psvt t1 inner join psvt t2 on (t1.sophieu=t2.sophieu) INNER join mavt t3 on(t1.mavt=t3.mavt) INNER JOIN masp t4 on (t1.mavt = t4.masp) where t2.loaiphieu=1";// Tồn đầu kỳ
        $query = $this->re_query($sql);
        while ($data = $this->re_fetch($query)) {
            $row[$data['sophieu']][] = $data;
        }
