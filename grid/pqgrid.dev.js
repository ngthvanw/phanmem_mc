...
(function($) {
	...
	fnPG._refreshPage = function() {
		...
		for (var i = 0, len = arr.length; i < len; i++) {
			var ele = arr[i];
			if (ele == "{0}") {
				str.push("<input autocomplete='off' type='text' tabindex='0' class='ui-corner-all' />")
			} else {
				if (ele == "{1}") {
		...
		if (typeof edtype == "function") {
			...
			if (edtype == "checkbox") {
				...
				if (edtype == "textarea" || edtype == "select" || edtype == "textbox") {
					if (edtype == "textarea") {
						inp = "<textarea autocomplete='off' class='" + cls2 + "' " + attr + " " + style + " name='" + dataIndx + "' ></textarea>"
					} else {
						if (edtype == "select") {
							...
							inp = "<input autocomplete='off' class='" + cls2 + "' " + attr + " " + style + " type=text name='" + dataIndx + "' />"
						}
					}
...
(function($) {
					...
					if (type === "textarea") {
						...
						if (type === "select") {
							...
							if (type == "checkbox") {
								...
								strS = ["<input autocomplete='off' ", checked, " name='", dataIndx, "' type=checkbox class='" + cls + "' style='" + style + "' " + attr + "/>"].join("")
							} else {
	...
	_pHeaderSearch._input = function(dataIndx, value, wd, cls, style, attr) {
		return ['<input autocomplete="off" value="', value, "\" name='", dataIndx, "' type=text style='width:", wd, "px;" + style + "' class='" + cls + "' " + attr + " />"].join("")
	};
	...
	fnTB._create = function() {
		...
		for (var i = 0, len = items.length; i < len; i++) {
			var item = items[i],
				...
				$ctrl;
			if (type == "textbox") {
				$ctrl = $("<input autocomplete='off' type='text' class='" + cls + "' " + attr + " " + style + ">").appendTo(element)
			} else {
				if (type == "checkbox") {
