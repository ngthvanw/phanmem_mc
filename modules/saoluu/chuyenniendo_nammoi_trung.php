...
function backup_Database($array_thanhphanchuyen,$chuyentatcatonkho)
{
    ...
    mysqli_query($con,"SET FOREIGN_KEY_CHECKS=0;");
        ...
        if (in_array("tonkho", $array_thanhphanchuyen)) {
		   ...
		   if($_SESSION['phuongphaptonkho']==2){// Liên hoàn
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                $dem++;
                $sql_val.="('{$dem}','{$row['mavt']}','".addslashes($row['tenvt'])."','{$row['matk']}','{$row['dvt']}','{$row['dongiabinhquan']}','{$row['thanhtientonck']}','{$row['soluongtonck']}','{$row['thuesuat']}','{$row['makho']}','{$row['tenkho']}','{$row['quycach']}','{$row['manhom']}','{$row['tennhom']}'),";
            ...
            if($chuyentatcatonkho=='false') {
                 $result = mysqli_query($con,"INSERT INTO mavt(sott,mavt,tenvt,dvt,manhom,matk,rate,tenkd,loaivl,niendo,tkdoanhthu,giaban,heso) SELECT 0,mavt,tenvt,dvt,manhom,matk,rate,tenkd,loaivl,{$namtruoc},tkdoanhthu,giaban,heso FROM {$database_namtruoc}.mavt WHERE mavt in (SELECT mavt FROM {$database_namtruoc}.tkthang where soluongtonck!=0 and thang= 12) and  mavt NOT in (select mavt from {$database_hientai}.mavt )") or die(mysqli_error($con)."mavt");// Lấy dữ liệu của bản cần sao lưu
				 mysqli_query($con,"update {$database_hientai}.mavt set sott=stt where sott=0");
			}else{
                 $result = mysqli_query($con,"INSERT INTO mavt(sott,mavt,tenvt,dvt,manhom,matk,rate,tenkd,loaivl,niendo,tkdoanhthu,giaban,heso) SELECT 0,mavt,tenvt,dvt,manhom,matk,rate,tenkd,loaivl,{$namtruoc},tkdoanhthu,giaban,heso FROM {$database_namtruoc}.mavt WHERE mavt NOT in (select mavt from {$database_hientai}.mavt)") or die(mysqli_error($con)."MaVT");// Lấy dữ liệu của bản cần sao lưu
                 mysqli_query($con,"update {$database_hientai}.mavt set sott=stt where sott=0");
			}
        ...
        if (in_array("congno", $array_thanhphanchuyen)) {
			...
			///Chép MAKH Từ năm cũ qua năm mới
			mysqli_query($con,"SET FOREIGN_KEY_CHECKS=0;");// Vô hiệu hoá khoá ngoại MaKH
			mysqli_query($con,"ALTER TABLE makh DROP PRIMARY KEY");// Vô hiệu hoá khoá chính MaKH
            $sql_INS_MAKH = "INSERT INTO makh(sott,makh,masothue,tenkh,makhcha,dienthoai,diachi,tenkd,loaitien,manhom,niendo) 
							SELECT 0,makh,masothue,tenkh,makhcha,dienthoai,diachi,tenkd,loaitien,manhom,{$namtruoc} FROM {$database_namtruoc}.makh 
							WHERE makh NOT in (select makh from {$database_hientai}.makh) ";
			mysqli_query($con,$sql_INS_MAKH);
			
			$sql_INS_MANHOMKH = "INSERT INTO manhomkh(manhom,tennhom,ghichu) 
							SELECT manhom,tennhom,ghichu FROM {$database_namtruoc}.manhomkh 
							WHERE manhom NOT in (select manhom from {$database_hientai}.manhomkh) ";
			mysqli_query($con,$sql_INS_MANHOMKH);
			
			// Cập nhập sott nếu số thứ tự bằng 0
			mysqli_query($con,"update {$database_hientai}.makh set sott=stt where sott=0");
			mysqli_query($con,"ALTER TABLE makh ADD PRIMARY KEY(`sott`);");
			mysqli_query($con,"SET FOREIGN_KEY_CHECKS=1;");
        }
        ...
        if (in_array("taisan", $array_thanhphanchuyen)) {
			...
			mysqli_query($con,"UPDATE {$database_hientai}.mats A JOIN {$database_namtruoc}.mats B
									...
									A.ngaysd = B.ngaysd,
									A.khauhao = B.khauhao,
									A.cpkhongduoctru = B.cpkhongduoctru") or die(mysqli_error($con));// Lấy dữ liệu của bản cần sao lưu
            mysqli_query($con,"SET FOREIGN_KEY_CHECKS=1;");
    ...
    if (in_array("tokhaithuetndn", $array_thanhphanchuyen)) {		
        $sqlstr_tndn = "UPDATE {$database_namtruoc}.tokhaitndn AS TNDN_NT, tokhaitndn AS TNDN_NN SET TNDN_NN.sotiendk = TNDN_NT.sotien WHERE TNDN_NN.machitieu = TNDN_NT.machitieu AND TNDN_NN.loaitokhai = TNDN_NT.loaitokhai";
        $sqlstr_tndn_tt80 = "UPDATE {$database_namtruoc}.tokhaitndn_tt80 AS TNDN_NT, tokhaitndn_tt80 AS TNDN_NN SET TNDN_NN.sotiendk = TNDN_NT.sotien WHERE TNDN_NN.machitieu = TNDN_NT.machitieu AND TNDN_NN.loaitokhai = TNDN_NT.loaitokhai";
        $sqlstr_kqkd_tt80 = "UPDATE {$database_namtruoc}.tmp_kqhdkd AS KQKD_NT, tmp_kqhdkd AS KQKD_NN SET KQKD_NN.sotiendk = KQKD_NT.sotien WHERE KQKD_NN.machitieu = KQKD_NT.machitieu AND KQKD_NN.loaitokhai = 'V'";
        mysqli_query($con,$sqlstr_tndn);
        mysqli_query($con,$sqlstr_tndn_tt80);
        mysqli_query($con,$sqlstr_kqkd_tt80);
    }
