...
if(isset($_GET['pq_filter'])){
    if($_SESSION['Level']=="1"){
        ...
        $OBJ->set_orderby(" (nguoigui='".$_SESSION['User']."' ) and ".$filterQuery);
    }
    $result = $OBJ->loadListDanhSachThuQuanLy($noiluu_phanmem);
}else{
    if($_SESSION['Level']=="1"){
        $OBJ->set_orderby("  1=1  ".$filterQuery);
    }else{
        $OBJ->set_orderby(" (nguoigui='".$_SESSION['User']."' ) ".$filterQuery);
    }
     $result = $OBJ->loadListDanhSachThuQuanLy($noiluu_phanmem);
}
