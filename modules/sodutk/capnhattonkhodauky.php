...
if (!$db_selected) {
    ...
    if ($count < 1) {
        ...
        while ($row = mysql_fetch_assoc($query)) {
            ...
            $val .= "('" . $row['mavt'] . "','" . $row['tenvt'] . "','" . $row['matk'] . "','" . $row['dvt'] . "','" . $row['soluong'] . "','" . $row['dongia'] . "','" . $row['thanhtien'] . "','" . $row['thuesuat'] . "','" . $row['chietkhau'] . "','" . $row['makho'] . "','" . $row['tenkho'] . "','" . $row['quycach'] . "','" . $row['manhom'] . "','" . $row['tennhom'] . "')" . $dauphay;
        }
        ...
        mysql_query($sql_emp);
        mysql_query($sql_emp2);
        $sql_in = "insert into tk(mavt,tenvt,matk,dvt,slck,gtvnck,dgxvnd,rate,chietkhau,makho,kho,quycach,manhom,tennhom) VALUE " . $val;
        mysql_query($sql_in);
        mysql_close($cnn);
        echo 1;
    }
