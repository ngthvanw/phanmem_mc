...
$re_result = $OBJ->re_query("select tkno as tk from dinhkhoan_psvt where tkno in ($String_tkno_KH) 
                                UNION ALL
...
$re_data = $OBJ->re_fetch_all($re_result);

foreach ($re_data as $itemtkno){
    $arr_tkno_KH[$itemtkno['tk']] = $itemtkno['tk'];
}
$OBJ->re_query("DELETE FROM tmp_dskhcn WHERE NOT EXISTS (select makh from makh WHERE tmp_dskhcn.makh  = makh.makh )");
$string_sql="";
...
/*
echo "<table border='1'>";
foreach($array_co as $itemCT_NoKH){
	if($itemCT_NoKH['tk']=='131'){
		echo "<tr>";
		echo "<td>{$itemCT_NoKH['makh']}</td>";
		echo "<td>{$itemCT_NoKH['psco']}</td>";
		echo "</tr>";
	}
}
echo "</table>";
...
foreach ($dataBCDTK as $itemCT){
    ...
    if($soduntcktinh>=0){
		...
		if($soduno==0 && $soduco==0 && $sodunops==0 && $soducops==0)
		{
			...
			$value1.= "('".$itemCT['makh']."','".addslashes($itemCT['tenkh'])."','".$itemCT['matk']."','".$soduno."','".$soduco."','".$sodunops."','".$soducops."','".$sodunock."','".$soducock."','".$itemCT['makhcha']."','".$sodunock_."','".$soducock_."','".$sodunont."','".$soducont."','".$sodunontps."','".$soducontps."','".$sodunontck."','".$soducontck."','".$sodunontck_."','".$soducontck_."','".$itemCT['loaitien']."','".$itemCT['manhom']."','{$itemCT['makh']}_{$itemCT['matk']}','{$phanloai}'),";
		}
    }
$OBJ->re_query("delete from cnkh");
$OBJ->re_query("ALTER TABLE cnkh AUTO_INCREMENT=1;");
$sql_ins1 = "insert into cnkh(makh,tenkh,matk,nodk,codk,nops,cops,nock,cock,makhcha,nock_,cock_,nontdk,contdk,nontps,contps,nontck,contck,nontck_,contck_,loaitien,manhom,makh_matk,phanloai) VALUE ".substr($value1, 0,-1);
