...
$danhsachbangcandoitk = $OBJCT->load_danhsach_bangcdkt();// lấy số đầu kỳ trong bản cdtk
...
foreach ($dataLCTT as $itemLCTT){
    ...
    if(substr($itemLCTT['tkno'],0,3)=='111'){
        $dataTK = $OBJCT->load_danhsach_socai_theotk_no_co($str_matk_tkno, "ALL", $str_matk_tkco);// Lấy chinh tiết sổ cái
    }else{
        $dataTK = $OBJCT->load_danhsach_socai_theotk_no_co($str_matk_tkco, "ALL", $str_matk_tkno);// Lấy chinh tiết sổ cái
    }
    $tongno = 0;
    $tongco = 0;
...
$tongtien60 = $danhsachbangcandoitk['110']['sodudk'];


$SOCUOIKY = $danhsachbangcandoitk['110']['soduck'];
...
$OBJCT->re_query(" update luuchuyentiente set soduck=" . $tongtien2 . " where maso='2'");
$OBJCT->re_query(" update luuchuyentiente set soduck=" . $tongtien20 . " where maso='20'");
$OBJCT->re_query(" update luuchuyentiente set soduck=" . $tongtien30 . " where maso='30'");
$OBJCT->re_query(" update luuchuyentiente set soduck=" . $tongtien40 . " where maso='40'");
$OBJCT->re_query(" update luuchuyentiente set soduck=" . $tongtien50 . " where maso='50'");
$OBJCT->re_query(" update luuchuyentiente set soduck=" . $tongtien60 . " where maso='60'");
$OBJCT->re_query(" update luuchuyentiente set soduck=" . $tongtien70 . " where maso='70'");
