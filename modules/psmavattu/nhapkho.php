...
$OBJ->re_query("ALTER TABLE `psvt` ADD `chietkhaudoanhso` BIGINT NOT NULL;");
$OBJ->re_query("ALTER TABLE `psvt` ADD `chiphikhongloaitru` INT NOT NULL;");
$OBJ->re_query("ALTER TABLE `psvt` ADD `loaitokhai` INT NOT NULL DEFAULT '1';");
$OBJ->re_query("ALTER TABLE `psvt` ADD `capnhat_chungtugoc` DATETIME NOT NULL;");
$OBJ->re_query("ALTER TABLE `psvt` ADD `capnhat_chiphikhongloaitru` DATETIME NOT NULL;");
...
$OBJ->re_query("ALTER TABLE `psvt` ADD `tencanhan` CHAR(200) NOT NULL;");
$OBJ->re_query("ALTER TABLE `psvt` ADD `lacongtrinh` INT(1) NOT NULL;");
...
foreach ($array_dinhkhoan as $item){
    if($item['tkno']!=""){
        $OBJ->re_query("insert into dinhkhoan_psvt(sophieu,tkno,tkco,sotien,sotiennt) VALUE ('".$sophieu."','".$item['tkno']."','".$item['tkco']."','".$item['sotien']."','".$item['sotiennt']."')");
    }
}
...
if($_GET['chungtugoc']==0){
    $OBJ->re_query("update psvt set capnhat_chungtugoc = '".date("Y-m-d h:i:s")."' where sophieu='".$sophieu."'");
}
if($_GET['chiphikhongloaitru']==1){
    $OBJ->re_query("update psvt set capnhat_chiphikhongloaitru = '".date("Y-m-d h:i:s")."' where sophieu='".$sophieu."'");
}
?>