...
$tongtientru = 0;
    ...
    if ($ngayhoadon >= $ngayGioiHan) {
        if($tamtinhtien>=5000000 && $tk==1111){
            if ($tontai == FALSE) {
                echo "Tổng tiền số phiếu này đã vượt quá 5.000.000 VNĐ . Bạn có muốn tiếp tục ?";
            } else {
                echo "Tổng tiền số phiếu " . substr($sophieu, 0, -1) . " của hóa đơn " . substr($sohoadon_str, 0, -1) . " đã vượt quá 5.000.000 VNĐ. Vui lòng kiểm tra lại [HÌNH THỨC THANH TOÁN] của phiều này. Nếu chính xác bạn có có thể tiếp tục ?";
            }
        }else{
            echo "K";
        }
