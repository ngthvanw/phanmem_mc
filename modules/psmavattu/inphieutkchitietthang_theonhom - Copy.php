...
class PDF extends tFPDF
{
    ...
    function TieuDe($Ngay, $TKCo, $SoPhieu, $SoTT, $LoaiPhieu, $TenPhieu, $tk1, $tk2)
    {
        ...
        $this->SetFont('timesi', '', 10);
        $this->Cell(65, 4, 'Ban hành '.$_SESSION['THONGTUMANG'][$_SESSION['theothongtu']]['thongtu'], 0, 0, R);
        ...
        $this->SetFont('timesi', '', 10);
        $this->Cell(65, 4,  'Ngày '.$_SESSION['THONGTUMANG'][$_SESSION['theothongtu']]['ngay'].' của Bộ Tài Chính', 0, 0, R);
    ...
    {
        $w = array(0, 7, 22, 73, 14, 17, 26, 17, 26, 17, 26, 17, 26);
        ...
        $tongcong = 0;
        $demall = 0;
        foreach ($_SESSION["LISTTKTHANG"] as $key => $tiemG) {
            $tongsldkg = 0;
            $tongthanhtiendkg = 0;
            $tongslckg = 0;
            $tongthanhtienckg = 0;
            ...
            foreach ($tiemG as $tiemCT) {
                ...
                $this->Cell($w[3], 5, mb_substr($tiemCT['tenvt'], 0,55,'UTF-8'), 'LR', 0, 'L');// LTRB là border c?a cell
                $this->Cell($w[4], 5, $tiemCT['dvt'], 'LR', 0, 'C');

                $this->Cell($w[5], 5, ($tiemCT['soluongtondk'] == 0) ? "":number_format($tiemCT['soluongtondk'],2,",","."), 'LR', 0, 'R');// Tồn đầu
                $this->Cell($w[6], 5, ($tiemCT['thanhtientondk'] == 0) ? "":number_format($tiemCT['thanhtientondk'],0,",","."), 'LR', 0, 'R');

                $this->Cell($w[7], 5, ($tiemCT['soluongnhap'] == 0) ? "":number_format($tiemCT['soluongnhap'],2,",","."), 'LR', 0, 'R');// Nhập trong kỳ
                $this->Cell($w[8], 5, ($tiemCT['thanhtiennhap'] == 0) ? "":number_format($tiemCT['thanhtiennhap'],0,",","."), 'LR', 0, 'R');

                $this->Cell($w[9], 5, ($tiemCT['soluongxuat'] == 0) ? "":number_format($tiemCT['soluongxuat'],2,",","."), 'LR', 0, 'R');// Xuất trong kỳ
                $this->Cell($w[10], 5, ($tiemCT['thanhtienxuat'] == 0) ? "":number_format($tiemCT['thanhtienxuat'],0,",","."), 'LR', 0, 'R');

                $this->Cell($w[11], 5, ($tiemCT['soluongtonck'] == 0) ? "":number_format($tiemCT['soluongtonck'],2,",","."), 'LR', 0, 'R');
                $this->Cell($w[12], 5, ($tiemCT['thanhtientonck'] == 0) ? "":number_format($tiemCT['thanhtientonck'],0,",","."), 'LR', 0, 'R');
                ...
                $this->Cell(array_sum($w), 5, '__ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _', '0', 0, 'R');
            ...
            $this->Cell($w[3], 5, $tiemCT['tennhom'], 'LR', 0, 'R');// LTRB là border c?a cell
            $this->Cell($w[4], 5, "", 'LR', 0, 'C');

            $this->Cell($w[5], 5, number_format($tongsldkg,2,",","."), 'LR', 0, 'R');// Tồn đầu
            $this->Cell($w[6], 5, number_format($tongthanhtiendkg,0,",","."), 'LR', 0, 'R');

            $this->Cell($w[7], 5, number_format($tongslnhapg,2,",","."), 'LR', 0, 'R');// Nhập trong kỳ
            $this->Cell($w[8], 5, number_format($tongthanhtiennhapg,0,",","."), 'LR', 0, 'R');

            $this->Cell($w[9], 5, number_format($tongslxuatg,2,",","."), 'LR', 0, 'R');// Xuất trong kỳ
            $this->Cell($w[10], 5, number_format($tongthanhtienxuatg,0,",","."), 'LR', 0, 'R');

            $this->Cell($w[11], 5, number_format($tongslckg,2,",","."), 'LR', 0, 'R');
            $this->Cell($w[12], 5, number_format($tongthanhtienckg,0,",","."), 'LR', 0, 'R');
            $this->Ln(0);

            $this->Cell(array_sum($w), 5, '__ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _', '0', 0, 'R');
        ...
        $this->Cell($w[6], 5, number_format($tongthanhtiendk,0,",","."), 'LTRB', 0, 'R');

        $this->Cell($w[7], 5, "", 'LTRB', 0, 'R');
        $this->Cell($w[8], 5, number_format($tongthanhtiennhapps,0,",","."), 'LTRB', 0, 'R');

        $this->Cell($w[9], 5, "", 'LTRB', 0, 'R');
        $this->Cell($w[10], 5, number_format($tongthanhtienxuatps,0,",","."), 'LTRB', 0, 'R');

        $this->Cell($w[11], 5, "", 'LTRB', 0, 'R');
        $this->Cell($w[12], 5, number_format($tongthanhtienck,0,",","."), 'LTRB', 0, 'R');
