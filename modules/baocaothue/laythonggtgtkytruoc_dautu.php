...
function MangThueThangTruoc($NgayBD,$NgayKT){    $array_quy =
        array(1 => "I",
            2 => "I",
            3 =>"I",
            4 => "II",
    ...
    if ($array_thangtruoc[$thangtinhthue."-".$namtinhthue] === 0) {// Lấy số dư đầu kỳ, không lấy từ số dư tài khoản đầu kỳ
        ...
        $data1 = $OBJCT->load_danhsach_tokhai_dautu('-1',1);// thông tin tồn đầu kỳ
       if($data1==0){
           $data = $OBJCT->load_danhsach_tokhai_dautu($thangtinhthue."-".$namtinhthue, $loaitokhai);
           $matkhai = "VI42";
           $ThueKyTruoc = $data['B']['thue'];
           $data[$matkhai]['thue'] = $ThueKyTruoc;
        ...
        $data = $OBJCT->load_danhsach_tokhai_dautu($array_thangtruoc[$thangtinhthue."-".$namtinhthue], $loaitokhai);// thông tin tồn đầu kỳ
    }
