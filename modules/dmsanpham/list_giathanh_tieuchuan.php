...
{
    $search  = array('masp', 'mavt', 'tensp');
    $replace = array('b1.masp', 'mavt', 'b2.tenkd');

    $pq_filter = ($_GET["pq_filter"]);
    $dsf = FilterHelper::deSerializeFilter($pq_filter);
    $string = str_replace($search, $replace,$dsf->query);
    $filterQuery = khu_dau_vn(($string));
}

$OBJ->re_query("update banggiathanhtieuchuan set dinhmuc='1'
                  WHERE (mavt = 'SXC-01' OR mavt = 'SXC-02') AND dinhmuc=0 ");

$OBJ->set_orderby($filterQuery);

$result = $OBJ->loadListGiaThanhTieuChuan();//
