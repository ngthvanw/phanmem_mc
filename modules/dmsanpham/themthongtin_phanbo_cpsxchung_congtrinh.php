...
$sql_emp_pskt = "DELETE chitiet_pskt, pskt 
                        ...
                        WHERE pskt.loaiphieu = '77' AND pskt.btps = '2' and ngayghiso>='".$tungay."' and ngayghiso<='".$denngay."';";
$OBJCT->re_query($sql_emp_pskt);

$sql_emp_pskt = "DELETE chitiet_pskt, pskt 
                        FROM pskt 
                        INNER JOIN chitiet_pskt ON pskt.sophieu = chitiet_pskt.sophieu 
                        WHERE pskt.loaiphieu = '0' and ngayghiso>='".$tungay."' and ngayghiso<='".$denngay."';";
$OBJCT->re_query($sql_emp_pskt);

$sql_emp_pskt = "DELETE chitiet_pskt, pskt 
                        FROM pskt 
                        INNER JOIN chitiet_pskt ON pskt.sophieu = chitiet_pskt.sophieu 
                        WHERE pskt.loaiphieu = '77' AND pskt.btps = '77' and ngayghiso>='".$tungay."' and ngayghiso<='".$denngay."';";
$OBJCT->re_query($sql_emp_pskt);
...
foreach ($TinhCPChuaPhanCap as $kmact => $itemChuaPhanCap) {
    if ($itemChuaPhanCap['loaisp'] == "CT") {// Mới bổ sung ngày 08-10-2024
        $LoaiPhieu = 77;
        $value_pskt .= "('" . ($sophieu) . "',1,'" . $denngay . "','154','{$LoaiPhieu}','" . abs($itemChuaPhanCap['sotiennc622pb']) . "','2'),";
        $value_chitiet_pskt .= "(1,'" . $denngay . "','" . abs($itemChuaPhanCap['sotiennc622pb']) . "','','" . $kmact . "','" . $itemChuaPhanCap['tenct'] . "','100094','Phân bổ chi phí nhân công, công trình:{$kmact}','','','622','','" . abs($itemChuaPhanCap['sotiennc622pb']) . "','4','{$LoaiPhieu}','" . ($sophieu) . "','2','" . $itemChuaPhanCap['loaisp'] . "'),";
...
$OBJCT->re_query("delete from bangphanbo_chiphi_sxchung where loaipb = 'CT'");
