...
$OBJ->re_query("CREATE TABLE chitiet_dinhmuc_hd AS (SELECT * FROM chitiet_dinhmuc_sp WHERE 1=2);");
$OBJ->re_query("ALTER TABLE `chitiet_dinhmuc_hd` CHANGE `sott` `sott` INT(11) NOT NULL AUTO_INCREMENT;");
$OBJ->re_query("ALTER TABLE `masp` CHANGE `quyettoan` `quyettoan` BIGINT NOT NULL;");

if ($list == 0) {
    if($loaidinhmucsp=="ALL")
        $OBJ->set_orderby(" ct.masp='" . $masp . "' ");
    else
        $OBJ->set_orderby(" ct.masp='" . $masp . "' and loaivl='".$loaidinhmucsp."' ");

    $result = $OBJ->loadListMaVT_HongDong();
} else {
    $OBJ->set_orderby($filterQuery);
    $result = $OBJ->loadListMaVTHDUnion($masp,$loaidinhmucsp);
}
