...
if ( isset($_GET["pq_filter"]))
{
    ...
    $search  = array('makho', 'manhom','tenvt','mavt','thaythe','dvt');
    $replace = array('makho.makho', 'manhom.manhom','thaythe.tenkd','mavt.mavt','mavt','mavt.dvt');
    $string= (str_replace($search,$replace,$dsf->query));
    $filterQuery = $string;
}
$dataListKho=$OBJKHO->loadListMaKho_CoKeyLaMa();
foreach ($dataListKho as $kKho=>$itemMaKho){
    $OBJ->themDSTKMaVT($kKho);
}
$OBJ->set_orderby($filterQuery);  
$result = $OBJ->loadListMaVT_TMP_DK();
