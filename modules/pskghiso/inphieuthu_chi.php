...
class PDF extends tFPDF
{
    ...
    function TieuDe($Ngay,$TKCo,$SoPhieu,$SoTT)
    {
        ...
        $this->Cell(50,4,'PHIẾU HẠCH TOÁN',0,0,C);
        ...
        $this->SetFont('timesi','',12);
        $this->Cell(70,4,'',0,0,R);
        $this->Cell(50,4,'Ngày '.$Ngay,0,0,C);
        $this->SetFont('times','',9);
        ...
        $this->SetFont('timesi','',12);
        $this->Cell(70,4,'',0,0,R);
        $this->Cell(50,4,'Số :'.$SoPhieu,0,0,C);
        $this->SetFont('times','',9);
        $this->Cell(85,3,'',0,0,R);
    ...
    function ImprovedTable($data)
    {
        ...
        foreach($data as $k=>$row)
        {
            ...
            $this->SetLineWidth(.1);
            $this->Cell(65,5,"-".$row['tennoidung'],'LR');// LTRB là border c?a cell
            $this->Cell(20,5,$row['tkco'],'R',0,'C');
            $this->Cell(20,5,($row['tkno']),'R',0,'C');
            $this->Cell(30,5,number_format(($row['gtvnd'])),'R',0,'R');
            ...
            $tongtien+=$row['gtvnd'];
            if($row['mand2']!=""){
                $this->Cell(65,5,"-".$row['tennoidung2'],'LR');// LTRB là border c?a cell
                $this->Cell(20,5,$row['tkco'],'R',0,'C');
                $this->Cell(20,5,($row['tkno_mand2']),'R',0,'C');
                $this->Cell(30,5,(number_format($row['gtvnd2'])),'R',0,'R');
                $this->Cell(60,5,($row['chuthich']),'R',0,'C');
                $tongtien+=$row['gtvnd2'];
                $this->Ln();
                for($i=0;$i<=97;$i++){// Tạo line --- trong bảng
        ...
        $this->Cell(20,5,"",'LTRB',0,'C');
        $this->Cell(30,5,number_format($tongtien),'LTRB',0,'R');
        ...
        $this->Ln(3);
        $this->SetFont('timesi','',10);
        $this->Cell(97,3,'Kèm theo 1 chứng từ gốc',0,0,L);

        $this->SetFont('times','',10);
        $this->Ln(8);
        $this->Cell(97,3,'',0,0,C);

        $this->Cell(97,3,'Ngày '.date("d/m/Y"),0,0,C);
        $this->Ln(4);
...
$i=0;
foreach($_SESSION['PhieuGhiSo'] as $item_data){// Duyết data theo tuân
    $data = $item_data;
    $pdf->AddPage('P','A4');

    $pdf->TieuDe(dd_mm_yyy($item_data['date']),$data['tkco'],$data['mapskt'],$data['sott']);
    $pdf->ImprovedTable($_SESSION['ChiTietPhieuGhiSo']);
    $i++;
}
