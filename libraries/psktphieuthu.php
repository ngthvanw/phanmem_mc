...
class psktphieuthu extends database
{
    public $tringorder;
    public $table = "pskt";
    ...
    function loadListPSKT()
    {
        ...
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
        $sql = "SELECT pskt.*,chitiet_pskt.maloai,chitiet_pskt.ngayhoadon,chitiet_pskt.sct,chitiet_pskt.seri,gtvnd1,gtvnd2,chitiet_pskt.tongtien,chitiet_pskt.tkno1,chitiet_pskt.tkno2,chitiet_pskt.loaitokhai,chitiet_pskt.loaihddt,loaisp,mabp,bophan,ngaykhaithue,chungtuthamchieu,mand1,noidung1 from pskt INNER JOIN chitiet_pskt on (pskt.sophieu = chitiet_pskt.sophieu) WHERE 0=0 and pskt.loaiphieu='".$this->getLP()."' $sql_w  order by cast(SUBSTRING_INDEX(pskt.mapskt, '-',-1) as unsigned) DESC,ngayghiso DESC ".$this->getLimit();;
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
            $trees[] = $data;
