...
class phieukiemtra extends database
{
    ...
    function loadListDanhSachGiaoNhan($noiluu_phanmem)
    {
        ...
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
        $sql = "SELECT * FROM dulieuchung.danhsach_giaonhan_chungtu_{$noiluu_phanmem} WHERE 0=0 $sql_w  order by bosung DESC,sott DESC LIMIT 500";
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
            $i++;
    ...
    function loadListDanhSachThuQuanLy($noiluu_phanmem)
    {
        ...
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
        $sql = "SELECT * FROM dulieuchung.danhsach_thuquanly_{$noiluu_phanmem} WHERE 0=0 $sql_w  order by sott";
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
            $i++;
    ...
    function loadListDanhSachKeHoach($noiluu_phanmem)
    {
        ...
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
        $sql = "SELECT * FROM dulieuchung.danhsach_kehoach_{$noiluu_phanmem} WHERE 0=0 $sql_w  order by sott,nguoigui limit 200";
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
            $i++;
    ...
    public function suaPhieuNhatKy($sott,$truongnhomduyet=0,$giamdocduyet=0,$nguoiduyet="",$ngayduyet="",$giamdocduyet1=0,$nguoiduyet1="",$ngayduyet1="",$ghichu="")
    {
         $sql = "update nhatkykiemphieu set truongnhomduyet='" . $truongnhomduyet . "',giamdocduyet='" . $giamdocduyet . "',nguoiduyet='" . $nguoiduyet . "',ngayduyet='" . $ngayduyet . "',giamdocduyet1='" . $giamdocduyet1 . "',nguoiduyet1='" . $nguoiduyet1 . "',ngayduyet1='" . $ngayduyet1 . "',ghichu='" . $ghichu . "'
                  WHERE sott = '" . $sott . "'";
        $this->query($sql);
    }
