...
class mataisan extends database
{
    ...
    function loadListAllDSMaTaiSan_SanLuong($parentid = 0, $printto = 10)
    { // c?p 1 la cha
        $trees = array();
        $fill = $this->get_orderby();
        if ($fill != "")
            $sql_w = $this->get_orderby() . " and ";
        $sql = "SELECT * FROM sanluong_khauhao_taisan WHERE $sql_w matscha = '$parentid'  order by mats DESC ";
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
            $i++;
            $data['STT'] = $i;
            $trees[] = $data;
            $sql1 = "SELECT * FROM sanluong_khauhao_taisan WHERE $sql_w matscha ='" . $data['mats'] . "' order by mats ";
            $query = $this->re_query($sql1);
            while ($data1 = $this->re_fetch($query)) {
                if ($printto == 1) {
                    break;
                ...
                $data1['STT'] = $i;
                $trees[] = $data1;
                $sql2 = "SELECT * FROM sanluong_khauhao_taisan WHERE $sql_w matscha ='" . $data1['mats'] . "' order by mats ";
                $query1 = $this->re_query($sql2);
                while ($data2 = $this->re_fetch($query1)) {
                    if ($printto == 2) {
                        break;
    ...
    function loadListAllDSMaTaiSan_SanLuong_W($parentid = 0, $printto = 10)
    { // c?p 1 la cha
        $trees = array();
        $fill = $this->get_orderby();
        if ($fill != "")
            $sql_w = " and ".$this->get_orderby();
        $sql = "SELECT * FROM sanluong_khauhao_taisan WHERE 0=0 $sql_w  order by mats DESC ";
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
            $i++;
    ...
    function loadBangKHTaiSan_CaNam($tuthang,$denthang,$Loai){
	    ...
	    function loadBangKHTaiSanTangGiam_CaNam($tuthang,$denthang,$Loai){
        $trees = array();
        $fill = $this->get_orderby();
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
        if($Loai=="BK"){
            $sql = "SELECT sott,matk,tents,tylekh,max(daban) as daban,(select max(nguyengia) from bangkhtaisan t where t.mats=bangkhtaisan.mats GROUP BY mats ORDER by thang DESC LIMIT 1 ) as nguyengia,sum(sokh) as sokh,tkno,sum(tienno) as tienno ,tkco,matk,dvt,mabp,tenbp,(select gtconlai from bangkhtaisan t where t.mats=bangkhtaisan.mats ORDER by thang DESC LIMIT 1 ) as gtconlai,tgsudung,max(soluong) as soluong,mand from bangkhtaisan where thang>='" . $tuthang . "' and thang<='" . $denthang . "' GROUP by matk,mand HAVING max(daban)=0";
        }else {
            $sql = "SELECT sott,matk,tents,tylekh,max(daban) as daban,(select max(nguyengia) from bangkhtaisan t where t.mats=bangkhtaisan.mats GROUP BY mats ORDER by thang DESC LIMIT 1 ) as nguyengia,sum(sokh) as sokh,tkno,sum(tienno) as tienno ,tkco,matk,dvt,mabp,tenbp,(select gtconlai from bangkhtaisan t where t.mats=bangkhtaisan.mats ORDER by thang DESC LIMIT 1 ) as gtconlai,tgsudung,max(soluong) as soluong,mand from bangkhtaisan where thang>='" . $tuthang . "' and thang<='" . $denthang . "' GROUP by matk,mand";
        }
        $this->query($sql);
        ...
        while ($data = $this->fetch()) {
            $i++;
            $trees[$data['matk']][$data['mand']] = $data;
        }
        return $trees;
    }
	
	    function loadBangKHTaiSan_CuoiNam($tuthang,$denthang,$Loai){
        $trees = array();
        $fill = $this->get_orderby();
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
        if($Loai=="BK"){
            $sql = "SELECT sott,matk,tents,tylekh,max(daban) as daban,(select max(nguyengia) from bangkhtaisan t where t.mats=bangkhtaisan.mats GROUP BY mats ORDER by thang DESC LIMIT 1 ) as nguyengia,sum(sokh) as sokh,tkno,sum(tienno) as tienno ,tkco,matk,dvt,mabp,tenbp,(select gtconlai from bangkhtaisan t where t.mats=bangkhtaisan.mats ORDER by thang DESC LIMIT 1 ) as gtconlai,tgsudung,max(soluong) as soluong,mand from bangkhtaisan where thang>='" . $tuthang . "' and thang<='" . $denthang . "' GROUP by matk HAVING max(daban)=0";
        }else {
            $sql = "SELECT sott,matk,tents,tylekh,max(daban) as daban,(select max(nguyengia) from bangkhtaisan t where t.mats=bangkhtaisan.mats GROUP BY mats ORDER by thang DESC LIMIT 1 ) as nguyengia,sum(sokh) as sokh,tkno,sum(tienno) as tienno ,tkco,matk,dvt,mabp,tenbp,(select gtconlai from bangkhtaisan t where t.mats=bangkhtaisan.mats ORDER by thang DESC LIMIT 1 ) as gtconlai,tgsudung,max(soluong) as soluong from bangkhtaisan where thang>='" . $tuthang . "' and thang<='" . $denthang . "' GROUP by matk";
        }
        $this->query($sql);
    ...
    function loadSanLuong_KhauHao_TaiSan(){
        $trees = array();
        $sql = "SELECT * from sanluong_khauhao_taisan";
        $res = $this->re_query($sql);
        while ($data = $this->re_fetch($res)) {
            $trees[$data['mats']] = $data;
        }
        return $trees;
    }
    ...
    function thembangkhauhaotstheothang($tuthang, $denthang, $sophieu,$ngaycuoithang,$xoakhauhaodatrichtrongky,$khongtaobuttoandinhkhoan,$khauhaotheo,$khauhaotaisanhetkhauhao)
    {
        ...
        $data_SanLuong_KhauHao_TaiSan = $this->loadSanLuong_KhauHao_TaiSan();
        if ($tuthang == 1) {
        ...
        while ($data = $this->fetch()) {
            ...
            //Điều chỉnh tổng khấu hao tháng theo sản lượng sử dụng--------------------------------------------
            // Có bổ sung hàm $data_SanLuong_KhauHao_TaiSan = $this->loadSanLuong_KhauHao_TaiSan();
            $thang_SanLuong_KH_TS = "thang".$tuthang;
            if($data_SanLuong_KhauHao_TaiSan[$data['mats']]['tongcong']!=0){
                // Tính số tháng chênh lệch ngày sử dụng đến cuối năm
                $NgayCuoiNam = ngaycuoithang(12,$_SESSION['NienDo']);// Lấy ngày cuối của tháng 12
                $NgayCuoiNgaySuDung = ngaycuoithang($Month_SD,$Year_SD);// Lấy ngày cuối của tháng sử dụng
                $tienkhngay = $tienkhthang/30;// Tính số tiền khấu hao của 1 ngày
                $startDate = new DateTime($data['ngaysd']);// Ngày sử dụng tài sản
                $endDate = new DateTime($NgayCuoiNam);// Ngày cuối năm
                $endDate2 = new DateTime($NgayCuoiNgaySuDung);// Ngày cuối năm
                $interval = $startDate->diff($endDate);
                $years = $interval->y;
                $ThangDaKH = $interval->m + ($years * 12);
                $NgayDaKH = abs($startDate - $endDate2)+1;
                if($ThangDaKH>=12){
                    $ThangDaKH = 12;
                    $NgayDaKH = 0;
                }
                $TyLe_SanLuong_KhauHao = $data_SanLuong_KhauHao_TaiSan[$data['mats']][$thang_SanLuong_KH_TS]/$data_SanLuong_KhauHao_TaiSan[$data['mats']]['tongcong'];
                $TongKH_SanLuong_Thang = (($tongkhthang*$ThangDaKH)+($tienkhngay*$NgayDaKH))*$TyLe_SanLuong_KhauHao;
                $tongkhthang =  round($TongKH_SanLuong_Thang);
            }
            //End điều chỉnh tổng khấu hao tháng theo sản lượng sử dụng------------------------------------------
    ...
    function daukytaisan()
    {
	    ...
	    function daukytaisan_tanggiam()
    {
        $trees = array();
        $fill = $this->get_orderby();
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
        $sql = "SELECT matk,sum(nguyengia) as nguyengia,sum(muckhthang) as muckhthang,sum(muckhnam) as muckhnam,tylekh,sum(giatriconlai) as giatriconlai FROM psts where tanggiam=0 group by matk ";
        $result = $this->re_query($sql);
        while ($data = $this->re_fetch($result)) {
            $trees[$data['matk']] = $data;
        }
	    ...
	    function tangtaisan_tanggiam()
    {
        $trees = array();
        $fill = $this->get_orderby();
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
        $sql = "SELECT matk,sum(nguyengia) as nguyengia,sum(muckhthang) as muckhthang,sum(muckhnam) as muckhnam,tylekh,sum(giatriconlai) as giatriconlai,mand FROM psts where tanggiam=1 group by matk,mand ";
        $result = $this->re_query($sql);
        while ($data = $this->re_fetch($result)) {
            $trees[$data['matk']][$data['mand']] = $data;
        }
	    ...
	    function giamtaisan_tanggiam()
    {
        $trees = array();
        $fill = $this->get_orderby();
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
        $sql = "SELECT matk,sum(nguyengia) as nguyengia,sum(muckhthang) as muckhthang,sum(muckhnam) as muckhnam,tylekh,sum(giatriconlai) as giatriconlai,mand FROM psts where tanggiam=2 group by matk,mand ";
        $result = $this->re_query($sql);
        while ($data = $this->re_fetch($result)) {
            $trees[$data['matk']][$data['mand']] = $data;
        }
