...
class mavattu extends database
{
    ...
    {
        $sql = "select max(stt) as sott from mavt";
        $this->query($sql);
        if ($this->num_rows() == 1) {
            $data = $this->fetch();
            return $data['sott'] + 1;
    ...
    function loadListMaVT_NhapHoaDon($sophieu)
    {
        $trees = array();
        $fill = $this->get_orderby();
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
        $sql = "SELECT nhaphoadon.*,manhom.tennhom FROM nhaphoadon inner join manhom on (nhaphoadon.manhom = manhom.manhom) WHERE sophieu='".$sophieu."' $sql_w  order by nhaphoadon.sott ";
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
            $trees[] = $data;
    ...
    function loadListMaVT_ChiTiet_xksx($sottpsct)
    {
        $res = $this->re_query("select mavt_pbchiphi from chitiet_psvt where sophieu='{$sottpsct}' limit 1");
        $data_chitiet = $this->re_fetch($res);
        $mavt_arr = explode(",", $data_chitiet['mavt_pbchiphi']);
    ...
    function loadListMaVT_DuThua()
    {
        ...
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
        $sql = "SELECT mavt.mavt,mavt.tenvt,sum(tk.slck) as soluongdk,sum(chitiet_psvt.soluongnhap) as soluongps FROM mavt left join tk on (mavt.mavt = tk.mavt) left join chitiet_psvt on (mavt.mavt = chitiet_psvt.mavt) WHERE 0=0 $sql_w GROUP BY mavt.mavt HAVING soluongdk is null and soluongps is null  order by mavt.tenkd " . $this->getLimit();
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
            $i++;
    ...
    public function suaMaVT_ChiTietNhapHoaDon()
    {
        $sql_ps = "update nhaphoadon set tenvt = '" . $this->get_TenVT() . "',dvt='" . $this->get_DVT() . "',tenkd='" . $this->get_TenKD() . "',mavt = '" . $this->get_MaVT() . "',manhom = '" . $this->get_MaNhom() . "',matk = '" . $this->get_MaTK() . "',tkdoanhthu = '" . $this->getTKDoanhThu() . "',thuesuat = '" . $this->get_Rate() . "' where sott = '" . $this->get_SoTT() . "' ";
        $this->re_query($sql_ps);
    ...
    function ThemBangPhanBoChiPhi($tuthang, $denthang, $sophieu, $NgayCuoiThang, $khongtaobuttoandinhkhoan,$xoaphanbocp,$phanbotheo)
    {
		$GiaTriPhanBo=0;
		$TieuThucPhanBo='thanhtien';
        ...
        $sql1 = "select mavt_pbchiphi,sum(gtvnd1) as thanhtien from pskt inner join chitiet_pskt on (pskt.sophieu=chitiet_pskt.sophieu) where month(ngayghiso)='{$tuthang}' and chitiet_pskt.loaiphieu in(2,4) and mavt_pbchiphi!='' and tkno1='1562' group by mavt_pbchiphi
              ...
              union all
              select chitiet_psvt.mavt_pbchiphi,sum(thanhtien) as thanhtien from chitiet_psvt INNER JOIN dinhkhoan_psvt on (chitiet_psvt.sophieu=dinhkhoan_psvt.sophieu) INNER JOIN psvt on (psvt.sophieu=chitiet_psvt.sophieu) WHERE dinhkhoan_psvt.tkno='1562' and month(ngayghiso)='{$tuthang}' and chitiet_psvt.mavt_pbchiphi!='' and psvt.loaiphieu='3' and sotien!=0 group by chitiet_psvt.mavt_pbchiphi";
        $res = $this->re_query($sql1);
        $data_chitiet = $this->re_fetch_all($res);// Lấy tất cả giá trị chi phí mua hàng ở các phiếu
		...
		if($phanbotheo=="soluongnhap"){
			$TieuThucPhanBo = "soluongnhap";
			$GiaTriPhanBo = $tongluongnhap;
		}else{
			$TieuThucPhanBo = "thanhtien";
			$GiaTriPhanBo =$tongtiennhap;
		}
        $tongcpphanbo = 0;
        $leng = count($trees);
        $i = 0;
        ...
        foreach ($trees as $k => $itemtrees) {// Duyệt tất cả các hóa đơn có mặt hàng CP mua hàng trong tháng
            ...
            if ($leng == $i) {// Nếu là mavt cuối cùng thì phân bổ hết cho mặt hàng đó
                ...
                $cpphanbo_1mathang = ($tongcpmuahang / $GiaTriPhanBo);
                $j=0;
                ...
                foreach ($data_mavt as $item_data_mavt) {//Duyệt từng kho
                    ...
                    if ($leng_mavt_kho == $j) {// Nếu là kho hàng cuối
                        ...
                        if($xoaphanbocp=="false") {
                            // Lấy danh sách chi tiết nhập của kho hiện tại
                            $sqlchitiet_nhapkho = "SELECT mavt,chitiet_psvt.sott,psvt.kho,sum(soluongnhap) as soluongnhap,sum(thanhtien) as thanhtien,psvt.sophieu FROM psvt inner join chitiet_psvt on (psvt.sophieu = chitiet_psvt.sophieu) WHERE month(ngayghiso)='{$tuthang}' and loaiphieu='1' and mavt = '" . $itemtrees['mavt'] . "' and psvt.kho='".$item_data_mavt['kho']."' group by psvt.sophieu";
                            $reschitiet_nhapkho = $this->re_query($sqlchitiet_nhapkho);
                            $data_chitiet_nhapkho = $this->re_fetch_all($reschitiet_nhapkho);

                            $lengchitiet_nhapkho = count($data_chitiet_nhapkho);
                            $s = 0;
                            $Tongcpphanbo_tungmathang_tungkho=0;
                            foreach ($data_chitiet_nhapkho as $item_chitiet_nhapkho) {
                                $s++;
                                if ($lengchitiet_nhapkho == $s) { // nếu là mặt hàng cuối thì phân bổ hết
                                    $cpphanbo_tungmathang_tungkho = $cpphanbotungkho - $Tongcpphanbo_tungmathang_tungkho; // Số tiền phân bổ của từng hàng hóa trong tháng
                                    $this->re_query("UPDATE chitiet_psvt 
                                         SET cpmuahang =cpmuahang+" . $cpphanbo_tungmathang_tungkho . " 
                                         where sott='" . $item_chitiet_nhapkho['sott'] . "'"
                                    );// Cập nhật chi phí mua hàng mã vạt tư

                                    $Tongcpphanbo_tungmathang_tungkho+=$cpphanbo_tungmathang_tungkho;
                                } else {// Không cuối thì phân bổ bình thường

                                    $cpphanbo_tungmathang_tungkho = round(($cpphanbotungkho/$item_data_mavt['thanhtien'])*$item_chitiet_nhapkho['thanhtien']); // Số tiền phân bổ của từng hàng hóa trong tháng
                                    $this->re_query("UPDATE chitiet_psvt 
                                         SET cpmuahang =cpmuahang+" . $cpphanbo_tungmathang_tungkho . " 
                                         where sott='" . $item_chitiet_nhapkho['sott'] . "'"
                                    );// Cập nhật chi phí mua hàng mã vạt tư
                ...
                $cpphanbo = round(($tongcpmuahang / $GiaTriPhanBo) * $itemtrees[$TieuThucPhanBo]);// Chi phí phân bổ của từng mặt hàng
                $cpphanbo_1mathang = ($tongcpmuahang / $GiaTriPhanBo);
                $j=0;
                $tongcpphanbotungkho=0;
        ...
        // Lấy tổng TK 152,156
         $sql_TongTKMH = "SELECT mavt.matk,sum(cpmuahang) as tongcpmuahang FROM psvt inner join chitiet_psvt on (psvt.sophieu = chitiet_psvt.sophieu) inner join mavt on(chitiet_psvt.mavt = mavt.mavt) WHERE month(ngayghiso)='{$tuthang}' and loaiphieu='1' group by mavt.matk";
         $re_query_TongTKMH = $this->re_query($sql_TongTKMH);
        while ($data_TongTKMH = $this->re_fetch($re_query_TongTKMH)) {
            $value_pskt.= "('" . $sophieu . "','1','" . $NgayCuoiThang . "','".$data_TongTKMH['matk']."','89','" . $data_TongTKMH['tongcpmuahang'] . "'),";
            $value_chitiet_pskt.= "('1','" . $NgayCuoiThang . "','" . $data_TongTKMH['tongcpmuahang'] . "','0001','Toàn Bộ','100091','Kết chuyển chi phí mua hàng tháng $tuthang - $_SESSION[NienDo]','1562','" . $data_TongTKMH['tongcpmuahang'] . "','4','89','" . $sophieu . "',''),";
            $sophieu++;
        }// Lấy danh sách các mặt hàng phân bổ trong tháng đó
		...
		if($tongtiennhap==0 && $tongluongnhap==0){
			echo "Không có hàng hoá nhập trong tháng {$tuthang}";
		}
    }
