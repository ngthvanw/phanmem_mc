...
class danhmuccptratruoc extends database
{
    ...
    { // c?p 1 la cha
        ...
        if ($fill != "")
            $sql_w = $this->get_orderby() . " and ";
        $sql = "SELECT * FROM cptratruoc WHERE $sql_w 0=0  order by mats DESC ";
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
            $i++;
	...
	function loadTongCP_DaPhanBo_DenThang($TuThang,$DenThang){
        $trees = array();
        $sql = "SELECT mats,sum(tienno) as tongtienpb from bangpbchiphi where thang<".$DenThang." group by mats";
        $res = $this->re_query($sql);
        while ($data = $this->re_fetch($res)) {
            $trees[$data['mats']] = $data['tongtienpb'];
        }
        return $trees;
    }
    ...
    function loadSanLuong_KhauHao_TaiSan(){
        $trees = array();
        $sql = "SELECT * from sanluong_phanbo_congcu";
        $res = $this->re_query($sql);
        while ($data = $this->re_fetch($res)) {
            $trees[$data['mats']] = $data;
        }
    ...
    function thembangkhauhaotstheothang($tuthang, $denthang, $sophieu, $ngaycuoithang, $xoakhauhaodatrichtrongky, $khongtaobuttoandinhkhoan, $khauhaotheo)
    {
		...
		$data_TongCP_DaPhanBo_DenThang_TuBangPB = $this->loadTongCP_DaPhanBo_DenThang($tuthang,$denthang);
		//debug($data_SanLuong_KhauHao_TaiSan);
        if ($tuthang == 1) {
        ...
        while ($data = $this->fetch()) {
            ...
            if ($data['khauhao'] == 0 || $gtconlai <= 0) {// Nếu không phân bổ thì không cho phân bổ
				...
				if($data_SanLuong_KhauHao_TaiSan[$data['mats']]['tongcong']!=0){
					...
					$TongKH_SanLuong_Thang = (($tongkhthang*$sothangconlai))*$TyLe_SanLuong_KhauHao;
            ...
            if (number_format($tongnguyengia) != 0) {// Chỉ phân bổ khi nguyên giá công cụ >0
                $sothangdakh = $datathangdakh + 1;
                if ($sothangdakh <= $sothangconlai) { // Không được khấu hao vượt quá số tháng còn lại thì tính tháng cuối cùng
                    if ($sothangdakh == $sothangconlai && $data['khauhao'] != 0) {
						if($data_SanLuong_KhauHao_TaiSan[$data['mats']]['tongcong']!=0){
							$tongnguyengiacuakh = $data_TongCP_DaPhanBo_DenThang_TuBangPB[$data['mats']]+$tongkhthang;
						}else{
    ...
    { // c?p 1 la cha
        ...
        if ($fill != "")
            $sql_w = $this->get_orderby() . " and ";
        $sql = "SELECT * FROM sanluong_phanbo_congcu WHERE $sql_w matscha = '$parentid'  order by mats DESC ";
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
            $i++;
            $data['STT'] = $i;
            $trees[] = $data;
            $sql1 = "SELECT * FROM sanluong_phanbo_congcu WHERE $sql_w matscha ='" . $data['mats'] . "' order by mats ";
            $query = $this->re_query($sql1);
            while ($data1 = $this->re_fetch($query)) {
                if ($printto == 1) {
                    break;
                ...
                $data1['STT'] = $i;
                $trees[] = $data1;
                $sql2 = "SELECT * FROM sanluong_phanbo_congcu WHERE $sql_w matscha ='" . $data1['mats'] . "' order by mats ";
                $query1 = $this->re_query($sql2);
                while ($data2 = $this->re_fetch($query1)) {
                    if ($printto == 2) {
                        break;
    ...
    { // c?p 1 la cha
        ...
        if ($fill != "")
            $sql_w = " and ".$this->get_orderby();
        $sql = "SELECT * FROM sanluong_phanbo_congcu WHERE 0=0 $sql_w  order by mats DESC ";
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
            $i++;
