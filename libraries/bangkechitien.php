...
class bangkechitien extends database
{
    public $table = "psvt";
    public $tringorder;
    public $tringorder1;
     ...
     * @return mixed
     */
    public function getMabangke()
    {
        return $this->mabangke;
    }

    /**
     * @param mixed $mabangke
     */
    public function setMabangke($mabangke)
    {
        $this->mabangke = $mabangke;
    }

    /**
     * @return mixed
     */
    public function getHotennguoichi()
    {
        return $this->hotennguoichi;
    }

    /**
     * @param mixed $hotennguoichi
     */
    public function setHotennguoichi($hotennguoichi)
    {
     ...
     * @param mixed $bophan
     */
    public function setBophan($bophan)
    {
        $this->bophan = $bophan;
    }
     ...
     * @param mixed $lydochi
     */
    public function setLydochi($lydochi)
    {
        $this->lydochi = $lydochi;
    }
    public $hotennguoichi;
    public $bophan;
    public $lydochi;
    public $ngaychi;
    /**
 * @return mixed
 */
public function getNgaychi()
{
    return $this->ngaychi;
}/**
 * @param mixed $ngaychi
 */
public function setNgaychi($ngaychi)
{
...
public $Limit;
    ...
    public function createMaPSKT()
    {
        ...
        $sql = "select max(mabangke) as mapskt from bangkechitien where 0=0 $sql_w ";
        $this->query($sql);
        $data = $this->fetch();
        if ($data['mapskt'] != "") {
    ...
    function checkBangKeChiTien()
    {// Kiểm tra xem phiếu nhập kho có tòn tại chưa
        $sql = "select * from bangkechitien where mabangke='" . $this->getMabangke() . "'";
        $this->query($sql);
        if ($this->num_rows() > 0) {
            return TRUE;
        } else {
    ...
    {// Kiểm tra xem phiếu nhập kho có tòn tại chưa
        $sql = "select * from " . $this->table . " where seri='" . $this->getSeri() . "' and loaiphieu='" . $this->getLP() . "' and sct='" . $this->getSCT()."' and sophieu!='" . $this->getSoPhieu()."' and makh='" . $this->getMaKH()."'";
        $this->query($sql);
        if ($this->num_rows() > 0) {
            return $this->fetch();
        } else {
    ...
    {// Kiểm tra xem phiếu nhập kho có tòn tại chưa
        $sql = "select * from " . $this->table . " where seri='" . $this->getSeri() . "' and loaiphieu='" . $this->getLP() . "' and sct='" . $this->getSCT()."' and sophieu!='" . $this->getSoPhieu()."'";
        $this->query($sql);
        if ($this->num_rows() > 0) {
            return $this->fetch();
        } else {
    ...
    {// Kiểm tra xem phiếu nhập kho có tòn tại chưa
        $sql = "select * from psvt INNER JOIN dinhkhoan_psvt on(psvt.sophieu = dinhkhoan_psvt.sophieu) where  loaiphieu='" . $this->getLP() . "'  and ngayhoadon='" . $this->getNgayHD()."' and makh='" . $this->getMaKH()."' and dinhkhoan_psvt.tkco='" . $this->getMaTKCo()."' group by psvt.sophieu";
        $this->query($sql);
        if ($this->num_rows() > 0) {
    ...
    {// Kiểm tra xem phiếu nhập kho có tòn tại chưa
        $sql = "select max(sct) as sct from " . $this->table . " where loaiphieu='" . $this->getLP() . "' and seri='".$this->getSeri()."'";
        $this->query($sql);
        $data = $this->fetch();
		return $data['sct'];
    }
    ...
    {// Kiểm tra xem phiếu nhập kho có tòn tại chưa
        $sql = "select max(sct) as sct from chitiet_pskt where loaiphieu IN (1,3) and seri='".$this->getSeri()."'";
        $this->query($sql);
        $data = $this->fetch();
        return $data['sct'];
    }
    ...
    function loadListPSVT_IN()
    {
        ...
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
        $sql = "SELECT psvt.*,makho.tenkho FROM psvt left JOIN makho on (psvt.kho= makho.makho) WHERE 0=0 $sql_w  order by mapskt";
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
            $i++;
    ...
    function loadListPSVT($parentid = 0)
    {
        ...
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
       $sql = "SELECT * FROM " . $this->table . " WHERE loaiphieu='" . $this->getLP() . "' $sql_w  order by mapskt DESC ".$this->getLimit();
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
            $i++;
    ...
    function loadList_KhongChungTuGoc()
    {
        ...
        $sql = "SELECT sott,mapskt,loaiphieu,ngayghiso,seri,sct,makh,tenkh,diachi,masothue,noidung,tienhang,tienthue,tongcong,chuthich,1 as nhapxuat,0 as tentkco,capnhat_chungtugoc,chungtugoc FROM psvt WHERE chungtugoc=0 $sql_w
                UNION ALL 
                SELECT pskt.sott,pskt.mapskt,pskt.loaiphieu,pskt.ngayghiso,seri,sct,makh,tenkh,diachi,masothue,noidung1 as noidung,gtvnd1 as tienhang,gtvnd2 as tienthue,tongcong,chuthich,2 as nhapxuat,pskt.tentkco,capnhat_chungtugoc,chungtugoc from pskt inner join chitiet_pskt on(pskt.sophieu = chitiet_pskt.sophieu) WHERE chungtugoc=0 $sql_w1
 ";
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
    ...
    function loadList_CPKhongDuocTru()
    {
        ...
        $sql = "SELECT sott,mapskt,loaiphieu,ngayghiso,seri,sct,makh,tenkh,diachi,masothue,noidung,tienhang,tienthue,tongcong,chuthich,1 as nhapxuat,0 as tentkco,capnhat_chungtugoc,chungtugoc FROM psvt WHERE chiphikhongloaitru=1 $sql_w
                UNION ALL 
                SELECT pskt.sott,pskt.mapskt,pskt.loaiphieu,pskt.ngayghiso,seri,sct,makh,tenkh,diachi,masothue,noidung1 as noidung,gtvnd1 as tienhang,gtvnd2 as tienthue,tongcong,chuthich,2 as nhapxuat,pskt.tentkco,capnhat_chungtugoc,chungtugoc from pskt inner join chitiet_pskt on(pskt.sophieu = chitiet_pskt.sophieu) WHERE  chiphikhongloaitru=1 $sql_w1
 ";
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
    ...
    function loadDanhsSachPSVT($LoaiPhieu,$tuthang,$denthang)
    {
        $trees = array();
        $thangtruoc = $tuthang-1;
        $sql_max = "select max(mapskt) mapsktmax from psvt where loaiphieu='" . $LoaiPhieu . "' and  MONTH(ngayghiso) < '".$tuthang."' ";
        ...
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
        $sql = "SELECT * FROM " . $this->table . " WHERE loaiphieu='" . $LoaiPhieu . "' and MONTH(ngayghiso)>= '".$tuthang."' and MONTH(ngayghiso)<= '".$denthang."' $sql_w  order by ngayghiso,sophieu ";
        $this->query($sql);
        $i = $somax;
        $sql_update="";
        while ($data = $this->fetch()) {
            ...
            $this->re_query("UPDATE psvt SET mapskt='".$i."' WHERE sophieu='".$data['sophieu']."';");
        }
        return $trees;
    }
    function loadDanhsSachPSKT($LoaiPhieu,$tuthang,$denthang)
    {
        $trees = array();
        $thangtruoc = $tuthang;
        $sql_max = "select max(mapskt) mapsktmax from pskt where loaiphieu='" . $LoaiPhieu . "' and  MONTH(ngayghiso) < '".$tuthang."' ";
        $result_max = $this->re_query($sql_max);
        $data_max = $this->re_fetch($result_max);
        $somax = $data_max['mapsktmax'];
        ...
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
        $sql = "SELECT * FROM pskt WHERE loaiphieu='" . $LoaiPhieu . "' and MONTH(ngayghiso)>= '".$tuthang."' and MONTH(ngayghiso)<= '".$denthang."' $sql_w  order by ngayghiso,sophieu ";
        $this->query($sql);
        $i = $somax;
        $sql_update="";
        while ($data = $this->fetch()) {
            ...
            //$data['sophieu'] = $i;
            $trees[$data['sophieu']] = $i;
            $this->re_query("UPDATE pskt SET mapskt='".$i."' WHERE sophieu='".$data['sophieu']."';");
        }
        return $trees;
    }

	
	function loadListBangKeChiTien()
    {
        $trees = array();
        $fill = $this->get_orderby();
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
        $sql = "SELECT mabangke,hotennguoichi,lydochi FROM bangkechitien WHERE 0=0 $sql_w  order by mabangke DESC";
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
            $i++;
    ...
    function getBangKeChiTietChiTien()
    {
        $trees = array();
        $fill = $this->get_orderby();
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
        $sql = "SELECT * FROM chitiet_bangke_chitien WHERE 0=0 $sql_w  order by sott";
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
            $i++;
    ...
    public function getBangKeChiTien()
    {
        $sql = "select * from bangkechitien where mabangke='" . $this->getMabangke() . "'";
        $this->query($sql);
        if ($this->num_rows() == 0) {
    ...
    public function themBangKe()
    {
        $sql = "";
        $sql = "INSERT INTO bangkechitien (mabangke,hotennguoichi,bophan,lydochi,ngaychi) ";
        $sql .= "VALUES ('" . $this->getMabangke() . "','" . $this->getHotennguoichi() . "', '" . $this->getBophan() . "', '" . $this->getLydochi() . "','" . $this->getNgaychi() . "');";
        $this->query($sql);
    }

    public function themBangKeChiTiet()
    {
        $sql = "";
        $sql = "INSERT INTO chitiet_bangke_chitien (ngaychi,noidungchi,sotien,mabangke) ";
        $sql .= "VALUES ('" . $this->getNgaychi() . "','" . $this->getNoidung() . "', '" . $this->getSotien() . "', '" . $this->getMabangke() . "');";
        $this->query($sql);
    }

    public function xoabangkechitien($mabangke){
        $sql = "delete from bangkechitien WHERE mabangke = '".$mabangke."' and year(ngaychi)='0000'";
        $this->query($sql);
    }

    public function xoachitietbangke($sott){
        $sql = "delete from chitiet_bangke_chitien WHERE sott = '".$sott."'";
        $this->query($sql);
    }

    public function xoabangkechitienchitiet($ma){
        $sql = "delete from bangkechitien WHERE mabangke = '".$ma."'";
        $sql1 = "delete from chitiet_bangke_chitien WHERE mabangke = '".$ma."'";
        $this->query($sql);
        $this->query($sql1);
    }
    ...
    public function suaBangKe()
    {
       $sql = "update bangkechitien set
              hotennguoichi='" . $this->getHotennguoichi() . "',
              bophan= '" . $this->getBophan() . "',
              lydochi= '" . $this->getLydochi() . "',
              ngaychi = '" . $this->getNgaychi() . "'
              where mabangke =  '" . $this->getMabangke() . "'";
        $this->query($sql);
    }
	function ChiTietHangHoaVatTu(){//
        ...
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
		    $sql="select mavt,psvt.mapskt as sophieu,ngayghiso,ngayhoadon,sct,tenkh,soluongnhap,donggianhap as dongianhap,thanhtien,vat as soluongxuat,vat as thanhtienxuat,MONTH(ngayghiso) as thang,loaiphieu,(select tkco FROM dinhkhoan_psvt where dinhkhoan_psvt.sophieu = chitiet_psvt.sophieu limit 1) as tkdu  FROM psvt INNER JOIN chitiet_psvt on (psvt.sophieu = chitiet_psvt.sophieu) WHERE loaiphieu = 1 and ngayghiso>='".$this->getTuNgay()."' and ngayghiso<='".$this->getDenNgay()."' {$sql_w}
		UNION all 
		      select mavt,psvt.mapskt as sophieu,ngayghiso,ngayhoadon,sct,tenkh,vat as soluongnhap,donggianhap as dongianhap,vat as thanhtien,soluongnhap as soluongxuat,thanhtien as soluongxuat,MONTH(ngayghiso) as thang,loaiphieu,632 as tkdu FROM psvt INNER JOIN chitiet_psvt on (psvt.sophieu = chitiet_psvt.sophieu) WHERE loaiphieu in ('2','3') and ngayghiso>='".$this->getTuNgay()."' and ngayghiso<='".$this->getDenNgay()."' {$sql_w} order by ngayghiso,loaiphieu,sophieu";
        $this->query($sql);
        if($this->num_rows() == 0){
            return 0;
        }else{
    ...
    function LoadBangChiTietHangHoaVatTuLaiGop_CongDon($SapXep){// bảng cộng dồn xuất kho
        ...
        $sql="select maspkt,mavt,tenvt,dvt,sum(soluongxuat) as soluongxuat, sum(thanhtienxuat) as thanhtienxuat, avg(giavon) as giavon, ngayhoadon, ngayghiso,makh,tenkh, sct, seri,avg(giaban) as giaban, sum(thanhtienvon) as thanhtienvon,makho from bangchitiet_laigop where 0=0 {$sql_w} group by mavt order by {$SapXep} ";
        $this->query($sql);
        if($this->num_rows() == 0){
            return 0;
        }else{
