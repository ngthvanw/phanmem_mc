...
class makhachhang extends database
{
    ...
    function loadListMaKHNODK_W($loaitien,$tkcongno)
    {
        ...
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
        $sql = "SELECT makh.tenkh,makh.masothue,sdcn.makh,sdcn.matk,sdkno,sdkco,makh.makhcha,makh.diachi,makh.dienthoai,makh.ngaytra,makh.ghichu,sdcn.sott,loaitien,tygiapt,tygiaptr,thanhtienntpt,thanhtienntptr,phanloai FROM makh inner join sdcn on (makh.makh = sdcn.makh ) WHERE  {$sql_lt} $sql_w  order by sdcn.makh,sdcn.matk";
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
            $i++;
    ...
    { // c?p 1 la cha
        ...
        if ($fill != "")
            $sql_w = $this->get_orderby() . " and ";
        $sql = "SELECT makh,tenkh,makhcha FROM makh WHERE $sql_w makhcha = '$parentid'  order by makh ";
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
            $i++;
            $data['STT'] = $i;
            $trees[] = $data;
            $sql1 = "SELECT makh,tenkh,makhcha FROM makh WHERE $sql_w makhcha ='" . $data['makh'] . "' order by makh ";
            $query = $this->re_query($sql1);
            while ($data1 = $this->re_fetch($query)) {
                if ($printto == 1) {
                    break;
                ...
                $data1['STT'] = $i;
                $trees[] = $data1;
                $sql2 = "SELECT makh,tenkh,makhcha FROM makh WHERE $sql_w makhcha ='" . $data1['makh'] . "' order by makh ";
                $query1 = $this->re_query($sql2);
                while ($data2 = $this->re_fetch($query1)) {
                    if ($printto == 2) {
                        break;
                    ...
                    $data2['STT'] = $i;
                    $trees[] = $data2;
                    $sql3 = "SELECT makh,tenkh,makhcha FROM makh WHERE $sql_w makhcha ='" . $data2['makh'] . "' order by makh ";
                    ...
                    while ($data3 = $this->re_fetch($query3)) {
                        ...
                        $data3['STT'] = $i;
                        $trees[] = $data3;
                        $sql4 = "SELECT makh,tenkh,makhcha FROM makh WHERE $sql_w makhcha ='" . $data3['makh'] . "' order by makh ";
    ...
    { // c?p 1 la cha
        ...
        if ($fill != "")
            $sql_w = $this->get_orderby() . " and ";
        $sql = "SELECT makh.tenkh,sdcn.makh,sdcn.matk,sdkno,sdkco,makh.makhcha,makh.diachi,makh.dienthoai,makh.ngaytra,makh.ghichu,sdcn.sott,makh.masothue,loaitien,tygiapt,tygiaptr,thanhtienntpt,thanhtienntptr,phanloai from makh inner join sdcn on (makh.makh = sdcn.makh ) WHERE $sql_w makh.makhcha = '$parentid' and sdcn.matk='{$tkcongno}' and {$sql_lt}  order by sdcn.matk ASC ";
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
            $i++;
    ...
    {
        $sql = "update sdcn set sdkno='" . $this->getsdkno() . "',sdkco='" . $this->getsdkco() . "',matk='" . $this->get_MaTK() . "',tygiapt='" . $this->getTyGiaPT() . "',tygiaptr='" . $this->getTyGiaPTr() . "',thanhtienntpt='" . $this->getThanhTienNTPT() . "',thanhtienntptr ='" . $this->getThanhTienNTPTr() . "',phanloai ='" . $this->getPhanLoai() . "'
