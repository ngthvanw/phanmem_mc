...
class pskt extends database
{
    ...
    public function createMaPSKT()
    {
        ...
        $sql = "select max(mapskt) as mapskt from " . $this->table . " where $sql_w ";
        $this->query($sql);
        $data = $this->fetch();
        if ($data['mapskt'] != "") {
    ...
    function loadListPSKT_W_IN()
    {
        ...
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
        $sql = "SELECT * FROM " . $this->table . " WHERE 0=0 $sql_w  order by mapskt";
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
            $i++;
    ...
    function loadListPSKT()
    {
        ...
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
        $sql = "SELECT chitiet_pskt.*,pskt.ngayghiso,tkco,tentkco,makh,tenkh,makh_nh,tenkh_nh,diachi,masothue,tongcong,tongcongnt from pskt inner join chitiet_pskt on(pskt.sophieu = chitiet_pskt.sophieu) WHERE 0=0 $sql_w  order by sott DESC ";
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
    ...
    {// Kiểm tra xem phiếu nhập kho có tòn tại chưa
        $sql = "select * from pskt inner join chitiet_pskt on (pskt.sophieu = chitiet_pskt.sophieu) where pskt.loaiphieu='" . $this->getLP() . "' and chitiet_pskt.sct='" . $this->getSCT() . "' and pskt.sophieu!='" . $this->getSoPhieu() . "' and pskt.makh='" . $this->getMaKH() . "' and chitiet_pskt.seri='" . $this->getSeri() . "'";
        $this->query($sql);
        if ($this->num_rows() > 0) {
            return $this->fetch();
        } else {
    ...
    {// Kiểm tra xem phiếu nhập kho có tòn tại chưa
        $sql = "select * from pskt inner join chitiet_pskt on (pskt.sophieu = chitiet_pskt.sophieu) where pskt.loaiphieu='" . $this->getLP() . "' and chitiet_pskt.sct='" . $this->getSCT() . "' and pskt.sophieu!='" . $this->getSoPhieu() . "' and chitiet_pskt.seri='" . $this->getSeri() . "'";
        $this->query($sql);
        if ($this->num_rows() > 0) {
            return $this->fetch();
        } else {
    ...
    {
        $sql = "INSERT INTO " . $this->table . "(sophieu,mapskt,ngayghiso,makh,tenkh,diachi,masothue,tkco,tentkco,tongcong,loaiphieu,makh_nh,tenkh_nh,tongcongnt)
                 VALUES('" . $this->getSoPhieu() . "','" . $this->getMaPSKT() . "','" . $this->getNgay() . "','" . $this->getMaKH() . "','" . $this->getTenKH() . "','" . $this->getDiaChi() . "','" . $this->getMaSoThue() . "','" . $this->getMaTKCo() . "','" . $this->getTenTKCo() . "','" . $this->getTongCong() . "','" . $this->getLP() . "','" . $this->getMaKH3() . "','" . $this->getTenKH3() . "','" . $this->getTongCongNT() . "');";
    ...
    public function suaPSKT()
    {
       ...
       WHERE mapskt = '" . $this->getMaPSKT() . "' and  loaiphieu='" . $this->getLP() . "' ";
        $this->query($sql);
    }
    ...
    {// Kiểm tra xem phiếu nhập kho có tòn tại chưa
        $sql = "select *,(gtvnd1+gtvnd2) as tongtienphieu from pskt INNER JOIN chitiet_pskt on(pskt.sophieu = chitiet_pskt.sophieu) where  pskt.loaiphieu='" . $this->getLP() . "'  and ngayhoadon='" . $this->getNgayHD() . "' and makh='" . $this->getMaKH() . "' and pskt.tkco='" . $this->getMaTKCo() . "' group by pskt.sophieu";
        $this->query($sql);
        if ($this->num_rows() > 0) {
