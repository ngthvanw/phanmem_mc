...
class psktphieughino extends database
{
    public $tringorder;
    public $table = "pskt";
    ...
    function loadListPSKT()
    {
        ...
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
        $sql = "SELECT pskt.*,tenkh,diachi,masothue from pskt inner join makh on(pskt.makh = makh.makh) WHERE 0=0 and lp=4 $sql_w  order by sott DESC ";
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
    ...
    function loadInPhieu()// In Phiêu ghi sổ
    {
        ...
        if ($fill != "")
            $sql_w = " and " . $this->get_orderby();
        $sql = "select pskt.mapskt,pskt.date,pskt.tkno,chitiet_pskt.mand,chitiet_pskt.mand2,chitiet_pskt.gtvnd,chitiet_pskt.gtvnd2,mand.tennoidung,mand.tkco,chitiet_pskt.chuthich from pskt INNER join chitiet_pskt on (pskt.mapskt=chitiet_pskt.mapskt) INNER JOIN mand ON(chitiet_pskt.mand=mand.mand) WHERE 0=0  $sql_w  ";
        $this->query($sql);
        $i = 0;
        while ($data = $this->fetch()) {
