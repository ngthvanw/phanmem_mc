...
(function($) {
	...
	fnPG.options = {
		...
		totalRecords: 0,
		msg: "",
		rPPOptions: [10, 20, 30, 40, 50, 100,150,200],
		rPP: 20
	};
	fnPG._regional = {
		strDisplay: "Hiển thị {0} đến {1} của {2} danh sách.",
		strFirstPage: "Trang đầu",
		strLastPage: "Trang cuối",
		strNextPage: "Trang kế",
		strPage: "Trang {0} của {1}",
		strPrevPage: "Trang trước",
		strRefresh: "Refresh",
		strRpp: "Số dòng hiển thị:{0}"
	};
...
(function($) {
	...
	_pGenerateView.renderCell = function(objP) {
		...
		var str = ["<td tabindex=0 class='", cls, "' ", attr, " >", this._renderCell(objP, dataCell), "</td>"].join("");
		return str
	};
	...
	fn._editCell = function(objP) {
		...
		if (typeof edtype == "function") {
			...
			if (edtype == "checkbox") {
				...
				if (edtype == "textarea" || edtype == "select" || edtype == "textbox" || edtype == "date" ||  edtype == "number") {
					if (edtype == "textarea") {
						...
						if (edtype == "select") {
							...
							inp = "<input autocomplete='off' class='" + cls2 + "' " + attr + " " + style + " type=date name='" + dataIndx + "' />"
						}
                        else if(edtype == "number") {
                            inp = "<input autocomplete='off' class='" + cls2 + " sothapphan' " + attr + " " + style + " type=number name='" + dataIndx + "' />"
                        }
                        else {
	...
	fn._createHeader = function() {
		...
		if (depth >= 1) {
			buffer.push("<tr>");
			if (numberCell.show) {
				buffer.push("<td tabindex=1 style='width:" + (numberCell.width + 1) + "px;' ></td>")
			}
			for (var col = 0; col <= finalH; col++) {
				...
				buffer.push("<td tabindex=2 style='width:" + wd + "px;' pq-col-indx=" + col + "></td>")
			}
			buffer.push("</tr>")
		}
...
(function($) {
		...
		for (var i = 0; i <= finalH; i++) {
			...
			if (filter) {
				...
				if (type === "textbox") {
                    ...
                    cls = cls + " ui-corner-all pq-search-txt";
                    if (condition == "between") {
                        strS = this._inputdate(dataIndx, value, wdhalf, cls + " pq-from", style, attr);
                        strS += "-" + this._inputdate(dataIndx, value2, wdhalf, cls + " pq-to", style, attr)
                    } else {
                        strS = this._inputdate(dataIndx, value, wd, cls, style, attr)
                    }
                } else {
					if (type === "textarea") {
						value = value ? value : "";
    ...
    _pHeaderSearch._inputdate = function(dataIndx, value, wd, cls, style, attr) {
        return ['<input value="', value, "\" name='", dataIndx, "' type=date style='width:", wd, "px;" + style + "' class='" + cls + "' " + attr + " />"].join("")
    };
	...
	_pGenerateView._generateDetailRow = function(rowData, rowIndx, thisColModel, buffer, objP, lastFrozenRow) {
		...
		buffer.push("<td tabindex='0' class='" + const_cls + " pq-detail-child' colSpan='20'></td>");
		buffer.push("</tr>");
		return buffer
	};
								...
								if (location == "remote") {
									...
									if (typeof value == "string") {
										try {
											...
											value = new RegExp(locdau(value), modifiers);
										} catch (ex) {
											value = /.*/
										}
									}
